...
try:
    with urllib.request.urlopen(url) as r:
        ...
        print(f"Checking {len(item_ids)} items for creation date...")
        ...
        print(f"\n--- ITEMS CREATED TODAY ({len(created_today)}) ---")
        for x in sorted(created_today, key=lambda i: i["created"]):
            print(f"ID: {x['item_id']} | SKU: {x['sku']} | Created: {x['created']} | Title: {x['title']}")
