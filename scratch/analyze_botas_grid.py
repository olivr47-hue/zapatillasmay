...
try:
    with urllib.request.urlopen(url) as r:
        ...
        print(f"\n--- GRID DETAILS FOR OLD BOTAS/BOTINES ({len(results)}) ---")
        for x in sorted(results, key=lambda i: (i["grid_id"] or "", i["grid_row"] or "")):
            print(f"GridID: {x['grid_id']} | Row: {x['grid_row']} | Size: {x['size']} | SKU: {x['sku']}")
