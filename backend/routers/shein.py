...
@router.get("/test-swatch")
def test_swatch(url: str):
    """Diagnostico aislado: prueba _subir_swatch_color con una sola imagen, sin tocar publishOrEdit."""
    inicio = time.time()
    resultado = _subir_swatch_color(url)
    return {"ok": bool(resultado), "url_shein": resultado, "segundos": round(time.time() - inicio, 2)}
