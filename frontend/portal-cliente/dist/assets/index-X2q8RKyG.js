(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))e(n);new MutationObserver(n=>{for(const s of n)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&e(i)}).observe(document,{childList:!0,subtree:!0});function a(n){const s={};return n.integrity&&(s.integrity=n.integrity),n.referrerPolicy&&(s.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?s.credentials="include":n.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function e(n){if(n.ep)return;n.ep=!0;const s=a(n);fetch(n.href,s)}})();const C="/api",et="portal_sesion",yt="portal_carrito",W=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],X=document.getElementById("app"),r={sesion:null,data:null,carrito:[],tab:"tienda",filtroCat:"",busqueda:""},M=t=>parseFloat(t)||0,y=t=>"$"+Math.round(t).toLocaleString("es-MX"),p=t=>String(t??"").replace(/"/g,"&quot;").replace(/</g,"&lt;"),mt=t=>{try{const o=new URL(String(t??""),window.location.origin);return o.protocol==="http:"||o.protocol==="https:"?o.href:""}catch{return""}};function at(){var o;const t=(o=r.sesion)==null?void 0:o.token;return t?{"Content-Type":"application/json",Authorization:"Bearer "+t}:{"Content-Type":"application/json"}}function nt(t){return t&&t.status===401?(bt(),!0):!1}function St(){var t;try{r.sesion=JSON.parse(localStorage.getItem(et)||"null")}catch{r.sesion=null}try{r.carrito=JSON.parse(localStorage.getItem(yt)||"[]")}catch{r.carrito=[]}(t=r.sesion)!=null&&t.cliente_id?_t():R()}let D="telefono",A="pedir",q={};function R(){const t=D==="telefono",o=A==="codigo"?`
    <p style="text-align:center;font-size:.85rem;color:#c8a8de;margin:0 0 16px">
      Te enviamos un código por <b>${p(q.canal||(t?"WhatsApp":"correo"))}</b> a <b>${p(q.destino||"")}</b>
    ...
    </div>`,A==="pedir"){const a=document.getElementById("l-send");a&&(a.onclick=tt);const e=document.getElementById("l-dato");e&&e.addEventListener("keydown",n=>{n.key==="Enter"&&tt()})}else{const a=document.getElementById("l-verify");a&&(a.onclick=ut);const e=document.getElementById("l-codigo");e&&e.addEventListener("keydown",n=>{n.key==="Enter"&&ut()}),document.getElementById("l-resend").onclick=n=>{n.preventDefault(),tt()},document.getElementById("l-back").onclick=n=>{n.preventDefault(),A="pedir",R()}}document.getElementById("l-demo").onclick=a=>{a.preventDefault(),jt()},Tt()}window.__loginMetodo=t=>{D=t,A="pedir",R()};async function Tt(){var t;try{window._seoConfig||(window._seoConfig=await fetch(C+"/seo/config").then(e=>e.json()).catch(()=>({})));const o=(t=window._seoConfig)==null?void 0:t.google_client_id;if(!o||(await Lt(),typeof google>"u"))return;google.accounts.id.initialize({client_id:o,callback:It,auto_select:!1});const a=document.getElementById("g-login-btn");a&&google.accounts.id.renderButton(a,{theme:"outline",size:"large",width:300,locale:"es_MX",text:"continue_with"})}catch{}}function Lt(){return new Promise(t=>{var a,e;if((e=(a=window.google)==null?void 0:a.accounts)!=null&&e.id)return t();const o=document.createElement("script");o.src="https://accounts.google.com/gsi/client",o.async=!0,o.defer=!0,o.onload=t,o.onerror=t,document.head.appendChild(o)})}async function It(t){const o=t==null?void 0:t.credential;if(!o)return $("Error al conectar con Google");try{const a=await fetch(C+"/portal/login/google",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id_token:o})}),e=await a.json();if(!a.ok||!e.token)return $(e.error||"No se pudo entrar con Google");it(e)}catch{$("Error conectando con Google")}}function $(t){const o=document.getElementById("l-err");o&&(o.textContent=t,o.style.display="block")}function it(t){r.sesion={nombre:t.cliente.nombre,cliente_id:t.cliente.id,tipo:t.cliente.tipo,token:t.token,demo:!!t.demo},localStorage.setItem(et,JSON.stringify(r.sesion)),_t()}async function tt(){$("");const t=(document.getElementById("l-dato").value||"").trim();if(!t)return $(D==="telefono"?"Escribe tu teléfono":"Escribe tu correo");const o=document.getElementById("l-send");o.disabled=!0,o.textContent="Enviando...";try{const a=await fetch(C+"/portal/otp/solicitar",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({metodo:D,valor:t})}),e=await a.json();if(!a.ok){$(e.error||"No se pudo enviar el código"),o.disabled=!1,o.textContent="Enviarme un código";return}q={valor:t,destino:e.destino,canal:e.canal,enviado:e.enviado},A="codigo",R()}catch{$("Error conectando con el servidor"),o.disabled=!1,o.textContent="Enviarme un código"}}async function ut(){$("");const t=(document.getElementById("l-codigo").value||"").replace(/\D/g,"");if(t.length<4)return $("Escribe el código que te enviamos");const o=document.getElementById("l-verify");o.disabled=!0,o.textContent="Entrando...";try{const a=await fetch(C+"/portal/otp/verificar",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({metodo:D,valor:q.valor,codigo:t})}),e=await a.json();if(!a.ok||!e.token){$(e.error||"Código incorrecto"),o.disabled=!1,o.textContent="Entrar";return}it(e)}catch{$("Error conectando con el servidor"),o.disabled=!1,o.textContent="Entrar"}}async function jt(){$("");try{const t=await fetch(C+"/portal/demo-login",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}),o=await t.json();if(!t.ok||!o.token)return $(o.error==="Demo deshabilitado"?"Demo deshabilitado en el servidor (activa PORTAL_DEMO=1).":o.error||"No se pudo cargar el demo");it(o)}catch{$("Error cargando demo")}}function bt(){localStorage.removeItem(et),r.sesion=null,R()}async function _t(){X.innerHTML='<div class="spinner">Cargando catálogo…</div>';try{const[t,o,a]=await Promise.all([fetch(C+"/productos/").then(n=>n.json()),fetch(C+"/variantes/").then(n=>n.json()),fetch(C+"/inventario/").then(n=>n.json()).catch(()=>[])]),e={};(Array.isArray(a)?a:[]).forEach(n=>{e[n.variante_id]=(e[n.variante_id]||0)+(n.cantidad||0)}),r.data={productos:(Array.isArray(t)?t:[]).filter(n=>n.activo&&!/^oferta/i.test(n.nombre||"")&&!/^oferta/i.test(n.sku_interno||"")).map(n=>Pt(n)),variantes:Array.isArray(o)?o:[],stockPorVar:e},zt()}catch{X.innerHTML=`<div class="empty"><div class="ic">😕</div><p>No se pudo cargar. Reintenta.</p>
    ...
    <button data-tab="${a.id}" class="${r.tab===a.id?"active":""}" onclick="window.__nav('${a.id}')">
      ...
      <span>${a.label}</span>
    </button>`).join("")}</div>`}window.__nav=K;function K(t){var e;r.tab=t;const o=document.querySelector(".bottomnav");o&&(o.outerHTML=I());const a=document.getElementById("tb-sub");a&&(a.textContent=(e=r.sesion)!=null&&e.nombre?`Hola, ${r.sesion.nombre.split(" ")[0]}`:""),t==="inicio"?Bt():t==="tienda"?wt():t==="catalogos"?Ut():t==="carrito"?U():t==="pedidos"?Rt():t==="maya"?Vt():t==="cuenta"&&Gt()}const E=()=>document.getElementById("page");async function Bt(){var a,e;const t=N(),o=$t();E().innerHTML=`
    ${(a=r.sesion)!=null&&a.demo?`<div class="row" style="background:#fff7ed;border-color:#fed7aa;color:#9a3412;font-size:.8rem">⚠️ Modo demo: viendo datos reales de <b>${p(r.sesion.nombre)}</b> solo para revisión.</div>`:""}
    <div class="row">
    ...
    <div class="row">
      ...
      <p class="muted" style="font-size:.85rem;margin:0">Eres cliente <b>${p(((e=r.sesion)==null?void 0:e.tipo)||"mayoreo")}</b>. En el catálogo ves el precio por par, y baja automático al llegar a 3+, 6+ o corrida completa.</p>
    </div>
    ...
    </div>`}function wt(){const t=[...new Set(r.data.productos.map(o=>o.categoria).filter(Boolean))];E().innerHTML=`
    <input class="search" id="cat-search" placeholder="🔍 Buscar modelo o SKU…" value="${p(r.busqueda)}">
    <div class="chips">
    ...
    <div class="grid" id="cat-grid"></div>`,document.getElementById("cat-search").addEventListener("input",o=>{r.busqueda=o.target.value,ft()}),ft()}window.__filtro=t=>{r.filtroCat=t,wt()};function ft(){const t=r.busqueda.trim().toLowerCase();let o=r.data.productos;r.filtroCat&&(o=o.filter(e=>e.categoria===r.filtroCat)),t&&(o=o.filter(e=>(e.nombre||"").toLowerCase().includes(t)||(e.sku_interno||"").toLowerCase().includes(t)));const a=document.getElementById("cat-grid");if(a){if(!o.length){a.innerHTML='<p class="muted" style="grid-column:1/-1;text-align:center;padding:30px">Sin resultados</p>';return}a.innerHTML=o.slice(0,120).map(e=>`
    ...
    </div>`).join("")}}let w={productoId:null,color:null,modo:"variado"},k=1;window.__abrir=Ht;function At(t){const o=r.data.variantes.filter(e=>e.producto_id===t),a=[...new Set(o.map(e=>e.color).filter(Boolean))];return a.length?a.some(e=>Ot(t,e)):!1}function Ot(t,o){return F(t,o)>=1}function F(t,o){if(!o)return 0;const a=r.data.variantes.filter(s=>s.producto_id===t&&s.color===o);if(!a.length)return 0;const e=a.some(s=>s.talla&&(s.talla.includes(".5")||s.talla.includes("½")||s.talla.includes("/")));let n=0;for(let s=1;s<=6;s++)if(e){if(a.filter(u=>(r.data.stockPorVar[u.id]||0)>=s).length>=6){n=s;continue}let c=0;if(a.forEach(u=>{const f=r.data.stockPorVar[u.id]||0;c+=Math.min(f,2*s)}),c>=6*s)n=s;else break}else{let i=0;if(a.forEach(c=>{const u=r.data.stockPorVar[c.id]||0;i+=Math.min(u,2*s)}),i>=6*s)n=s;else break}return n}function rt(t,o,a){const e=r.data.variantes.filter(c=>c.producto_id===t&&c.color===o).sort((c,u)=>W.indexOf(c.talla)-W.indexOf(u.talla)),n=e.some(c=>c.talla&&(c.talla.includes(".5")||c.talla.includes("½")||c.talla.includes("/"))),s={};if(n){const c=e.filter(u=>(r.data.stockPorVar[u.id]||0)>=a);if(c.length>=6)return c.slice(0,6).forEach(u=>{s[u.id]=a}),s}let i=6*a;for(const c of e){const u=r.data.stockPorVar[c.id]||0,f=Math.min(u,2*a,i);if(f>0&&(s[c.id]=f,i-=f),i<=0)break}return s}function Ht(t){const o=r.data.productos.find(s=>s.id===t);if(!o)return;const a=r.data.variantes.filter(s=>s.producto_id===t),e=[...new Set(a.map(s=>s.color).filter(Boolean))];w={productoId:t,color:e[0]||null,modo:"variado"},k=1;const n=document.createElement("div");n.className="modal-overlay",n.id="pmodal",n.innerHTML=`
    <div class="modal">
      <div class="m-head">
        ${o.imagen_principal?`<img id="pm-img" src="${o.imagen_principal}">`:""}
        <div style="flex:1;min-width:0">
          <div style="font-weight:700;line-height:1.25">${p(o.nombre)}</div>
          <div class="muted" style="font-size:.76rem">${p(o.sku_interno||"")}</div>
      ...
      <div class="m-scroll">
        ...
        <div class="swatches" id="pm-swatches">
          ${e.map(s=>{var f;const i=a.filter(v=>v.color===s),c=i.map(v=>v.foto_url).find(Boolean),u=((f=i[0])==null?void 0:f.color_hex)||"#999";return`<div class="swatch ${s===w.color?"active":""}" data-color="${p(s)}" onclick="window.__pickColor('${p(s)}')">
    ...
    `}}window.__pickColor=t=>{w.color=t,k=1,document.querySelectorAll("#pm-swatches .swatch").forEach(n=>n.classList.toggle("active",n.dataset.color===t));const a=r.data.variantes.filter(n=>n.producto_id===w.productoId&&n.color===t).map(n=>n.foto_url).find(Boolean),e=document.getElementById("pm-img");e&&a&&(e.src=a),ht(),G()};window.__modoModal=t=>{w.modo=t,k=1,document.querySelectorAll("#pm-mode button").forEach(o=>o.classList.toggle("on",o.dataset.modo===t)),G()};function qt(){const{productoId:t,color:o}=w;return r.data.variantes.filter(a=>a.producto_id===t&&a.color===o).sort((a,e)=>W.indexOf(a.talla)-W.indexOf(e.talla))}function G(){const t=document.getElementById("pm-tallas");if(!t)return;const{color:o,modo:a,productoId:e}=w,n=r.data.productos.find(c=>c.id===e),s=qt(),i=document.getElementById("pm-tallas-label");if(a==="corrida"){const c=F(e,o);k>c&&(k=Math.max(1,c)),i&&(i.textContent=`Corrida completa — ${o} · ${y(n.precio_corrida)} x par`);const u=rt(e,o,k);t.className="corrida-rows",t.innerHTML=`
      ...
      ${s.map(f=>{const v=u[f.id]||0,l=r.data.stockPorVar[f.id]||0;return`<div class="crow ${l<=0?"off":""}">
          <span class="ct">Talla ${f.talla}</span>
          <span class="cs muted">Stock ${l}</span>
          <div class="cstep">
            <span style="font-size: 1rem; color: var(--pink); font-weight: 800;">${v} ${v===1?"par":"pares"}</span>
          </div>
        </div>`}).join("")}
    `}else i&&(i.textContent="Tallas — "+o),t.className="tallas",t.innerHTML=s.map(c=>{var v;const u=r.data.stockPorVar[c.id]||0,f=((v=r.carrito.find(l=>l.variante_id===c.id&&!l.es_corrida))==null?void 0:v.cantidad)||0;return`<button class="talla ${f?"on":""}" ${u<=0?"disabled":""} onclick="window.__addTalla('${c.id}')">
        <span class="t">${c.talla}</span>
        <span class="s">${u<=0?"Agotado":"Stock "+u}</span>
        ${f?`<span class="q">${f}</span>`:""}
      </button>`}).join("");Dt()}function Dt(){const t=document.getElementById("pm-foot");t&&(w.modo==="corrida"?t.innerHTML=`
      <button class="btn-primary" onclick="window.__agregarCorrida()">Agregar ${k} ${k===1?"corrida":"corridas"} (${k*6} pares)</button>
    `:t.innerHTML=`<button class="btn-primary" onclick="document.getElementById('pmodal').remove();window.__nav('carrito')">Listo · ver carrito</button>`)}window.__changeCorridaM=t=>{const{productoId:o,color:a}=w,e=F(o,a);k=Math.min(e,Math.max(1,k+t)),G()};window.__agregarCorrida=()=>{const{productoId:t,color:o}=w,a=r.data.productos.find(c=>c.id===t),e=rt(t,o,k);let n=0;if(Object.entries(e).forEach(([c,u])=>{if(u<=0)return;const f=r.data.variantes.find(l=>l.id===c),v=r.carrito.find(l=>l.variante_id===c&&l.es_corrida);v?v.cantidad+=u:r.carrito.push({variante_id:c,producto_id:a.id,nombre:a.nombre,color:f.color,talla:f.talla,cantidad:u,es_corrida:!0,precio_menudeo:a.precio_menudeo,precio_mayoreo3:a.precio_mayoreo3,precio_mayoreo6:a.precio_mayoreo6,precio_corrida:a.precio_corrida,imagen:f.foto_url||a.imagen_principal||null}),n+=u}),!n){S("Elige al menos una corrida");return}k=1,z();const s=document.querySelector(".bottomnav");s&&(s.outerHTML=I());const i=document.getElementById("pmodal");i&&i.remove(),K("carrito"),S("Corrida agregada · "+n+" pares")};window.__addTalla=t=>{const o=r.data.variantes.find(i=>i.id===t),a=r.data.productos.find(i=>i.id===o.producto_id),e=r.data.stockPorVar[t]||0,n=r.carrito.find(i=>i.variante_id===t&&!i.es_corrida);if(((n==null?void 0:n.cantidad)||0)>=e){S("Sin más stock ("+e+")");return}n?n.cantidad+=1:r.carrito.push({variante_id:t,producto_id:a.id,nombre:a.nombre,color:o.color,talla:o.talla,cantidad:1,es_corrida:!1,precio_menudeo:a.precio_menudeo,precio_mayoreo3:a.precio_mayoreo3,precio_mayoreo6:a.precio_mayoreo6,precio_corrida:a.precio_corrida,imagen:o.foto_url||a.imagen_principal||null}),z(),G(),document.querySelector(".bottomnav").outerHTML=I(),S("Agregado · "+N()+" pares")};function N(){return r.carrito.reduce((t,o)=>t+o.cantidad,0)}function st(){return r.carrito.filter(t=>!t.es_corrida).reduce((t,o)=>t+o.cantidad,0)}function ot(t,o){return t.es_corrida?t.precio_corrida:o>=6?t.precio_mayoreo6:o>=3?t.precio_mayoreo3:t.precio_menudeo}function $t(){const t=st();return r.carrito.reduce((o,a)=>o+a.cantidad*ot(a,t),0)}function z(){try{localStorage.setItem(yt,JSON.stringify(r.carrito))}catch{}}function Nt(){const t=st();return r.carrito.every(o=>o.es_corrida)&&r.carrito.length?"Corrida":t>=6?"Mayoreo 6+":t>=3?"Mayoreo 3+":"Menudeo"}function U(){if(!r.carrito.length){E().innerHTML=`<div class="empty"><div class="ic">🛒</div><p>Tu carrito está vacío</p>
      <button class="btn-primary" style="max-width:220px;margin:14px auto 0" onclick="window.__nav('tienda')">Ver catálogo</button></div>`;return}const t=st(),o=$t(),a=r.carrito.filter(i=>!i.es_corrida),e=r.carrito.filter(i=>i.es_corrida),n={};e.forEach(i=>{const c=`${i.producto_id}-${i.color}`;n[c]||(n[c]={key:c,es_grupo_corrida:!0,producto_id:i.producto_id,color:i.color,nombre:i.nombre,imagen:i.imagen,precio_corrida:i.precio_corrida,cantidad:0,items:[]}),n[c].cantidad+=i.cantidad,n[c].items.push(i)});const s=[...a.map((i,c)=>({...i,originalIdx:r.carrito.indexOf(i)})),...Object.values(n)];E().innerHTML=`
    ...
    ${s.map(i=>i.es_grupo_corrida?`
          <div class="row">
            <div class="r-top">
              ...
              <div style="flex:1;min-width:0">
                <div style="font-weight:600;font-size:.9rem">${p(i.nombre)}</div>
                <div class="muted" style="font-size:.78rem">${p(i.color)} · Corrida</div>
              ...
              <button onclick="window.__delGrupoCorrida('${i.producto_id}', '${p(i.color)}')" style="background:none;border:none;color:#ccc;font-size:1.2rem">✕</button>
            </div>
            <div class="qty" style="margin-top:10px">
              <button onclick="window.__qtyGrupoCorrida('${i.producto_id}', '${p(i.color)}', -1)">−</button>
        ...
        `:`
          <div class="row">
            <div class="r-top">
              ...
              <div style="flex:1;min-width:0">
                <div style="font-weight:600;font-size:.9rem">${p(i.nombre)}</div>
                <div class="muted" style="font-size:.78rem">${p(i.color)} · T${p(i.talla)}</div>
    ...
    <div class="sticky-total">
    ...
    </div>`}window.__qty=(t,o)=>{const a=r.carrito[t];if(a){if(o>0){const e=r.data.stockPorVar[a.variante_id]||0;if(r.carrito.filter(s=>s.variante_id===a.variante_id).reduce((s,i)=>s+i.cantidad,0)>=e){S("Sin más stock ("+e+")");return}}a.cantidad=Math.max(1,a.cantidad+o),z(),U(),document.querySelector(".bottomnav").outerHTML=I()}};window.__delItem=t=>{r.carrito.splice(t,1),z(),U(),document.querySelector(".bottomnav").outerHTML=I()};window.__qtyGrupoCorrida=(t,o,a)=>{const i=r.carrito.filter(l=>l.producto_id===t&&l.color===o&&l.es_corrida).reduce((l,b)=>l+b.cantidad,0)/6+a;if(i<=0){window.__delGrupoCorrida(t,o);return}const c=F(t,o);if(i>c){S(`Sin existencias para ${i} corridas (máximo ${c})`);return}const u=rt(t,o,i);r.carrito=r.carrito.filter(l=>!(l.producto_id===t&&l.color===o&&l.es_corrida));const f=r.data.productos.find(l=>l.id===t);Object.entries(u).forEach(([l,b])=>{if(b<=0)return;const T=r.data.variantes.find(V=>V.id===l);r.carrito.push({variante_id:l,producto_id:f.id,nombre:f.nombre,color:T.color,talla:T.talla,cantidad:b,es_corrida:!0,precio_menudeo:f.precio_menudeo,precio_mayoreo3:f.precio_mayoreo3,precio_mayoreo6:f.precio_mayoreo6,precio_corrida:f.precio_corrida,imagen:T.foto_url||f.imagen_principal||null})}),z(),U();const v=document.querySelector(".bottomnav");v&&(v.outerHTML=I())};window.__delGrupoCorrida=(t,o)=>{r.carrito=r.carrito.filter(e=>!(e.producto_id===t&&e.color===o&&e.es_corrida)),z(),U();const a=document.querySelector(".bottomnav");a&&(a.outerHTML=I())};window.__enviarPedido=async()=>{var t;if(r.carrito.length){if((t=r.sesion)!=null&&t.demo){S("Modo demo: no se envía pedido real");return}if(confirm("¿Enviar este pedido al negocio? Te confirmarán existencia y total."))try{const o=await fetch(C+"/portal/carrito",{method:"POST",headers:at(),body:JSON.stringify({items:r.carrito.map(e=>({variante_id:e.variante_id,cantidad:e.cantidad,es_corrida:!!e.es_corrida}))})});if(nt(o))return;const a=await o.json();if(!o.ok||!a.ok)throw new Error(a.error||"");r.carrito=[],z(),S(a.fusionado?"Se agregó a tu carrito anterior":"¡Pedido enviado!"),K("pedidos")}catch{alert("No se pudo enviar el pedido. Intenta de nuevo.")}}};async function Rt(){E().innerHTML='<div class="spinner">Cargando pedidos…</div>';let t=[];try{const o=await fetch(C+"/portal/pedidos",{headers:at()});if(nt(o))return;t=await o.json()}catch{}if(t=Array.isArray(t)?t.filter(o=>o.status!=="borrador"):[],!t.length){E().innerHTML='<div class="empty"><div class="ic">🛍️</div><p>Aún no tienes pedidos confirmados</p></div>';return}E().innerHTML=t.map(o=>{const a=o.pedido_items||[],e=a.reduce((s,i)=>s+(i.cantidad||0),0),n=o.created_at?new Date(o.created_at).toLocaleDateString("es-MX",{day:"2-digit",month:"short",year:"numeric"}):"";return`<div class="row">
      ...
      ${a.slice(0,4).map(s=>{var i,c;return`<div class="muted" style="font-size:.78rem;margin-top:4px">• ${p(((c=(i=s.variantes)==null?void 0:i.productos)==null?void 0:c.nombre)||s.nombre||"Producto")} ${s.color?"· "+p(s.color):""} T${p(s.talla||"")} ×${s.cantidad}</div>`}).join("")}
      ...
      ${o.numero_guia?`<div style="margin-top:10px;padding-top:10px;border-top:1px solid var(--line)">
        <div class="muted" style="font-size:.74rem">📦 ${p(o.paqueteria||"Envío")} · Guía ${p(o.numero_guia)}</div>
        ${mt(o.tracking_url)?`<a href="${p(mt(o.tracking_url))}" target="_blank" rel="noopener noreferrer" style="color:var(--pink);font-size:.8rem;font-weight:600;text-decoration:none">Rastrear envío →</a>`:""}
      </div>`:""}
    </div>`}).join("")}function Ft(t){const o={borrador:["#eef2ff","#3730a3","Borrador"],pendiente_pago:["#fffbeb","#b45309","Por pagar"],confirmado:["#ecfdf5","#065f46","Confirmado"],pagado:["#ecfdf5","#065f46","Pagado"],enviado:["#eff6ff","#1d4ed8","Enviado"],entregado:["#f0fdf4","#15803d","Entregado"],cancelado:["#fef2f2","#991b1b","Cancelado"]},[a,e,n]=o[t]||["#f3f4f6","#374151",t||"—"];return`<span class="badge-status" style="background:${a};color:${e}">${n}</span>`}async function Gt(){E().innerHTML='<div class="spinner">Cargando…</div>';let t=null;try{const o=await fetch(C+"/portal/me",{headers:at()});if(nt(o))return;t=await o.json()}catch{}t=t||{nombre:r.sesion.nombre,tipo:r.sesion.tipo},E().innerHTML=`
    <div class="row">
      <p class="section-title">Mis datos</p>
      <p style="font-weight:700;margin:0 0 2px">${p(t.nombre||"")}</p>
      <p class="muted" style="font-size:.84rem;margin:0">${p(t.telefono||"Sin teléfono")}</p>
      <p class="muted" style="font-size:.84rem;margin:2px 0 0">${p(t.email||"Sin correo")}</p>
      <p style="margin:10px 0 0"><span class="badge-status" style="background:#fce4f3;color:var(--pink)">Cliente ${p(t.tipo||"mayoreo")}</span></p>
    </div>
    ${t.direccion||t.ciudad?`<div class="row">
      <p class="section-title">Dirección</p>
      <p class="muted" style="font-size:.84rem;margin:0">${p(t.direccion||"")} ${p(t.ciudad||"")} ${p(t.estado||"")} ${p(t.codigo_postal||"")}</p>
    </div>`:""}
    ...
    <p class="muted" style="text-align:center;font-size:.7rem;margin-top:16px">Portal mayoreo · prototipo</p>`}window.__salir=bt;async function Ut(){E().innerHTML='<div class="spinner">Cargando catálogos…</div>';let t=[];try{t=await fetch(C+"/catalogos/").then(e=>e.json())}catch{}t=Array.isArray(t)?t:[];const o=[["tacones","👠 Tacones"],["sandalias","👡 Sandalias"],["flats","🥿 Flats"],["botas","🥾 Botas"],["botines","👢 Botines"],["plataformas","⬆️ Plataformas"],["tenis","👟 Tenis"],["nina","🎀 Niña"],["accesorios","👜 Accesorios"]],a=t.length?t.map(e=>`
    <div class="cat-card">
    ...
    <div class="modal">
      ...
      <div class="m-foot"><button class="btn-primary" onclick="window.__descargarCatalogo('${t}','${p(o)}',this)">⬇ Descargar todas las páginas</button></div>
    </div>`,document.body.appendChild(e),e.addEventListener("click",n=>{n.target===e&&e.remove()})};window.__descargarCatalogo=async(t,o,a)=>{const e=a?a.textContent:"";a&&(a.disabled=!0,a.textContent="Descargando…");try{const n=await kt(t);if(!n.length){S("Este catálogo no tiene páginas"),a&&(a.disabled=!1,a.textContent=e);return}const s=(o||"catalogo").replace(/[^\w]+/g,"_");let i=0;for(const c of n){i++;try{const f=await(await fetch(c.imagen_url)).blob(),v=URL.createObjectURL(f),l=document.createElement("a");l.href=v,l.download=`${s}_${String(c.pagina_numero||i).padStart(2,"0")}.jpg`,document.body.appendChild(l),l.click(),l.remove(),URL.revokeObjectURL(v),await new Promise(b=>setTimeout(b,300))}catch{window.open(c.imagen_url,"_blank")}}S("Descarga lista · "+n.length+" páginas")}catch{S("No se pudo descargar")}a&&(a.disabled=!1,a.textContent=e)};function Vt(){r.mayaMsgs||(r.mayaMsgs=[]),E().innerHTML=`
    <div class="chat-wrap">
      <div class="chat-msgs" id="maya-msgs">
        ${r.mayaMsgs.length?r.mayaMsgs.map(Ct).join(""):'<div class="chat-intro">👋 ¡Hola! Soy <b>Maya</b>, tu asesora de Zapatillas&nbsp;May.<br>Pregúntame por modelos, precios de mayoreo, envíos o lo que necesites.</div>'}
      </div>
      <div class="chat-input">
        <input id="maya-in" placeholder="Escribe tu mensaje…" autocomplete="off">
        <button id="maya-send">➤</button>
      </div>
    </div>`,document.getElementById("maya-send").onclick=window.__mayaSend,document.getElementById("maya-in").addEventListener("keydown",t=>{t.key==="Enter"&&window.__mayaSend()}),setTimeout(()=>{const t=document.getElementById("maya-msgs");t&&(t.scrollTop=t.scrollHeight)},50)}function Ct(t){if(t.typing)return'<div class="msg bot typing">Maya está escribiendo…</div>';const o=(t.fotos||[]).map(a=>`<img src="${p(a)}" loading="lazy">`).join("");return`<div class="msg ${t.role==="me"?"me":"bot"}">${p(t.content)}${o}</div>`}function gt(){const t=document.getElementById("maya-msgs");t&&(t.innerHTML=r.mayaMsgs.map(Ct).join(""),t.scrollTop=t.scrollHeight)}function Jt(t){const o=[];let a=String(t||"").replace(/ENVIAR_FOTO:\[?(\S+?)\]?(?=\s|$)/g,(e,n)=>(o.push(n.replace(/[\[\]]/g,"")),""));return a=a.replace(/BUSCAR_COLORES:\[?[A-Za-z0-9_\-]+\]?/g,"").replace(/\n{3,}/g,`

`).trim(),{texto:a||"👍",fotos:o}}window.__mayaSend=async()=>{const t=document.getElementById("maya-in");if(!t)return;const o=(t.value||"").trim();if(!o)return;t.value="",r.mayaMsgs||(r.mayaMsgs=[]);const a=r.mayaMsgs.filter(e=>!e.typing).map(e=>({role:e.role==="me"?"user":"assistant",content:e.content}));r.mayaMsgs.push({role:"me",content:o}),r.mayaMsgs.push({role:"bot",typing:!0}),gt();try{const n=await(await fetch(C+"/chatbot/mensaje",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({mensaje:o,historial:a})})).json();r.mayaMsgs.pop();const{texto:s,fotos:i}=Jt(n.respuesta);r.mayaMsgs.push({role:"bot",content:s,fotos:i})}catch{r.mayaMsgs.pop(),r.mayaMsgs.push({role:"bot",content:"Ups, no me pude conectar. Intenta de nuevo en un momento."})}gt()};let vt=null;function S(t){const o=document.getElementById("toast");o&&(o.textContent=t,o.classList.add("show"),clearTimeout(vt),vt=setTimeout(()=>o.classList.remove("show"),1600))}St();
