(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))o(r);new MutationObserver(r=>{for(const n of r)if(n.type==="childList")for(const i of n.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function a(r){const n={};return r.integrity&&(n.integrity=r.integrity),r.referrerPolicy&&(n.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?n.credentials="include":r.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function o(r){if(r.ep)return;r.ep=!0;const n=a(r);fetch(r.href,n)}})();const h="/api",gt="portal_sesion",Mt="portal_carrito",dt=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],lt=document.getElementById("app"),s={sesion:null,data:null,carrito:[],tab:localStorage.getItem("pc_active_tab")||"tienda",filtroCat:"",busqueda:"",modoCompartir:!1,compartirSeleccion:[]},O=t=>parseFloat(t)||0,$=t=>"$"+Math.round(t).toLocaleString("es-MX"),f=t=>String(t??"").replace(/"/g,"&quot;").replace(/</g,"&lt;"),xt=t=>{try{const e=new URL(String(t??""),window.location.origin);return e.protocol==="http:"||e.protocol==="https:"?e.href:""}catch{return""}};function A(){var e;const t=(e=s.sesion)==null?void 0:e.token;return t?{"Content-Type":"application/json",Authorization:"Bearer "+t}:{"Content-Type":"application/json"}}function yt(t){return t&&t.status===401?(Tt(),!0):!1}function Bt(){var t;try{s.sesion=JSON.parse(localStorage.getItem(gt)||"null")}catch{s.sesion=null}try{s.carrito=JSON.parse(localStorage.getItem(Mt)||"[]")}catch{s.carrito=[]}(t=s.sesion)!=null&&t.cliente_id?Lt():ot()}let tt="telefono",V="pedir",K={};function ot(){const t=tt==="telefono",e=V==="codigo"?`
    ...
    <div class="row">
      ...
      ${t?`<p style="font-size:1.5rem;font-weight:800;color:var(--pink);margin:0">${$(e)} <small class="muted" style="font-size:.8rem;font-weight:600">· ${t} pares</small></p>
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;width:100%">
      ...
      <button onclick="pcToggleModoCompartir()" style="padding:6px 12px;border-radius:100px;border:1px solid ${s.modoCompartir?"#25D366":"#d1d5db"};background:${s.modoCompartir?"#25D366":"#f3f4f6"};color:${s.modoCompartir?"white":"#374151"};font-weight:700;font-size:0.75rem;cursor:pointer;display:flex;align-items:center;gap:4px">📲 ${s.modoCompartir?"Listo":"Compartir fotos"}</button>
    </div>
...
`}),navigator.share&&navigator.canShare&&navigator.canShare({files:r}))await navigator.share({files:r,title:"Modelos de Zapatillas May"});else{const i=`https://wa.me/?text=${encodeURIComponent(n)}`;window.open(i,"_blank"),alert("Tu navegador no permite compartir archivos directamente. Se abrirá WhatsApp con los detalles y se descargarán las imágenes seleccionadas a tu dispositivo para que las subas.");for(let c=0;c<r.length;c++){const d=document.createElement("a");d.href=URL.createObjectURL(r[c]),d.download=r[c].name,document.body.appendChild(d),d.click(),document.body.removeChild(d),await new Promise(l=>setTimeout(l,400))}}}catch(o){alert("Error al compartir: "+o.message)}finally{t.innerHTML=e,t.disabled=!1,window.pcCancelarSeleccionCompartir()}};let S={productoId:null,color:null,modo:"variado"},z=1;window.__abrir=jt;function Ut(t){const e=s.data.variantes.filter(o=>o.producto_id===t),a=[...new Set(e.map(o=>o.color).filter(Boolean))];return a.length?a.some(o=>Gt(t,o)):!1}function Gt(t,e){return it(t,e)>=1}function it(t,e){if(!e)return 0;const a=s.data.variantes.filter(n=>n.producto_id===t&&n.color===e);if(!a.length)return 0;const o=a.some(n=>n.talla&&(n.talla.includes(".5")||n.talla.includes("½")||n.talla.includes("/")));let r=0;for(let n=1;n<=6;n++)if(o){if(a.filter(d=>(s.data.stockPorVar[d.id]||0)>=n).length>=6){r=n;continue}let c=0;if(a.forEach(d=>{const l=s.data.stockPorVar[d.id]||0;c+=Math.min(l,2*n)}),c>=6*n)r=n;else break}else{let i=0;if(a.forEach(c=>{const d=s.data.stockPorVar[c.id]||0;i+=Math.min(d,2*n)}),i>=6*n)r=n;else break}return r}function zt(t,e,a){const o=s.data.variantes.filter(c=>c.producto_id===t&&c.color===e).sort((c,d)=>dt.indexOf(c.talla)-dt.indexOf(d.talla)),r=o.some(c=>c.talla&&(c.talla.includes(".5")||c.talla.includes("½")||c.talla.includes("/"))),n={};if(r){const c=o.filter(d=>(s.data.stockPorVar[d.id]||0)>=a);if(c.length>=6)return c.slice(0,6).forEach(d=>{n[d.id]=a}),n}let i=6*a;for(const c of o){const d=s.data.stockPorVar[c.id]||0,l=Math.min(d,2*a,i);if(l>0&&(n[c.id]=l,i-=l),i<=0)break}return n}function jt(t){const e=s.data.productos.find(n=>n.id===t);if(!e)return;const a=s.data.variantes.filter(n=>n.producto_id===t),o=[...new Set(a.map(n=>n.color).filter(Boolean))];S={productoId:t,color:o[0]||null,modo:"variado"},z=1;const r=document.createElement("div");r.className="modal-overlay",r.id="pmodal",r.innerHTML=`
    <div class="modal">
      <div class="m-head">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-weight:800;color:var(--pink);margin-top:3px">${$(e.precio_menudeo)} <span class="muted" style="font-size:.7rem;font-weight:600">x par</span></div>
          <div class="tier-row" style="margin-top:5px">
            <span class="tier">3+ ${$(e.precio_mayoreo3)}</span>
            <span class="tier">6+ ${$(e.precio_mayoreo6)}</span>
            <span class="tier corr" id="pm-tier-corr">Corrida ${$(e.precio_corrida)}</span>
          </div>
        </div>
    ...
    `}else i&&(i.textContent="Tallas — "+e),t.className="tallas",t.innerHTML=n.map(c=>{var v;const d=s.data.stockPorVar[c.id]||0,l=((v=s.carrito.find(m=>m.variante_id===c.id&&!m.es_corrida))==null?void 0:v.cantidad)||0;return`<button class="talla ${l?"on":""}" ${d<=0?"disabled":""} onclick="window.__addTalla('${c.id}')">
          ...
          <div class="row">
            <div class="r-top">
              ...
              <div style="flex:1;min-width:0">
                ...
                <div style="font-weight:700;color:var(--pink);margin-top:3px">${$(i.precio_corrida)} <span class="muted" style="font-size:.7rem;font-weight:600">x par</span></div>
              </div>
            ...
            <div class="qty" style="margin-top:10px">
              ...
              <button onclick="window.__qtyGrupoCorrida('${i.producto_id}', '${f(i.color)}', 1)">+</button>
              <span style="margin-left:auto;font-weight:700">${$(i.cantidad*i.precio_corrida)}</span>
            </div>
          </div>
        `:`
          <div class="row">
            <div class="r-top">
              ...
              <div style="flex:1;min-width:0">
                ...
                <div style="font-weight:700;color:var(--pink);margin-top:3px">${$(ft(i,t))} <span class="muted" style="font-size:.7rem;font-weight:600">x par</span></div>
              </div>
            ...
            <div class="qty" style="margin-top:10px">
              ...
              <span style="font-weight:700;min-width:26px;text-align:center">${i.cantidad}</span>
              <button onclick="window.__qty(${i.originalIdx},1)">+</button>
              <span style="margin-left:auto;font-weight:700">${$(i.cantidad*ft(i,t))}</span>
            </div>
          </div>
        `).join("")}
    ...
    <div class="sticky-total">
      ...
      <div style="font-weight:800;color:var(--pink);font-size:1.1rem">${$(O(e.total))}</div>
    ...
    ${O(t.limite_credito)||O(t.credito_disponible)?`<div class="row">
      ...
      ${O(t.limite_credito)?`<p style="font-size:.86rem;margin:0">Límite: <b>${$(O(t.limite_credito))}</b>${t.dias_credito?" · "+t.dias_credito+" días":""}</p>`:""}
      ${O(t.credito_disponible)?`<p style="font-size:.86rem;margin:4px 0 0">Saldo a favor: <b>${$(O(t.credito_disponible))}</b></p>`:""}
    </div>`:""}
    ...
    </div>`}window.__switchSubTab=t=>{const e=document.getElementById("catalogos-general-content"),a=document.getElementById("catalogos-categorias-content"),o=document.getElementById("btn-cat-general"),r=document.getElementById("btn-cat-categorias");t==="categorias"?(e&&(e.style.display="none"),a&&(a.style.display="block"),o&&o.classList.remove("active"),r&&r.classList.add("active")):(e&&(e.style.display="block"),a&&(a.style.display="none"),o&&o.classList.add("active"),r&&r.classList.remove("active"))};window.__descargarPdfCategorias=async t=>{const e=t.getAttribute("data-categoria"),a=t.getAttribute("data-label"),o=document.getElementById("cat-pdf-msg"),r=document.querySelectorAll("#cat-pdf-btns button");r.forEach(n=>n.disabled=!0),o&&(o.style.display="block",o.textContent=`⏳ Cargando productos de ${a}...`);try{const n=s.data.productos.filter(u=>u.categoria&&String(u.categoria).trim().toLowerCase()===String(e).trim().toLowerCase());if(!n.length){o&&(o.textContent=`Sin productos activos en ${a}`),setTimeout(()=>{o&&(o.style.display="none")},3e3),r.forEach(u=>u.disabled=!1);return}const i=[];if(n.forEach(u=>{const C=s.data.variantes.filter(g=>g.producto_id===u.id&&g.activa!==!1),b=new Set;C.forEach(g=>{if(!g.color||b.has(g.color.trim().toUpperCase()))return;b.add(g.color.trim().toUpperCase());const L=g.foto_url||u.imagen_principal;L&&i.push({sku:u.sku_interno||"S/K",color:g.color.trim().toUpperCase(),imgUrl:L})}),b.size===0&&u.imagen_principal&&i.push({sku:u.sku_interno||"S/K",color:"ÚNICO",imgUrl:u.imagen_principal})}),!i.length){o&&(o.textContent=`Sin fotos disponibles para ${a}`),setTimeout(()=>{o&&(o.style.display="none")},3e3),r.forEach(u=>u.disabled=!1);return}o&&(o.textContent=`✏️ Generando catálogo PDF (${i.length} variantes)...`),window.jspdf||await new Promise((u,C)=>{const b=document.createElement("script");b.src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js",b.onload=u,b.onerror=C,document.head.appendChild(b)});const{jsPDF:c}=window.jspdf,d=2,l=3,v=d*l,m=1080,_=1440,k=40,D=80,p=24,x=32,w=(m-k*2-p*(d-1))/d,E=(_-D*2-x*(l-1))/l,I=E-50,X=u=>new Promise(C=>{if(!u)return C(null);const b=new Image;b.crossOrigin="anonymous",b.onload=()=>C(b),b.onerror=()=>C(null),b.src=u+(u.includes("?")?"&":"?")+"_t="+Date.now()}),rt=(u,C,b,g,L,M)=>{if(u.save(),u.fillStyle="#FFFFFF",u.fillRect(b,g,L,M),C){u.beginPath(),u.rect(b,g,L,M),u.clip();const F=C.naturalWidth/C.naturalHeight,ut=L/M;let Y,H,U,Z;F>ut?(H=M,Y=M*F,Z=g,U=b+(L-Y)/2):(Y=L,H=L/F,U=b,Z=g+(M-H)/2),u.drawImage(C,U,Z,Y,H)}else u.fillStyle="#F3F4F6",u.fillRect(b,g,L,M),u.fillStyle="#9CA3AF",u.font="20px sans-serif",u.textAlign="center",u.fillText("Sin imagen",b+L/2,g+M/2);u.restore()},N=[];for(let u=0;u<i.length;u+=v)N.push(i.slice(u,u+v));const y=new c({orientation:"portrait",unit:"px",format:[m,_]});let P=!0;for(let u=0;u<N.length;u++){o&&(o.textContent=`✏️ Generando página ${u+1} de ${N.length}...`);const C=N[u],b=document.createElement("canvas");b.width=m,b.height=_;const g=b.getContext("2d");g.fillStyle="#FAFAF8",g.fillRect(0,0,m,_),g.fillStyle="#2A1A0E",g.font="300 20px sans-serif",g.textAlign="center",g.letterSpacing="4px",g.fillText(`CATÁLOGO DE ${a.toUpperCase()}`,m/2,38),g.letterSpacing="0px",g.fillStyle="#C8967A",g.fillRect(k,48,m-k*2,1.5);for(let M=0;M<C.length;M++){const F=C[M],ut=M%d,Y=Math.floor(M/d),H=k+ut*(w+p),U=D+Y*(E+x),Z=await X(F.imgUrl);rt(g,Z,H,U,w,I),g.fillStyle="#E8DDD5",g.fillRect(H,U+I,w,1),g.fillStyle="#2A1A0E",g.textAlign="center",g.font="600 18px sans-serif",g.fillText(`${F.sku} ${F.color}`,H+w/2,U+I+28)}g.fillStyle="#C8967A",g.fillRect(k,_-48,m-k*2,1),g.fillStyle="#A07860",g.font="300 14px sans-serif",g.textAlign="center",g.fillText(`Página ${u+1} de ${N.length}`,m/2,_-30);const L=b.toDataURL("image/jpeg",.9);P||y.addPage([m,_]),y.addImage(L,"JPEG",0,0,m,_),P=!1}const st=`catalogo_${e}_${new Date().toISOString().slice(0,10)}.pdf`;y.save(st),o&&(o.textContent="✅ ¡Catálogo descargado!",setTimeout(()=>{o.style.display="none"},4e3))}catch(n){console.error("Error generating PDF:",n),o&&(o.textContent=`❌ Error al generar el PDF: ${n.message}`)}finally{r.forEach(n=>n.disabled=!1)}};async function Pt(t){const e=await fetch(h+"/catalogos/"+t+"/paginas").then(a=>a.json()).catch(()=>[]);return Array.isArray(e)?e:[]}window.__verCatalogo=async(t,e)=>{const a=await Pt(t),o=document.createElement("div");o.className="modal-overlay",o.id="catv",o.innerHTML=`
    <div class="modal">
      <div class="m-head">
...
`).trim(),{texto:a||"👍",fotos:e}}window.__mayaSend=async()=>{const t=document.getElementById("maya-in");if(!t)return;const e=(t.value||"").trim();if(!e)return;t.value="",s.mayaMsgs||(s.mayaMsgs=[]);const a=s.mayaMsgs.filter(o=>!o.typing).map(o=>({role:o.role==="me"?"user":"assistant",content:o.content}));s.mayaMsgs.push({role:"me",content:e}),s.mayaMsgs.push({role:"bot",typing:!0}),St();try{const r=await(await fetch(h+"/chatbot/mensaje",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({mensaje:e,historial:a})})).json();s.mayaMsgs.pop();const{texto:n,fotos:i}=ne(r.respuesta);s.mayaMsgs.push({role:"bot",content:n,fotos:i})}catch{s.mayaMsgs.pop(),s.mayaMsgs.push({role:"bot",content:"Ups, no me pude conectar. Intenta de nuevo en un momento."})}St()};let Et=null;function j(t){const e=document.getElementById("toast");e&&(e.textContent=t,e.classList.add("show"),clearTimeout(Et),Et=setTimeout(()=>e.classList.remove("show"),1600))}window.abrirLightboxPC=function(t,e,a){if(!t)return;const o=document.getElementById("pc-lightbox");if(o&&o.remove(),!e||!e.length){const p=document.getElementById("pm-swatches");p&&(e=Array.from(p.querySelectorAll("img")).map(x=>x.src)),(!e||!e.length)&&(e=[t]),e.includes(t)||(e=[t,...e])}let r=e.indexOf(t);r<0&&(r=0);const n=document.createElement("div");n.id="pc-lightbox",n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:19999;display:flex;flex-direction:column;align-items:center;justify-content:center;touch-action:none;user-select:none";let i=1,c=0,d=0,l=1,v=0;const m=()=>{i=1,c=0,d=0;const p=document.getElementById("lb-img");p&&(p.style.transform="",p.style.cursor="zoom-in")},_=()=>{const p=e.length;n.innerHTML=`
      ...
      ${p>1?`<div style="display:flex;gap:6px;margin-top:14px;z-index:2">${e.map((x,w)=>`<div onclick="lbGoto(${w})" style="width:${w===r?"22px":"8px"};height:8px;border-radius:4px;background:${w===r?"#E91E8C":"rgba(255,255,255,0.3)"};cursor:pointer;transition:all 0.2s"></div>`).join("")}</div>`:""}
    `};window.pcCompartirImagenLB=async()=>{const p=e[r];if(p)try{const w=await(await fetch(p)).blob(),E=(w.type.split("/")[1]||"jpg").replace("jpeg","jpg"),I=new File([w],`zapatillasmay-${Date.now()}.${E}`,{type:w.type});navigator.canShare&&navigator.canShare({files:[I]})?await navigator.share({files:[I]}):navigator.share?await navigator.share({url:p}):window.pcDescargarImagenLB()}catch(x){(x==null?void 0:x.name)!=="AbortError"&&j("No se pudo compartir la imagen")}},window.pcDescargarImagenLB=async()=>{const p=e[r];if(p)try{const w=await(await fetch(p)).blob(),E=document.createElement("a");E.href=URL.createObjectURL(w);const I=a?a.replace(/[^A-Za-z0-9_-]/g,""):"";E.download=I?`Zapatillas_May_${I}.jpg`:`Zapatillas_May_${Date.now()}.jpg`,document.body.appendChild(E),E.click(),document.body.removeChild(E),URL.revokeObjectURL(E.href),j("Imagen descargada")}catch{window.open(p,"_blank")}},window.lbNav=p=>{p<0&&r===0||p>0&&r===e.length-1||(r=Math.max(0,Math.min(e.length-1,r+p)),m(),_(),k())},window.lbGoto=p=>{r=p,m(),_(),k()};const k=()=>{const p=document.getElementById("lb-img");if(!p)return;let x=!1,w=0,E=0,I=0;p.addEventListener("touchstart",y=>{if(y.touches.length===1){const P=new Date().getTime(),st=P-I;if(st<300&&st>0){i>1?m():(i=2.5,p.style.transform=`scale(${i})`,p.style.cursor="grab"),y.preventDefault();return}I=P,i>1?(x=!0,w=y.touches[0].clientX-c,E=y.touches[0].clientY-d):w=y.touches[0].clientX}else y.touches.length===2&&(x=!1,l=i,v=Math.hypot(y.touches[0].clientX-y.touches[1].clientX,y.touches[0].clientY-y.touches[1].clientY))},{passive:!1}),p.addEventListener("touchmove",y=>{if(y.touches.length===1&&i>1&&x)y.preventDefault(),c=y.touches[0].clientX-w,d=y.touches[0].clientY-E,p.style.transform=`scale(${i}) translate(${c/i}px, ${d/i}px)`;else if(y.touches.length===2){y.preventDefault();const P=Math.hypot(y.touches[0].clientX-y.touches[1].clientX,y.touches[0].clientY-y.touches[1].clientY);i=Math.max(1,Math.min(4,l*(P/v))),i<=1?m():(p.style.transform=`scale(${i}) translate(${c/i}px, ${d/i}px)`,p.style.cursor="grab")}},{passive:!1}),p.addEventListener("touchend",y=>{if(x)x=!1;else if(i===1&&y.changedTouches.length===1){const P=y.changedTouches[0].clientX-w;Math.abs(P)>50&&window.lbNav(P<0?1:-1)}},{passive:!0}),p.addEventListener("wheel",y=>{y.preventDefault();const P=y.deltaY*-.005;i=Math.max(1,Math.min(4,i+P)),i<=1?m():(p.style.transform=`scale(${i}) translate(${c/i}px, ${d/i}px)`,p.style.cursor="grab")},{passive:!1});let X=!1,rt=0,N=0;p.addEventListener("mousedown",y=>{i>1&&(X=!0,rt=y.clientX-c,N=y.clientY-d,p.style.cursor="grabbing")}),window.addEventListener("mousemove",y=>{X&&i>1&&(c=y.clientX-rt,d=y.clientY-N,p.style.transform=`scale(${i}) translate(${c/i}px, ${d/i}px)`)}),window.addEventListener("mouseup",()=>{X&&(X=!1,p&&(p.style.cursor="grab"))})};n.addEventListener("click",p=>{p.target===n&&history.back()});const D=p=>{p.key==="ArrowRight"?window.lbNav(1):p.key==="ArrowLeft"?window.lbNav(-1):p.key==="Escape"&&history.back()};document.addEventListener("keydown",D),n.addEventListener("remove",()=>document.removeEventListener("keydown",D)),_(),document.body.appendChild(n),k(),window._zmPushBack&&window._zmPushBack(()=>{const p=document.getElementById("pc-lightbox");p&&p.remove(),document.removeEventListener("keydown",D)})};window._zmNavStack=[];window._zmPushBack=t=>{history.pushState({zmNav:window._zmNavStack.length+1},""),window._zmNavStack.push(t)};window.addEventListener("popstate",()=>{if(window._zmNavStack.length>0){const t=window._zmNavStack.pop();try{t()}catch{}}});Bt();
