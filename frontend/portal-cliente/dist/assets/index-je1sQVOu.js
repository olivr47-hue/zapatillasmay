(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))o(r);new MutationObserver(r=>{for(const i of r)if(i.type==="childList")for(const n of i.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&o(n)}).observe(document,{childList:!0,subtree:!0});function a(r){const i={};return r.integrity&&(i.integrity=r.integrity),r.referrerPolicy&&(i.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?i.credentials="include":r.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(r){if(r.ep)return;r.ep=!0;const i=a(r);fetch(r.href,i)}})();const h="/api",fe="portal_sesion",Se="portal_carrito",se=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],ce=document.getElementById("app"),s={sesion:null,data:null,carrito:[],tab:"tienda",filtroCat:"",busqueda:""},O=e=>parseFloat(e)||0,x=e=>"$"+Math.round(e).toLocaleString("es-MX"),f=e=>String(e??"").replace(/"/g,"&quot;").replace(/</g,"&lt;"),we=e=>{try{const t=new URL(String(e??""),window.location.origin);return t.protocol==="http:"||t.protocol==="https:"?t.href:""}catch{return""}};function A(){var t;const e=(t=s.sesion)==null?void 0:t.token;return e?{"Content-Type":"application/json",Authorization:"Bearer "+e}:{"Content-Type":"application/json"}}function ge(e){return e&&e.status===401?(Me(),!0):!1}function Ae(){var e;try{s.sesion=JSON.parse(localStorage.getItem(fe)||"null")}catch{s.sesion=null}try{s.carrito=JSON.parse(localStorage.getItem(Se)||"[]")}catch{s.carrito=[]}(e=s.sesion)!=null&&e.cliente_id?Te():te()}let Q="telefono",V="pedir",Z={};function te(){const e=Q==="telefono",t=V==="codigo"?`
    ...
    </div>`,document.body.appendChild(r),r.addEventListener("click",i=>{i.target===r&&r.remove()}),C.color&&(Ie(),ae())}function Ie(){const e=document.getElementById("pm-mode");if(!e)return;const{productoId:t,color:a}=C,r=oe(t,a)>=1,i=document.getElementById("pm-tier-corr");if(i&&(i.style.display=r?"":"none"),!r)C.modo==="corrida"&&(C.modo="variado"),e.innerHTML=`
      <button data-modo="variado" class="on" onclick="window.__modoModal('variado')">🧺 Surtido variado</button>
    ...
    `}}window.__pickColor=e=>{C.color=e,j=1,document.querySelectorAll("#pm-swatches .swatch").forEach(r=>r.classList.toggle("active",r.dataset.color===e));const a=s.data.variantes.filter(r=>r.producto_id===C.productoId&&r.color===e).map(r=>r.foto_url).find(Boolean),o=document.getElementById("pm-img");o&&a&&(o.src=a),Ie(),ae()};window.__modoModal=e=>{C.modo=e,j=1,document.querySelectorAll("#pm-mode button").forEach(t=>t.classList.toggle("on",t.dataset.modo===e)),ae()};function Xe(){const{productoId:e,color:t}=C;return s.data.variantes.filter(a=>a.producto_id===e&&a.color===t).sort((a,o)=>se.indexOf(a.talla)-se.indexOf(o.talla))}function ae(){const e=document.getElementById("pm-tallas");if(!e)return;const{color:t,modo:a,productoId:o}=C,r=s.data.productos.find(c=>c.id===o),i=Xe(),n=document.getElementById("pm-tallas-label");if(a==="corrida"){const c=oe(o,t);j>c&&(j=Math.max(1,c)),n&&(n.textContent=`Corrida completa — ${t} · ${x(r.precio_corrida)} x par`),e.className="corrida-rows",e.innerHTML=`
    ...
    `}else n&&(n.textContent="Tallas — "+t),e.className="tallas",e.innerHTML=i.map(c=>{var v;const d=s.data.stockPorVar[c.id]||0,l=((v=s.carrito.find(u=>u.variante_id===c.id&&!u.es_corrida))==null?void 0:v.cantidad)||0;return`<button class="talla ${l?"on":""}" ${d<=0?"disabled":""} onclick="window.__addTalla('${c.id}')">
    ...
    </div>`}window.__qty=(e,t)=>{const a=s.carrito[e];if(a){if(t>0){const o=s.data.stockPorVar[a.variante_id]||0;if(s.carrito.filter(i=>i.variante_id===a.variante_id).reduce((i,n)=>i+n.cantidad,0)>=o){I("Sin más stock ("+o+")");return}}a.cantidad=Math.max(1,a.cantidad+t),U(),J(),document.querySelector(".bottomnav").outerHTML=D()}};window.__delItem=e=>{s.carrito.splice(e,1),U(),J(),document.querySelector(".bottomnav").outerHTML=D()};window.__qtyGrupoCorrida=(e,t,a)=>{const n=s.carrito.filter(u=>u.producto_id===e&&u.color===t&&u.es_corrida).reduce((u,_)=>u+_.cantidad,0)/6+a;if(n<=0){window.__delGrupoCorrida(e,t);return}const c=oe(e,t);if(n>c){I(`Sin existencias para ${n} corridas (máximo ${c})`);return}const d=je(e,t,n);s.carrito=s.carrito.filter(u=>!(u.producto_id===e&&u.color===t&&u.es_corrida));const l=s.data.productos.find(u=>u.id===e);Object.entries(d).forEach(([u,_])=>{if(_<=0)return;const E=s.data.variantes.find(H=>H.id===u);s.carrito.push({variante_id:u,producto_id:l.id,nombre:l.nombre,color:E.color,talla:E.talla,cantidad:_,es_corrida:!0,precio_menudeo:l.precio_menudeo,precio_mayoreo3:l.precio_mayoreo3,precio_mayoreo6:l.precio_mayoreo6,precio_corrida:l.precio_corrida,imagen:E.foto_url||l.imagen_principal||null})}),U(),J();const v=document.querySelector(".bottomnav");v&&(v.outerHTML=D())};window.__delGrupoCorrida=(e,t)=>{s.carrito=s.carrito.filter(o=>!(o.producto_id===e&&o.color===t&&o.es_corrida)),U(),J();const a=document.querySelector(".bottomnav");a&&(a.outerHTML=D())};window.__enviarPedido=async()=>{var e;if(s.carrito.length){if((e=s.sesion)!=null&&e.demo){I("Modo demo: no se envía pedido real");return}if(confirm("¿Enviar este pedido al negocio? Te confirmarán existencia y total."))try{const t=await fetch(h+"/portal/carrito",{method:"POST",headers:A(),body:JSON.stringify({items:s.carrito.map(o=>({variante_id:o.variante_id,cantidad:o.cantidad,es_corrida:!!o.es_corrida}))})});if(ge(t))return;const a=await t.json();if(!t.ok||!a.ok)throw new Error(a.error||"");s.carrito=[],U(),I(a.fusionado?"Se agregó a tu carrito anterior":"¡Pedido enviado!"),le("pedidos")}catch{alert("No se pudo enviar el pedido. Intenta de nuevo.")}}};async function et(){B().innerHTML='<div class="spinner">Cargando pedidos…</div>';let e=[];try{const t=await fetch(h+"/portal/pedidos",{headers:A()});if(ge(t))return;e=await t.json()}catch{}if(e=Array.isArray(e)?e.filter(t=>t.status!=="borrador"):[],!e.length){B().innerHTML='<div class="empty"><div class="ic">🛍️</div><p>Aún no tienes pedidos confirmados</p></div>';return}B().innerHTML=e.map(t=>{const a=t.pedido_items||[],o=a.reduce((i,n)=>i+(n.cantidad||0),0),r=t.created_at?new Date(t.created_at).toLocaleDateString("es-MX",{day:"2-digit",month:"short",year:"numeric"}):"";return`<div class="row">
    ...
    </div>`,document.body.appendChild(o),o.addEventListener("click",r=>{r.target===o&&o.remove()})};window.__descargarCatalogo=async(e,t,a)=>{const o=a?a.textContent:"";a&&(a.disabled=!0,a.textContent="Descargando…");try{const r=await Pe(e);if(!r.length){I("Este catálogo no tiene páginas"),a&&(a.disabled=!1,a.textContent=o);return}const i=(t||"catalogo").replace(/[^\w]+/g,"_");let n=0;for(const c of r){n++;try{const l=await(await fetch(c.imagen_url)).blob(),v=URL.createObjectURL(l),u=document.createElement("a");u.href=v,u.download=`${i}_${String(c.pagina_numero||n).padStart(2,"0")}.jpg`,document.body.appendChild(u),u.click(),u.remove(),URL.revokeObjectURL(v),await new Promise(_=>setTimeout(_,300))}catch{window.open(c.imagen_url,"_blank")}}I("Descarga lista · "+r.length+" páginas")}catch{I("No se pudo descargar")}a&&(a.disabled=!1,a.textContent=o)};function nt(){s.mayaMsgs||(s.mayaMsgs=[]),B().innerHTML=`
    <div class="chat-wrap">
...
`).trim(),{texto:a||"👍",fotos:t}}window.__mayaSend=async()=>{const e=document.getElementById("maya-in");if(!e)return;const t=(e.value||"").trim();if(!t)return;e.value="",s.mayaMsgs||(s.mayaMsgs=[]);const a=s.mayaMsgs.filter(o=>!o.typing).map(o=>({role:o.role==="me"?"user":"assistant",content:o.content}));s.mayaMsgs.push({role:"me",content:t}),s.mayaMsgs.push({role:"bot",typing:!0}),Ce();try{const r=await(await fetch(h+"/chatbot/mensaje",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({mensaje:t,historial:a})})).json();s.mayaMsgs.pop();const{texto:i,fotos:n}=it(r.respuesta);s.mayaMsgs.push({role:"bot",content:i,fotos:n})}catch{s.mayaMsgs.pop(),s.mayaMsgs.push({role:"bot",content:"Ups, no me pude conectar. Intenta de nuevo en un momento."})}Ce()};let Ee=null;function I(e){const t=document.getElementById("toast");t&&(t.textContent=e,t.classList.add("show"),clearTimeout(Ee),Ee=setTimeout(()=>t.classList.remove("show"),1600))}window.abrirLightboxPC=function(e,t,a){if(!e)return;const o=document.getElementById("pc-lightbox");if(o&&o.remove(),!t||!t.length){const p=document.getElementById("pm-swatches");p&&(t=Array.from(p.querySelectorAll("img")).map($=>$.src)),(!t||!t.length)&&(t=[e]),t.includes(e)||(t=[e,...t])}let r=t.indexOf(e);r<0&&(r=0);const i=document.createElement("div");i.id="pc-lightbox",i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:19999;display:flex;flex-direction:column;align-items:center;justify-content:center;touch-action:none;user-select:none";let n=1,c=0,d=0,l=1,v=0;const u=()=>{n=1,c=0,d=0;const p=document.getElementById("lb-img");p&&(p.style.transform="",p.style.cursor="zoom-in")},_=()=>{const p=t.length;i.innerHTML=`
    ...
    `};window.pcCompartirImagenLB=async()=>{const p=t[r];if(p)try{const w=await(await fetch(p)).blob(),S=(w.type.split("/")[1]||"jpg").replace("jpeg","jpg"),P=new File([w],`zapatillasmay-${Date.now()}.${S}`,{type:w.type});navigator.canShare&&navigator.canShare({files:[P]})?await navigator.share({files:[P]}):navigator.share?await navigator.share({url:p}):window.pcDescargarImagenLB()}catch($){($==null?void 0:$.name)!=="AbortError"&&I("No se pudo compartir la imagen")}},window.pcDescargarImagenLB=async()=>{const p=t[r];if(p)try{const w=await(await fetch(p)).blob(),S=document.createElement("a");S.href=URL.createObjectURL(w);const P=a?a.replace(/[^A-Za-z0-9_-]/g,""):"";S.download=P?`Zapatillas_May_${P}.jpg`:`Zapatillas_May_${Date.now()}.jpg`,document.body.appendChild(S),S.click(),document.body.removeChild(S),URL.revokeObjectURL(S.href),I("Imagen descargada")}catch{window.open(p,"_blank")}},window.lbNav=p=>{p<0&&r===0||p>0&&r===t.length-1||(r=Math.max(0,Math.min(t.length-1,r+p)),u(),_(),E())},window.lbGoto=p=>{r=p,u(),_(),E()};const E=()=>{const p=document.getElementById("lb-img");if(!p)return;let $=!1,w=0,S=0,P=0;p.addEventListener("touchstart",y=>{if(y.touches.length===1){const z=new Date().getTime(),ie=z-P;if(ie<300&&ie>0){n>1?u():(n=2.5,p.style.transform=`scale(${n})`,p.style.cursor="grab"),y.preventDefault();return}P=z,n>1?($=!0,w=y.touches[0].clientX-c,S=y.touches[0].clientY-d):w=y.touches[0].clientX}else y.touches.length===2&&($=!1,l=n,v=Math.hypot(y.touches[0].clientX-y.touches[1].clientX,y.touches[0].clientY-y.touches[1].clientY))},{passive:!1}),p.addEventListener("touchmove",y=>{if(y.touches.length===1&&n>1&&$)y.preventDefault(),c=y.touches[0].clientX-w,d=y.touches[0].clientY-S,p.style.transform=`scale(${n}) translate(${c/n}px, ${d/n}px)`;else if(y.touches.length===2){y.preventDefault();const z=Math.hypot(y.touches[0].clientX-y.touches[1].clientX,y.touches[0].clientY-y.touches[1].clientY);n=Math.max(1,Math.min(4,l*(z/v))),n<=1?u():(p.style.transform=`scale(${n}) translate(${c/n}px, ${d/n}px)`,p.style.cursor="grab")}},{passive:!1}),p.addEventListener("touchend",y=>{if($)$=!1;else if(n===1&&y.changedTouches.length===1){const z=y.changedTouches[0].clientX-w;Math.abs(z)>50&&window.lbNav(z<0?1:-1)}},{passive:!0}),p.addEventListener("wheel",y=>{y.preventDefault();const z=y.deltaY*-.005;n=Math.max(1,Math.min(4,n+z)),n<=1?u():(p.style.transform=`scale(${n}) translate(${c/n}px, ${d/n}px)`,p.style.cursor="grab")},{passive:!1});let X=!1,ne=0,N=0;p.addEventListener("mousedown",y=>{n>1&&(X=!0,ne=y.clientX-c,N=y.clientY-d,p.style.cursor="grabbing")}),window.addEventListener("mousemove",y=>{X&&n>1&&(c=y.clientX-ne,d=y.clientY-N,p.style.transform=`scale(${n}) translate(${c/n}px, ${d/n}px)`)}),window.addEventListener("mouseup",()=>{X&&(X=!1,p&&(p.style.cursor="grab"))})};i.addEventListener("click",p=>{p.target===i&&i.remove()});const H=p=>{p.key==="ArrowRight"?window.lbNav(1):p.key==="ArrowLeft"?window.lbNav(-1):p.key==="Escape"&&(i.remove(),document.removeEventListener("keydown",H))};document.addEventListener("keydown",H),i.addEventListener("remove",()=>document.removeEventListener("keydown",H)),_(),document.body.appendChild(i),E()};Ae();
