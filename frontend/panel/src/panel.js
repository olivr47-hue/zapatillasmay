...
async function cargarEnviosMasivos() {
  ...
  content.innerHTML = '<p style="padding:2rem;color:#888">Cargando...</p>'
  try {
    const [resPlantillas, resClientes, resChats, resProductos] = await Promise.all([
    ...
    const productosConImg = resProductos.filter(p => p.activo && p.imagen_principal)
          ...
          <div style="background:white;border-radius:12px;border:1px solid #eee;padding:1.5rem">
            ...
            <div style="margin-bottom:1rem">
              ...
              <select id="envio-producto-sel" class="form-input" onchange="seleccionarImagenProductoEnvio()" style="margin-bottom:8px">
                ...
                ${productosConImg.map(p => `<option value="${p.imagen_principal}" data-nombre="${p.nombre}">${p.nombre}</option>`).join('')}
              </select>
...
window.filtrarAudienciaEnvio = () => {
  ...
  const todos = window._envioContactos || []
  const chats = window._envioChats || []
  const clientes = window._envioClientes || []

  let filtrados = todos
  if (filtro === 'clientes') {
    filtrados = clientes.filter(c => c.telefono).map(c => ({ telefono: c.telefono, nombre: c.nombre, fuente: 'cliente' }))
  } else if (filtro === 'whatsapp') {
    filtrados = chats.map(c => ({ telefono: c.telefono, nombre: c.nombre || c.telefono, fuente: 'whatsapp' }))
  } else if (['solo_pregunta','posible_comprador','comprador','seguimiento','frecuente'].includes(filtro)) {
    filtrados = chats.filter(c => c.etiqueta === filtro).map(c => ({ telefono: c.telefono, nombre: c.nombre || c.telefono, fuente: 'whatsapp' }))
  }

  window._envioContactosFiltrados = filtrados
  const countEl = document.getElementById('envio-count')
  if (countEl) countEl.textContent = filtrados.length

  const lista = document.getElementById('envio-lista')
  if (lista) {
    lista.innerHTML = filtrados.map(c => `
      <div style="display:flex;align-items:center;gap:10px;padding:8px;background:#f9f9f9;border-radius:8px">
        <div style="width:32px;height:32px;border-radius:50%;background:${c.fuente==='cliente' ? '#e91e8c' : '#25D366'};display:flex;align-items:center;justify-content:center;color:white;font-size:0.8rem;font-weight:700;flex-shrink:0">
...
window.iniciarEnvioMasivo = async () => {
  const contactos = window._envioContactosFiltrados || window._envioContactos || []
  if (contactos.length === 0) { alert('No hay contactos en la audiencia'); return }
  if (!confirm(`Enviar a ${contactos.length} contactos?`)) return
  ...
  try {
    ...
    resultado.innerHTML = `
      ...
      <p>Enviados: <strong>${data.enviados}</strong> · Fallidos: <strong>${data.fallidos}</strong></p>
      ${data.errores && data.errores.length > 0 ? '<p style="font-size:0.78rem;color:#c62828;margin-top:4px">' + data.errores.join('<br>') + '</p>' : ''}
    `
  } catch(e) {
