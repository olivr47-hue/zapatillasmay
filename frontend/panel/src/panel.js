      ...
      ${m.mensaje ? `
        <div style="display:flex;flex-direction:column;align-items:${esManual ? 'flex-end' : 'flex-start'}">
          ...
          <div style="max-width:70%;background:${esManual ? '#cfe9ff' : '#f5f5f5'};border-radius:${esManual ? '12px 12px 0 12px' : '12px 12px 12px 0'};padding:8px 12px;box-shadow:0 1px 2px rgba(0,0,0,0.08)">
            <p style="font-size:0.85rem;color:#333;white-space:pre-wrap">${m.mensaje.replace(/\[.+?\]:\s*/, '')}</p>
...
function renderVariante(i, datos) {
  ...
  if (d.imagenes && d.imagenes.length > 0) {
    fotosHTML = d.imagenes.map((url, fIdx) => {
      const esPortada = fIdx === 0
      return `<div style="position:relative;cursor:pointer" data-file-idx="${fIdx}" data-foto-url="${url}" ${esPortada ? 'data-es-portada="true"' : ''}>
        <img src="${url}" style="width:72px;height:72px;object-fit:cover;border-radius:10px;border:3px solid ${esPortada ? '#E91E8C' : '#eee'}" onclick="seleccionarPortadaExistente(${i}, ${fIdx})">
        ${esPortada ? '<span style="position:absolute;top:-6px;left:-6px;background:#E91E8C;color:white;font-size:0.55rem;padding:2px 6px;border-radius:100px;font-weight:700">PORTADA</span>' : ''}
        <button onclick="eliminarFotoExistente(${i}, this)" style="position:absolute;top:-6px;right:-6px;background:#c62828;color:white;border:none;border-radius:50%;width:18px;height:18px;cursor:pointer;font-size:0.65rem;display:flex;align-items:center;justify-content:center">✕</button>
      </div>`
...
window.seleccionarPortadaExistente = (idx, fotoIdx) => {
  ...
  // Quitar portada de todos
  preview.querySelectorAll('[data-foto-url]').forEach(div => {
    div.removeAttribute('data-es-portada')
    const img = div.querySelector('img')
    if (img) img.style.border = '3px solid #eee'
    const badge = div.querySelector('.portada-badge')
    if (badge) badge.remove()
  })
 
  // Marcar el seleccionado como portada
  const divs = preview.querySelectorAll('[data-foto-url]')
  if (divs[fotoIdx]) {
    divs[fotoIdx].dataset.esPortada = 'true'
    const img = divs[fotoIdx].querySelector('img')
    if (img) img.style.border = '3px solid #E91E8C'
    const badge = document.createElement('span')
    badge.className = 'portada-badge'
    badge.style.cssText = 'position:absolute;top:-6px;left:-6px;background:#E91E8C;color:white;font-size:0.55rem;padding:2px 6px;border-radius:100px;font-weight:700'
    badge.textContent = 'PORTADA'
    divs[fotoIdx].appendChild(badge)
  }
}
...
async function subirImagenesVariantes() {
  ...
  for (const v of variantesEls) {
    ...
    // Obtener fotos existentes en BD
    const fotosEnBD = colorExistente?.imagenes?.length > 0
      ? [...colorExistente.imagenes]
      : (colorExistente?.foto_url ? [colorExistente.foto_url] : [])
 
    // Obtener fotos existentes que siguen en el preview (no fueron eliminadas)
    const fotosExistentesEnPreview = []
    if (preview) {
      preview.querySelectorAll('[data-foto-url]').forEach(div => {
        const url = div.dataset.fotoUrl
        if (url) fotosExistentesEnPreview.push({ url, esPortada: div.dataset.esPortada === 'true' })
      })
    }
 
    // Determinar portada de fotos existentes
    const portadaExistente = fotosExistentesEnPreview.find(f => f.esPortada)
    const urlsExistentesOrdenadas = portadaExistente
      ? [portadaExistente.url, ...fotosExistentesEnPreview.filter(f => !f.esPortada).map(f => f.url)]
      : fotosExistentesEnPreview.map(f => f.url)
    ...
    if (inputImgs && inputImgs.files.length > 0) {
      ...
      for (const file of files) {
        ...
        try {
          const res = await fetch(API + '/imagenes/subir', { method: 'POST', body: formData })
          const data = await res.json()
          if (data.url) urlsNuevas.push(data.url)
        } catch(e) {
          console.error('Error subiendo imagen:', e)
        }
      }
 
      // Identificar portada entre fotos nuevas
      const portadaNuevaDiv = preview ? preview.querySelector('[data-file-idx][data-es-portada="true"]') : null
      const portadaNuevaIdx = portadaNuevaDiv ? parseInt(portadaNuevaDiv.dataset.fileIdx) : -1
 
      if (portadaExistente) {
        // La portada es una foto existente
        urls.push(portadaExistente.url, ...urlsNuevas, ...fotosExistentesEnPreview.filter(f => !f.esPortada).map(f => f.url))
      } else if (portadaNuevaIdx > 0 && portadaNuevaIdx < urlsNuevas.length) {
        // La portada es una foto nueva no primera
        const portada = urlsNuevas.splice(portadaNuevaIdx, 1)[0]
        urls.push(portada, ...urlsNuevas, ...urlsExistentesOrdenadas)
      } else {
        // Primera foto nueva es portada
        urls.push(...urlsNuevas, ...urlsExistentesOrdenadas)
      }
    } else {
      // Sin fotos nuevas — usar existentes en el orden del preview
      if (urlsExistentesOrdenadas.length > 0) {
        urls.push(...urlsExistentesOrdenadas)
      } else if (fotosEnBD.length > 0) {
        // No se tocó el preview — conservar BD tal cual
        urls.push(...fotosEnBD)
      }
    }
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem">
          ...
          <div style="background:white;border-radius:12px;border:1px solid #eee;padding:1.5rem">
            ...
            <div style="margin-bottom:1rem">
              ...
              <input type="text" id="envio-imagen" class="form-input" placeholder="O pega URL de Cloudinary manualmente..." oninput="limpiarSelProductoEnvio()">
            </div>
