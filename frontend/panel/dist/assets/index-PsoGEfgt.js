...
`,renderInventario()}catch{e.innerHTML='<p style="padding:2rem;color:red">Error conectando con el servidor</p>'}}window.renderInventario=()=>{const{sucursales:e,productos:t,variantes:o,inventario:a}=window._invData,n=(document.getElementById("inv-buscar")?document.getElementById("inv-buscar").value:"").toLowerCase(),r=document.getElementById("inv-categoria")?document.getElementById("inv-categoria").value:"",i=document.getElementById("inv-talla")?document.getElementById("inv-talla").value:"",s=document.getElementById("inv-estado")?document.getElementById("inv-estado").value:"",d=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],l=t.filter(m=>!(!m.activo||n&&!m.nombre.toLowerCase().includes(n)&&!(m.sku_interno||"").toLowerCase().includes(n)||r&&m.categoria!==r)),c=e.map(m=>{const u=a.filter(y=>y.sucursal_id===m.id),g=l.map(y=>{const b=o.filter(z=>z.producto_id===y.id);if(b.length===0)return"";const v=[...new Set(b.map(z=>z.color).filter(Boolean))].map(z=>{const x=b.filter(P=>P.color===z).sort((P,M)=>d.indexOf(P.talla)-d.indexOf(M.talla));if(i&&!x.find(P=>P.talla===i))return"";const T=x[0]?x[0].color_hex:"#888",_=x[0]?x[0].foto_url:null,E=x.map(P=>{const M=u.find(B=>B.variante_id===P.id),C=M?M.cantidad:null,S=M?M.stock_minimo:3;if(i&&P.talla!==i||s&&(s==="agotado"&&C!==0||s==="bajo"&&(C===null||C===0||C>S)||s==="disponible"&&(C===null||C===0||C<=S)))return"";let I,k;return C===null?(I="#f0f0f0",k="#aaa"):C===0?(I="#ffebee",k="#c62828"):C<=S?(I="#fff8e1",k="#f57f17"):(I="#e8f5e9",k="#2e7d32"),`
            <div style="display:flex;align-items:center;justify-content:space-between;background:${I};border-radius:10px;padding:8px 12px;border:1px solid ${k}30">
              <span style="font-size:0.85rem;font-weight:600;color:#555;min-width:44px">T${P.talla}</span>
              <div style="display:flex;align-items:center;gap:8px">
                <button onclick="cambiarStockInventario('${P.id}', '${m.id}', ${C!==null?C:0}, ${S}, -1)"
...
Ve a Inventario → Reabastecer para agregar las cantidades.`);else if(h&&b){const _=i.length>0?i:["Unica"];await new Promise(S=>setTimeout(S,800));let E=[];const P=s.length*_.length;for(let S=0;S<3&&(E=await fetch(f+"/variantes/producto/"+b).then(I=>I.json()),!(E.length>=P));S++)await new Promise(I=>setTimeout(I,600));const M=await fetch(f+"/inventario/").then(S=>S.json()),C=new Set(M.filter(S=>S.sucursal_id===h).map(S=>S.variante_id));for(const S of E){const I=d.find(j=>j.nombre.trim().toLowerCase()===(S.color||"").trim().toLowerCase());if(!I)continue;const k=String(S.talla||"").replace(".","_"),B=document.getElementById("stock-ini-"+I.id+"-"+k),$=B&&parseInt(B.value)||0;if($>0){const j=await fetch(f+"/movimientos/ajuste",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({variante_id:S.id,sucursal_id:h,cantidad:$,motivo:window._productoEditandoId?"Resurtido desde edicion de producto":"Stock inicial"})});if(j.ok)w++,console.log(`[Stock] ✓ ${S.color} T${S.talla} +${$}`);else{const O=await j.text();console.error(`[Stock] Error ${S.color} T${S.talla}:`,O),z.push(`${S.color} T${S.talla}`)}}else C.has(S.id)||(await fetch(f+"/movimientos/ajuste",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({variante_id:S.id,sucursal_id:h,cantidad:0,motivo:"Registro inicial (sin stock)"})}),console.log(`[Stock] Registro 0 creado: ${S.color} T${S.talla}`))}}if(y&&y.error){alert("Error: "+y.error),r&&(r.textContent="Guardar producto",r.disabled=!1),window._guardandoProducto=!1;return}let T="Producto guardado correctamente";w>0&&(T+=`
...
${o.telefono?'<a href="https://wa.me/'+(o.lada||"52")+o.telefono.replace(/\D/g,"")+'" target="_blank" class="btn btn-secondary" style="background:#25D366;color:white;border-color:#25D366;margin-left:auto">WhatsApp</a>':""}      </div>
  ...
  `};window.guardarTraspaso=async()=>{const e=document.getElementById("tra").value,t=document.getElementById("tra-origen").value,o=document.getElementById("tra-destino").value,a=document.getElementById("tra-cantidad").value;if(!e||!t||!o||!a){alert("Por favor completa todos los campos");return}if(t===o){alert("La sucursal origen y destino no pueden ser la misma");return}try{const r=await(await fetch(f+"/movimientos/traspaso",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({variante_id:e,sucursal_origen_id:t,sucursal_destino_id:o,cantidad:parseInt(a)})})).json();r.ok?(alert("Traspaso realizado correctamente. Se movieron "+r.cantidad_movida+" pares."),navegarA("inventario")):alert("Error: "+(r.error||JSON.stringify(r)))}catch{alert("Error conectando con el servidor")}};async function Xe(){const e=document.getElementById("content");try{const o=await(await fetch(f+"/pedidos/")).json();e.innerHTML=`
      <div style="margin-bottom:1rem;display:flex;gap:8px;flex-wrap:wrap;align-items:center">
...
<td>
  ...
  <button class="btn btn-secondary" style="padding:4px 8px;font-size:0.72rem;color:#c62828;border-color:#c62828" 
    ...
    Cancelar
        ...
        </tr>`}).join(""))};async function Ue(){try{const t=await(await fetch(f+"/pedidos/")).json(),a=await(await fetch(f+"/clientes/")).json(),r=await(await fetch(f+"/inventario/alertas")).json(),i=new Date;i.setHours(0,0,0,0);const s=new Date(i);s.setDate(s.getDate()-7);const d=new Date(i);d.setDate(d.getDate()-30);const l=t.filter($=>$.status==="confirmado"||$.status==="pagado"),c=l.filter($=>new Date($.created_at)>=i),p=l.filter($=>new Date($.created_at)>=s),m=c.reduce(($,j)=>$+parseFloat(j.total||0),0),u=p.reduce(($,j)=>$+parseFloat(j.total||0),0),g=a.filter($=>$.created_at&&new Date($.created_at)>=d).length,y=["Dom","Lun","Mar","Mie","Jue","Vie","Sab"],b={};y.forEach($=>b[$]=0),l.filter($=>new Date($.created_at)>=d).forEach($=>{const j=y[new Date($.created_at).getDay()];b[j]+=parseFloat($.total||0)});const h={};l.forEach($=>{h[$.canal||"sucursal"]=(h[$.canal||"sucursal"]||0)+parseFloat($.total||0)});const v={};l.forEach($=>{v[$.forma_pago||"efectivo"]=(v[$.forma_pago||"efectivo"]||0)+1});const w={};l.forEach($=>{w[$.empleado||"Admin"]=(w[$.empleado||"Admin"]||0)+parseFloat($.total||0)});const z={};l.forEach($=>{const j=new Date($.created_at).toLocaleDateString("es-MX",{month:"short",year:"numeric"});z[j]=(z[j]||0)+parseFloat($.total||0)});const x={};l.forEach($=>{$.clientes&&(x[$.clientes.nombre]=(x[$.clientes.nombre]||0)+parseFloat($.total||0))});const T=Object.entries(x).sort(($,j)=>j[1]-$[1]).slice(0,5),_=Object.entries(b).sort(($,j)=>j[1]-$[1])[0],E=Object.entries(w).sort(($,j)=>j[1]-$[1])[0];if(!document.getElementById("dashboard-contenido"))return;const M=l.filter($=>new Date($.created_at)>=d),C=M.reduce(($,j)=>$+parseFloat(j.total||0),0),S=[];for(let $=6;$>=0;$--){const j=new Date(i);j.setDate(j.getDate()-$);const O=j.toISOString().split("T")[0],L=j.toLocaleDateString("es-MX",{weekday:"short",day:"numeric"}),H=l.filter(D=>{var A;return(A=D.created_at)==null?void 0:A.startsWith(O)}).reduce((D,A)=>D+parseFloat(A.total||0),0);S.push({label:L,total:H,ds:O})}const I=($,j,O,L)=>{const H=document.getElementById($),D=document.getElementById($+"-sub");H&&(H.textContent=j,L&&(H.style.color=L)),D&&O&&(D.textContent=O)};I("kpi-ventas-hoy","$"+m.toLocaleString("es-MX",{maximumFractionDigits:0}),c.length+" pedidos","#C8967A"),I("kpi-pedidos-hoy",c.length,"confirmados"),I("kpi-ventas-7d","$"+u.toLocaleString("es-MX",{maximumFractionDigits:0}),p.length+" pedidos"),I("kpi-ventas-30d","$"+C.toLocaleString("es-MX",{maximumFractionDigits:0}),M.length+" pedidos"),I("kpi-clientes-nuevos",g,"últimos 30 días"),I("kpi-stock-bajo",r.length,r.length>0?"⚠ reabastecer":"✓ ok",r.length>0?"#f59e0b":"#10b981"),I("kpi-mejor-dia",_?_[0]:"—",_?"$"+_[1].toLocaleString("es-MX",{maximumFractionDigits:0}):""),I("kpi-total-clientes",a.length,"registrados");const k=document.getElementById("dash-top-clientes");k&&(k.innerHTML=T.length===0?'<p style="color:var(--text-muted);font-size:0.85rem">Sin datos aun</p>':T.map(([$,j],O)=>`
...
`})})),n.textContent=b}}catch(m){n.style.display="block",n.textContent="Error inesperado: "+m.message}finally{t.innerHTML=r,t.disabled=!1}}window.mlPublicarPreview=function(e){Pe(!0,e)};window.mlPublicarReal=function(e){confirm("¿Publicar TODAS las variantes activas de este producto en MercadoLibre? Esto creará nuevos items en tu cuenta.")&&Pe(!1,e)};window.descargarExcelTikTok=async function(e,t,o){const a=e.innerHTML;try{e.innerHTML="⏳ Generando...",e.disabled=!0;const n=await fetch(`${f}/tiktok/${t}`);if(!n.ok){const d=await n.text();throw new Error(`Error ${n.status}: ${d}`)}const r=await n.blob(),i=URL.createObjectURL(r),s=document.createElement("a");s.href=i,s.download=o,document.body.appendChild(s),s.click(),document.body.removeChild(s),URL.revokeObjectURL(i)}catch(n){alert("Error al descargar el archivo: "+n.message)}finally{e.innerHTML=a,e.disabled=!1}};async function Ye(){const e=document.getElementById("content");e.innerHTML=`
  ...
  `,document.getElementById("ml-titulo").addEventListener("input",function(){document.getElementById("ml-titulo-count").textContent=this.value.length}),window.mlGenerarPreview=async()=>{var d,l,c,p;const o=document.getElementById("ml-sku").value.trim().toUpperCase(),a=document.getElementById("ml-listing").value,n=document.getElementById("ml-titulo").value.trim(),r=document.getElementById("ml-precio").value.trim(),i=r?parseFloat(r):null;if(!o){alert("Ingresa el SKU del producto");return}if(!n){alert("Ingresa el título para ML");return}if(n.length>60){alert("El título no puede superar 60 caracteres");return}if(i!==null&&(isNaN(i)||i<=0)){alert("El precio debe ser mayor a 0");return}const s=document.getElementById("ml-btn-preview");s.textContent="⏳ Generando...",s.disabled=!0;try{const m=await fetch(`${f}/ml/publicar`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sku_interno:o,listing_type:a,solo_preview:!0})}),u=await m.json();if(!m.ok){alert("Error: "+(u.detail||JSON.stringify(u)));return}const g=u.resultados||[];if(!g.length){alert("No se encontraron variantes activas");return}const y=document.getElementById("ml-descripcion"),b=((c=(l=(d=g[0])==null?void 0:d.preview)==null?void 0:l.description)==null?void 0:c.plain_text)||"";y.value.trim()||(y.value=b);const h={},v=[],w=new Set;for(const P of g){const M=P.color||"Sin color";h[M]||(h[M]=[]);for(const C of((p=P.preview)==null?void 0:p.pictures)||[])C.source&&(h[M].find(S=>S.source===C.source)||h[M].push(C),w.has(C.source)||(w.add(C.source),v.push(C)))}const z={},x=new Set;for(const P of Object.keys(h))z[P]=0,x.add(P);const T=Object.keys(h);if(T.length>0){const P=document.getElementById("ml-fotos-wrap"),M=document.getElementById("ml-fotos-grid"),C=()=>{S();const I=g.filter(k=>x.has(k.color||"Sin color"));t(I,n,i,z,y.value.trim(),h)},S=()=>{M.innerHTML=T.map(I=>{const k=h[I];if(!k.length)return"";const B=x.has(I),$=I.replace(/'/g,"\\'"),j=I.replace(/[^a-zA-Z0-9]/g,"_");return`
                ...
                <div style="display:flex;gap:8px;flex-wrap:wrap;padding-left:24px">
                  ${k.map((O,L)=>`
                    <div onclick="${B?`mlSeleccionarPortadaColor('${$}',${L})`:""}"
                         id="ml-foto-${j}-${L}"
                         style="cursor:${B?"pointer":"default"};border:3px solid ${L===z[I]&&B?"#3483fa":"#ddd"};border-radius:8px;overflow:hidden;width:80px;height:80px;position:relative;flex-shrink:0">
                      <img src="${O.source}" style="width:100%;height:100%;object-fit:cover">
                      ${L===z[I]&&B?'<div style="position:absolute;bottom:0;left:0;right:0;background:#3483fa;color:#fff;font-size:0.55rem;text-align:center;padding:2px;font-weight:700">PORTADA</div>':""}
                    </div>
                  `).join("")}
                </div>
              </div>`}).join("")};S(),P.style.display="block",window.mlToggleColor=I=>{x.has(I)?x.delete(I):x.add(I),C()},window.mlSeleccionarPortadaColor=(I,k)=>{z[I]=k,C()}}const _=g.filter(P=>x.has(P.color||"Sin color"));t(_,n,i,z,y.value.trim(),h);const E=document.getElementById("ml-variantes-wrap");E.style.display="block",E.scrollIntoView({behavior:"smooth",block:"start"})}catch(m){alert("Error de conexión: "+m.message)}finally{s.textContent="🔍 Generar preview",s.disabled=!1}};function t(o,a,n,r,i,s){const d=document.getElementById("ml-variantes-titulo"),l=document.getElementById("ml-variantes-list");d.textContent=`Paso 2 — Revisa y edita (${o.length} variantes, título aplicado a todas)`,l.innerHTML=o.map((c,p)=>{const m={...c.preview,family_name:a};if(delete m.title,n!==null&&(m.price=n),i&&(m.description={plain_text:i}),s){const u=c.color||"Sin color",g=s[u]||[],y=r&&r[u]||0;let b=[...g];if(y>0&&y<b.length){const[h]=b.splice(y,1);b.unshift(h)}if(b.length<3&&s){for(const[h,v]of Object.entries(s))if(h!==u){for(const w of v)if(b.find(z=>z.source===w.source)||b.push(w),b.length>=12)break;if(b.length>=12)break}}m.pictures=b.slice(0,12)}return`
      ...
      `).join("")||'<p style="color:#888">Sin detalles</p>'}catch(l){s.textContent="❌ Error de conexión",d.innerHTML=`<p style="color:red">${l.message}</p>`}finally{r.textContent="🚀 Publicar todas en ML",r.disabled=!1}}}let X=[],Q="";async function Qe(){const e=document.getElementById("content");e.innerHTML=`
    <style>
...
Nuevo monto:`,o);if(a===null)return;const n=parseFloat(a);if(isNaN(n)||n<0)return alert("Monto inválido");try{(await fetch(f+"/clientes/"+e,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({credito_disponible:n})})).ok?await ge():alert("Error actualizando crédito")}catch(r){alert("Error: "+r.message)}};async function fe(){const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:#888">Cargando carritos...</p>';try{const[t,o,a]=await Promise.all([fetch(f+"/pedidos/?status=borrador").then(r=>r.json()).catch(()=>[]),fetch(f+"/clientes/").then(r=>r.json()),fetch(f+"/sucursales/").then(r=>r.json())]),n=Array.isArray(t)?t.filter(r=>r.status==="borrador"&&(!r.canal||r.canal==="sucursal"||r.canal==="mayoreo")):[];e.innerHTML=`
  ...
  `};window.crearNuevoCarrito=async()=>{var a;const e=document.getElementById("nc-cliente").value,t=document.getElementById("nc-sucursal").value,o=document.getElementById("nc-comentario").value;if(!e){alert("Selecciona un cliente");return}try{const r=await(await fetch(f+"/pedidos/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({cliente_id:e,sucursal_id:t,canal:"sucursal",forma_pago:"efectivo",status:"borrador",total:0,subtotal:0,comentarios:o||null,items:[]})})).json(),i=r.id||((a=r[0])==null?void 0:a.id);if(!i){alert("Error creando carrito");return}abrirCarrito(i)}catch(n){alert("Error: "+n.message)}};window.abrirCarrito=async e=>{const t=document.getElementById("content");t.innerHTML='<p style="padding:2rem;color:#888">Cargando carrito...</p>';try{const[o,a,n,r,i]=await Promise.all([fetch(f+"/pedidos/"+e).then(l=>l.json()),fetch(f+"/pedidos/"+e+"/items").then(l=>l.json()),fetch(f+"/variantes/?activa=eq.true").then(l=>l.json()),fetch(f+"/productos/").then(l=>l.json()),fetch(f+"/inventario/").then(l=>l.json()).catch(()=>[])]),s=Array.isArray(o)?o[0]:o,d=s.clientes||{};window._carritoActivo={pedidoId:e,items:Array.isArray(a)?a:[],variantes:n,productos:r,inventario:i,sucursalId:s.sucursal_id},it(s)}catch{t.innerHTML='<p style="padding:2rem;color:red">Error cargando carrito</p>'}};function it(e){const t=document.getElementById("content"),{pedidoId:o,items:a,variantes:n,productos:r,inventario:i,sucursalId:s}=window._carritoActivo,d=e.clientes||{},l=a.reduce((m,u)=>m+u.cantidad*u.precio_unitario,0),c=a.reduce((m,u)=>m+u.cantidad,0),p=e.created_at?Math.floor((Date.now()-new Date(e.created_at).getTime())/864e5):0;t.innerHTML=`
    <div style="max-width:860px">
      ...
      <div class="table-card" style="padding:1.2rem">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
          <p style="font-weight:600;color:#333;margin:0">Productos en carrito (${c} pares)</p>
          <p style="font-weight:700;font-size:1.2rem;color:#E91E8C;margin:0">Total: $${l.toFixed(2)}</p>
        </div>
        ...
        <div id="carrito-items-lista">
          ...
          ${a.map((m,u)=>{const g=m.variantes||{},y=g.productos||{},b=y.nombre||m.nombre||"—",h=g.color||m.color||"",v=g.talla||m.talla||"",w=y.imagen_principal||null,z=i.find(T=>T.variante_id===m.variante_id&&T.sucursal_id===s),x=z?z.cantidad:"?";return`
              <div style="display:flex;align-items:center;gap:10px;padding:10px;background:#f9f9f9;border-radius:8px;margin-bottom:8px;border:1px solid #eee;flex-wrap:wrap">
                ${w?`<img src="${w}" style="width:52px;height:52px;object-fit:cover;border-radius:8px;flex-shrink:0">`:'<div style="width:52px;height:52px;background:#eee;border-radius:8px;flex-shrink:0;display:flex;align-items:center;justify-content:center">👟</div>'}
                <div style="flex:1;min-width:140px">
                  <p style="font-weight:600;font-size:0.85rem;margin:0">${b}${h?" · "+h:""}${v?" T"+v:""}</p>
                  <p style="font-size:0.75rem;color:#888;margin:2px 0 0">Stock disponible: ${x} pares</p>
                </div>
                <div style="display:flex;align-items:center;gap:6px">
                  <button onclick="cambiarCantidadCarrito(${u},-1)" style="background:#eee;border:none;border-radius:4px;width:26px;height:26px;cursor:pointer;font-size:1rem">−</button>
                  <span style="font-weight:700;min-width:24px;text-align:center">${m.cantidad}</span>
                  <button onclick="cambiarCantidadCarrito(${u},1)" style="background:#eee;border:none;border-radius:4px;width:26px;height:26px;cursor:pointer;font-size:1rem">+</button>
                </div>
                <input type="number" value="${m.precio_unitario}" style="width:80px;padding:5px;border:1px solid #ddd;border-radius:6px;text-align:center;font-size:0.85rem"
                  onchange="actualizarPrecioCarrito(${u}, this.value)" placeholder="Precio">
                <strong style="color:#E91E8C;min-width:70px;text-align:right">$${(m.cantidad*m.precio_unitario).toFixed(2)}</strong>
                <button onclick="eliminarDeCarrito('${m.id}',${u})" style="background:none;border:none;color:#c62828;cursor:pointer;font-size:1.1rem">🗑</button>
              </div>
            `}).join("")}
        </div>
  ...
  `).join("")};window.seleccionarModeloCorrida=(e,t)=>{const{variantes:o,productos:a,inventario:n,sucursalId:r}=window._carritoActivo,i=a.find(m=>m.id===e);document.getElementById("c-buscar-corrida").value=`${(i==null?void 0:i.nombre)||""} · ${t}`,document.getElementById("c-corrida-resultados").style.display="none";const s=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],d=o.filter(m=>m.producto_id===e&&m.color===t).sort((m,u)=>s.indexOf(m.talla)-s.indexOf(u.talla)),l=parseFloat(i==null?void 0:i.precio_menudeo)||0,c=parseFloat(i==null?void 0:i.precio_corrida)||parseFloat(i==null?void 0:i.precio_mayoreo6)||(l>0?Math.round(l-70):0);document.getElementById("c-precio-corrida").value=c||"",window._corridaSeleccionada={productoId:e,color:t,variantes:d,prod:i,precioCorrida:c};const p=document.getElementById("c-corrida-tallas-grid");p.innerHTML=d.map(m=>{const u=n.find(b=>b.variante_id===m.id&&(r?b.sucursal_id===r:!0)),g=u?u.cantidad:null,y=g!==null&&g===0;return`
      <label style="display:flex;flex-direction:column;align-items:center;gap:4px;cursor:pointer;padding:8px 12px;border:2px solid ${y?"#fde":"#ddd"};border-radius:8px;background:${y?"#fff5f5":"white"};min-width:60px;opacity:${y?.5:1}">
        <input type="checkbox" ${y?"":"checked"} value="${m.id}" style="width:16px;height:16px;cursor:pointer;accent-color:#E91E8C" ${y?"disabled":""}>
        <span style="font-weight:700;font-size:0.9rem">T${m.talla}</span>
        <span style="font-size:0.65rem;color:${g===null?"#aaa":g>0?"#2e7d32":"#c62828"}">
          ${g===null?"?":g+" pares"}
        </span>
      </label>
    `}).join(""),document.getElementById("c-corrida-tallas").style.display="block"};window.agregarCorridaAlCarritoActivo=async()=>{var l;const{productoId:e,color:t,variantes:o,prod:a}=window._corridaSeleccionada||{};if(!a){alert("Selecciona un modelo primero");return}const n=parseFloat(document.getElementById("c-precio-corrida").value)||((l=window._corridaSeleccionada)==null?void 0:l.precioCorrida)||0;if(!n){alert("No se encontró precio para este producto. Ingrésalo manualmente.");return}const r=document.querySelectorAll("#c-corrida-tallas-grid input[type=checkbox]:checked");if(r.length===0){alert("Selecciona al menos una talla");return}const i=window._carritoActivo.pedidoId;let s=0;for(const c of r){const p=c.value,m=o.find(u=>u.id===p);await fetch(f+"/pedidos/"+i+"/items",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({variante_id:p,cantidad:1,precio_unitario:n,subtotal:n,nombre:a.nombre,color:t,talla:(m==null?void 0:m.talla)||""})}),s+=n}const d=window._carritoActivo.items.reduce((c,p)=>c+p.cantidad*p.precio_unitario,0);await fetch(f+"/pedidos/"+i,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({total:d+s})}),await abrirCarrito(i)};window.liberarCarrito=async e=>{if(confirm("¿Liberar este carrito? Los productos quedan disponibles para otros clientes."))try{(await(await fetch(f+"/pedidos/"+e+"/cancelar",{method:"POST"})).json()).ok&&(alert("Carrito liberado."),fe())}catch(t){alert("Error: "+t.message)}};document.querySelector("#app").style.cssText="display:flex;min-height:100vh;width:100%";const Ae="erp_empleado";function ye(){document.querySelector("#app").innerHTML=`
