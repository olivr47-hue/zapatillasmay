...
¿Quieres reactivar todos?`)&&(localStorage.removeItem("ordenes_pospuestos"),Y())};window.guardarOrdenCompra2=async()=>{var r,i,s,d;const{seleccionados:e,sucursalId:t}=window._ordenModal||{};if(!e)return;const a=(r=document.getElementById("orden-fecha"))==null?void 0:r.value,o=((i=document.getElementById("orden-notas"))==null?void 0:i.value)||"",n={};e.forEach(l=>{var u,g;const c=l.proveedor_id||"sin-proveedor",p=((u=l.proveedor)==null?void 0:u.nombre)||"Sin proveedor";n[c]||(n[c]={nombre:p,items:[]});const m=l.variantes||[];if(m.length===0){const y=parseInt(((g=document.getElementById("qty-orden-"+l.producto_id))==null?void 0:g.value)||l.cantidad_sugerida);n[c].items.push({variante_id:null,cantidad:y,costo_unitario:l.costo_unitario})}else m.forEach(y=>{var v;const b=document.getElementById("chk-var-"+y.id);if(!(b!=null&&b.checked))return;const x=parseInt(((v=document.getElementById("qty-var-"+y.id))==null?void 0:v.value)||0);x<=0||n[c].items.push({variante_id:y.id,cantidad:x,costo_unitario:l.costo_unitario})})});try{for(const[l,c]of Object.entries(n)){if(c.items.length===0)continue;const p=c.items.reduce((y,b)=>y+b.cantidad*b.costo_unitario,0),g=(s=(await(await fetch(f+"/finanzas/ordenes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({proveedor_id:l==="sin-proveedor"?null:l,sucursal_id:t,status:"borrador",total:p,notas:o,fecha_entrega_estimada:a||null})})).json())[0])==null?void 0:s.id;if(g)for(const y of c.items)await fetch(f+"/finanzas/ordenes/"+g+"/items",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...y,subtotal:y.cantidad*y.costo_unitario})})}(d=document.getElementById("modal-orden"))==null||d.remove(),alert("Orden de compra guardada"),Y()}catch(l){alert("Error guardando orden: "+l.message)}};function Te(){return`
  ...
  `}async function Q(){var t,a,o,n,r,i;const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:#888">Cargando finanzas...</p>';try{const d=await(await fetch(f+"/sucursales/")).json(),l=(t=d[0])==null?void 0:t.id,[c,p,m,u,g,y,b]=await Promise.all([fetch(f+"/finanzas/caja/hoy/"+l),fetch(f+"/finanzas/reporte/"+l),fetch(f+"/finanzas/gastos/"+l),fetch(f+"/finanzas/estado-resultados/"+l),fetch(f+"/finanzas/flujo/"+l),fetch(f+"/finanzas/cuentas-por-cobrar"),fetch(f+"/finanzas/gastos-categorias/"+l)]),x=await c.json(),v=await p.json(),w=await m.json(),C=await u.json(),h=await g.json(),z=await y.json(),_=await b.json(),$=x.find(k=>k.status==="abierta"),j=new Date().toISOString().split("T")[0],M=w.filter(k=>{var T;return(T=k.created_at)==null?void 0:T.startsWith(j)}),S=M.reduce((k,T)=>k+parseFloat(T.monto||0),0),I=z.reduce((k,T)=>k+parseFloat(T.total||0),0);e.innerHTML=`
      <div style="margin-bottom:1.5rem;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
        <div>
    ...
    `,window._finanzasData={sucursalId:l,gastosHoy:M,totalGastosHoy:S,estadoResultados:C,flujo:h,cxc:z,categorias:_,historial:[]},window._finanzasSucursalId=l;const E=await(await fetch(f+"/finanzas/caja/historial/"+l)).json();window._finanzasData.historial=E,mostrarTabFinanzas("gastos")}catch(s){e.innerHTML='<p style="padding:2rem;color:red">Error cargando finanzas: '+s.message+"</p>"}}window.mostrarTabFinanzas=e=>{var c,p,m,u,g,y,b,x,v,w,C,h,z;const{gastosHoy:t,totalGastosHoy:a,estadoResultados:o,flujo:n,cxc:r,categorias:i,historial:s,sucursalId:d}=window._finanzasData,l=document.getElementById("fin-tab-contenido");if(l)if(e==="gastos"){const _=i.reduce(($,j)=>$+j.total,0);l.innerHTML=`
...
⚠️ Requiere que los productos estén en el catálogo de Meta Commerce.`;else if(d.style.background="#e8f5e9",d.style.borderColor="#a5d6a7",d.style.color="#333",a){let y;t==="personalizado"?y=(((g=document.getElementById("texto-personalizado"))==null?void 0:g.value)||"").replace("{nombre}","María"):y=a.mensaje("María"),d.textContent=y}}const l=document.getElementById("campana-count");l&&(l.textContent=r.length+" clientes");const c=document.getElementById("campana-lista");if(c){if(!r.length){c.innerHTML='<div style="padding:2rem;text-align:center;color:#888;font-size:0.85rem">No hay clientes en este segmento con teléfono registrado</div>';return}window._campanaFiltrados=r,window._campanaPlantillaId=t,c.innerHTML=r.map((y,b)=>{var C;let x;t==="personalizado"?x=(((C=document.getElementById("texto-personalizado"))==null?void 0:C.value)||"").replace("{nombre}",y.nombre.split(" ")[0]):x=a.mensaje(y.nombre.split(" ")[0]);const v=encodeURIComponent(x),w=(y.lada||"52")+y.telefono.replace(/\D/g,"");return`
    ...
    <div style="background:white;border-radius:16px;padding:2rem;max-width:440px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.3);text-align:center">
      ...
      <p style="font-size:0.82rem;color:#888">Conectando con WhatsApp Business</p>
        ...
        </div>`};window._campanaJobId=g;const x=setInterval(async()=>{try{const w=await(await fetch(f+"/campanas/estado/"+g)).json();b(w),w.terminado&&(clearInterval(x),window._campanaUltimosResultados=w.resultados||[],setTimeout(()=>{const C=w.enviados||0,h=w.fallidos||0;l.innerHTML=`
    ...
    `};window._campanaAvanzar=()=>{if(i++,i>=r.length){s.innerHTML=`
      ...
      `;return}d()},d(),document.body.appendChild(s)};async function Pe(){const e=document.getElementById("content");try{const[t,a]=await Promise.all([fetch(f+"/clientes/"),fetch(f+"/pedidos/")]),o=await t.json(),n=await a.json(),r=new Date,i=new Date(r-30*24*60*60*1e3),s=new Date(r-90*24*60*60*1e3),d=o.map(u=>{const g=n.filter(_=>_.cliente_id===u.id&&(_.status==="confirmado"||_.status==="pagado")),y=g.reduce((_,$)=>_+parseFloat($.total||0),0),b=g.length>0?new Date(g[0].created_at):null,x=g.filter(_=>new Date(_.created_at)>=i).length,v=b?Math.floor((r-b)/(1e3*60*60*24)):null;let w="nuevo",C="⚪ Nuevo",h="#f5f5f5",z="#888";return g.length===0?(w="nuevo",C="⚪ Sin compras",h="#f5f5f5",z="#888"):y>=5e3&&x>=1?(w="vip",C="⭐ VIP",h="#fff8e1",z="#f57f17"):v>90?(w="inactivo",C="🔴 Inactivo",h="#ffebee",z="#c62828"):v>30?(w="riesgo",C="🟡 En riesgo",h="#fff8e1",z="#f57f17"):x>=2?(w="frecuente",C="🟢 Frecuente",h="#e8f5e9",z="#2e7d32"):(w="activo",C="🔵 Activo",h="#e3f2fd",z="#1565c0"),{...u,totalGastado:y,ultimoPedido:b,pedidos30:x,diasSinComprar:v,segmento:w,segmentoLabel:C,segmentoBg:h,segmentoColor:z,totalPedidos:g.length}}).sort((u,g)=>g.totalGastado-u.totalGastado),l=d.filter(u=>u.segmento==="vip").length,c=d.filter(u=>u.segmento==="inactivo").length,p=d.filter(u=>u.segmento==="riesgo").length,m=d.filter(u=>u.segmento==="frecuente"||u.segmento==="activo").length;window._clientesData=d,e.innerHTML=`
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px;margin-bottom:1.5rem">
...
`,renderInventario()}catch{e.innerHTML='<p style="padding:2rem;color:red">Error conectando con el servidor</p>'}}window.renderInventario=()=>{const{sucursales:e,productos:t,variantes:a,inventario:o}=window._invData,n=(document.getElementById("inv-buscar")?document.getElementById("inv-buscar").value:"").toLowerCase(),r=document.getElementById("inv-categoria")?document.getElementById("inv-categoria").value:"",i=document.getElementById("inv-talla")?document.getElementById("inv-talla").value:"",s=document.getElementById("inv-estado")?document.getElementById("inv-estado").value:"",d=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],l=t.filter(m=>!(!m.activo||n&&!m.nombre.toLowerCase().includes(n)&&!(m.sku_interno||"").toLowerCase().includes(n)||r&&m.categoria!==r)),c=e.map(m=>{const u=o.filter(y=>y.sucursal_id===m.id),g=l.map(y=>{const b=a.filter(C=>C.producto_id===y.id);if(b.length===0)return"";const v=[...new Set(b.map(C=>C.color).filter(Boolean))].map(C=>{const h=b.filter(j=>j.color===C).sort((j,M)=>d.indexOf(j.talla)-d.indexOf(M.talla));if(i&&!h.find(j=>j.talla===i))return"";const z=h[0]?h[0].color_hex:"#888",_=h[0]?h[0].foto_url:null,$=h.map(j=>{const M=u.find(k=>k.variante_id===j.id),S=M?M.cantidad:null,I=M?M.stock_minimo:3;if(i&&j.talla!==i||s&&(s==="agotado"&&S!==0||s==="bajo"&&(S===null||S===0||S>I)||s==="disponible"&&(S===null||S===0||S<=I)))return"";let A,E;return S===null?(A="#f0f0f0",E="#aaa"):S===0?(A="#ffebee",E="#c62828"):S<=I?(A="#fff8e1",E="#f57f17"):(A="#e8f5e9",E="#2e7d32"),`
            <div style="display:flex;align-items:center;justify-content:space-between;background:${A};border-radius:10px;padding:8px 12px;border:1px solid ${E}30">
              ...
              <div style="display:flex;align-items:center;gap:8px">
                ...
                <span id="stock-${j.id}-${m.id}" style="font-size:1.1rem;font-weight:700;color:${E};min-width:32px;text-align:center">${S!==null?S:"—"}</span>
                <button onclick="cambiarStockInventario('${j.id}', '${m.id}', ${S!==null?S:0}, ${I}, 1)"
                        style="background:#fff;border:1px solid #ddd;border-radius:6px;width:34px;height:34px;cursor:pointer;font-size:1.2rem;font-weight:700;touch-action:manipulation">+</button>
              </div>
            </div>
  ...
  `};window.guardarSalida=async()=>{const e=document.getElementById("sal").value,t=document.getElementById("sal-sucursal").value,a=document.getElementById("sal-cantidad").value,o=document.getElementById("sal-motivo").value;if(!e||!t||!a){alert("Por favor completa todos los campos");return}try{const r=await(await fetch(f+"/movimientos/entrada",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({variante_id:e,sucursal_id:t,cantidad:-parseInt(a),motivo:o})})).json();r.ok?(alert("Salida registrada. Anterior: "+r.cantidad_anterior+" pares ÔåÆ Nuevo: "+r.cantidad_nueva+" pares"),navegarA("inventario")):alert("Error: "+JSON.stringify(r))}catch{alert("Error conectando con el servidor")}};window.mostrarInventarioMasivo=async()=>{const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:#888">Cargando...</p>';try{const a=await(await fetch(f+"/sucursales/")).json(),n=await(await fetch(f+"/productos/")).json(),i=await(await fetch(f+"/variantes/")).json(),d=await(await fetch(f+"/inventario/")).json(),l=[...new Set(n.map(p=>p.categoria).filter(Boolean))],c=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"];window._invMasivo={sucursales:a,productos:n,variantes:i,inventario:d},e.innerHTML=`
    ...
    `,renderTablasMasivo()}catch{e.innerHTML='<p style="padding:2rem;color:red">Error cargando inventario</p>'}};window.renderTablasMasivo=()=>{const{productos:e,variantes:t,inventario:a}=window._invMasivo,o=document.getElementById("im-sucursal").value,n=document.getElementById("im-categoria").value,r=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],i=e.filter(c=>!(n&&c.categoria!==n)),s=a.filter(c=>c.sucursal_id===o),d=i.map(c=>{const p=t.filter(g=>g.producto_id===c.id);if(p.length===0)return"";const u=[...new Set(p.map(g=>g.color).filter(Boolean))].map(g=>{const y=p.filter(x=>x.color===g).sort((x,v)=>r.indexOf(x.talla)-r.indexOf(v.talla));return`
        <div style="margin-bottom:1rem">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            ${y.map(x=>{const v=s.find(z=>z.variante_id===x.id),w=v?v.cantidad:0,C=v?v.stock_minimo:3;let h="#ddd";return w===0?h="#ffcdd2":w<=C?h="#ffe082":h="#a5d6a7",`
                <div style="text-align:center">
...
<td>
  ...
  <button class="btn btn-secondary" style="padding:4px 8px;font-size:0.72rem;color:#c62828;border-color:#c62828" 
    ...
    Cancelar
        ...
        </tr>`}).join(""))};async function De(){try{const t=await(await fetch(f+"/pedidos/")).json(),o=await(await fetch(f+"/clientes/")).json(),r=await(await fetch(f+"/inventario/alertas")).json(),i=new Date;i.setHours(0,0,0,0);const s=new Date(i);s.setDate(s.getDate()-7);const d=new Date(i);d.setDate(d.getDate()-30);const l=t.filter(E=>E.status==="confirmado"||E.status==="pagado"),c=l.filter(E=>new Date(E.created_at)>=i),p=l.filter(E=>new Date(E.created_at)>=s),m=c.reduce((E,k)=>E+parseFloat(k.total||0),0),u=p.reduce((E,k)=>E+parseFloat(k.total||0),0),g=o.filter(E=>E.created_at&&new Date(E.created_at)>=d).length,y=["Dom","Lun","Mar","Mie","Jue","Vie","Sab"],b={};y.forEach(E=>b[E]=0),l.filter(E=>new Date(E.created_at)>=d).forEach(E=>{const k=y[new Date(E.created_at).getDay()];b[k]+=parseFloat(E.total||0)});const x={};l.forEach(E=>{x[E.canal||"sucursal"]=(x[E.canal||"sucursal"]||0)+parseFloat(E.total||0)});const v={};l.forEach(E=>{v[E.forma_pago||"efectivo"]=(v[E.forma_pago||"efectivo"]||0)+1});const w={};l.forEach(E=>{w[E.empleado||"Admin"]=(w[E.empleado||"Admin"]||0)+parseFloat(E.total||0)});const C={};l.forEach(E=>{const k=new Date(E.created_at).toLocaleDateString("es-MX",{month:"short",year:"numeric"});C[k]=(C[k]||0)+parseFloat(E.total||0)});const h={};l.forEach(E=>{E.clientes&&(h[E.clientes.nombre]=(h[E.clientes.nombre]||0)+parseFloat(E.total||0))});const z=Object.entries(h).sort((E,k)=>k[1]-E[1]).slice(0,5),_=Object.entries(b).sort((E,k)=>k[1]-E[1])[0],$=Object.entries(w).sort((E,k)=>k[1]-E[1])[0],j=document.getElementById("dashboard-contenido");if(!j)return;const M=j.querySelectorAll(".stat-card"),S=[{val:"$"+m.toFixed(0),sub:c.length+" pedidos hoy",color:"var(--pink)"},{val:c.length,sub:"confirmados hoy"},{val:"$"+u.toFixed(0),sub:p.length+" pedidos"},{val:g,sub:"ultimos 30 dias"},{val:r.length,sub:"por reabastecer",color:r.length>0?"var(--amber)":"var(--green)"},{val:_?_[0]:"—",sub:_?"$"+_[1].toFixed(0)+" prom.":""},{val:$?$[0]:"—",sub:$?"$"+$[1].toFixed(0):"",small:!0},{val:o.length,sub:"registrados"}];M.forEach((E,k)=>{if(!S[k])return;const T=E.querySelector(".stat-value"),B=E.querySelector(".stat-sub");T&&(T.textContent=S[k].val,T.style.color=S[k].color||"var(--text-primary)",S[k].small&&(T.style.fontSize="1rem")),B&&(B.textContent=S[k].sub||"")});const I=document.getElementById("dash-top-clientes");I&&(I.innerHTML=z.length===0?'<p style="color:var(--text-muted);font-size:0.85rem">Sin datos aun</p>':z.map(([E,k],T)=>`
