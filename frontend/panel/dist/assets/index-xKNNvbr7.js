...
¿Quieres reactivar todos?`)&&(localStorage.removeItem("ordenes_pospuestos"),re())};window.guardarOrdenCompra2=async()=>{var i,s,r,d;const{seleccionados:e,sucursalId:t}=window._ordenModal||{};if(!e)return;const o=(i=document.getElementById("orden-fecha"))==null?void 0:i.value,a=((s=document.getElementById("orden-notas"))==null?void 0:s.value)||"",n={};e.forEach(l=>{var u,g;const c=l.proveedor_id||"sin-proveedor",p=((u=l.proveedor)==null?void 0:u.nombre)||"Sin proveedor";n[c]||(n[c]={nombre:p,items:[]});const m=l.variantes||[];if(m.length===0){const y=parseInt(((g=document.getElementById("qty-orden-"+l.producto_id))==null?void 0:g.value)||l.cantidad_sugerida);n[c].items.push({variante_id:null,cantidad:y,costo_unitario:l.costo_unitario})}else m.forEach(y=>{var v;const b=document.getElementById("chk-var-"+y.id);if(!(b!=null&&b.checked))return;const h=parseInt(((v=document.getElementById("qty-var-"+y.id))==null?void 0:v.value)||0);h<=0||n[c].items.push({variante_id:y.id,cantidad:h,costo_unitario:l.costo_unitario})})});try{for(const[l,c]of Object.entries(n)){if(c.items.length===0)continue;const p=c.items.reduce((y,b)=>y+b.cantidad*b.costo_unitario,0),g=(r=(await(await fetch(f+"/finanzas/ordenes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({proveedor_id:l==="sin-proveedor"?null:l,sucursal_id:t,status:"borrador",total:p,notas:a,fecha_entrega_estimada:o||null})})).json())[0])==null?void 0:r.id;if(g)for(const y of c.items)await fetch(f+"/finanzas/ordenes/"+g+"/items",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...y,subtotal:y.cantidad*y.costo_unitario})})}(d=document.getElementById("modal-orden"))==null||d.remove(),alert("Orden de compra guardada"),re()}catch(l){alert("Error guardando orden: "+l.message)}};function Ye(){var o;const e=new Date().toLocaleDateString("es-MX",{weekday:"long",day:"numeric",month:"long"});return`
  ...
  `}async function se(){var t,o,a,n,i,s;const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:#888">Cargando finanzas...</p>';try{const d=await(await fetch(f+"/sucursales/")).json(),l=(t=d[0])==null?void 0:t.id,[c,p,m,u,g,y,b]=await Promise.all([fetch(f+"/finanzas/caja/hoy/"+l),fetch(f+"/finanzas/reporte/"+l),fetch(f+"/finanzas/gastos/"+l),fetch(f+"/finanzas/estado-resultados/"+l),fetch(f+"/finanzas/flujo/"+l),fetch(f+"/finanzas/cuentas-por-cobrar"),fetch(f+"/finanzas/gastos-categorias/"+l)]),h=await c.json(),v=await p.json(),w=await m.json(),E=await u.json(),x=await g.json(),P=await y.json(),L=await b.json(),$=h.find(B=>B.status==="abierta"),z=new Date().toISOString().split("T")[0],q=w.filter(B=>{var _;return(_=B.created_at)==null?void 0:_.startsWith(z)}),C=q.reduce((B,_)=>B+parseFloat(_.monto||0),0),T=P.reduce((B,_)=>B+parseFloat(_.total||0),0);e.innerHTML=`
      ...
      <div style="background:white;border-radius:12px;border:1px solid #eee;overflow:hidden">
        ...
        ${$.length===0?'<p style="padding:2rem;text-align:center;color:#888">Sin ventas con costo registrado en los últimos 30 días</p>':`<div style="overflow-x:auto">
            <table style="width:100%;border-collapse:collapse;font-size:0.83rem">
              ...
              <tbody>
                ${$.map(B=>{const _=B.subtotal_venta>0?(B.subtotal_venta-B.subtotal_costo)/B.subtotal_venta*100:0,M=_>=30?"#2e7d32":_>=15?"#f57f17":"#c62828";return`<tr style="border-bottom:1px solid #f5f5f5">
    ...
    `}else if(e==="gastos"){const $=s.reduce((z,q)=>z+q.total,0);l.innerHTML=`
  ...
  `,document.body.appendChild(o),o.addEventListener("click",a=>{a.target===o&&o.remove()})};window.guardarProveedor=async e=>{const t={nombre:document.getElementById("prov-nombre").value,contacto:document.getElementById("prov-contacto").value,telefono:document.getElementById("prov-telefono").value,email:document.getElementById("prov-email").value,ciudad:document.getElementById("prov-ciudad").value,direccion:document.getElementById("prov-direccion").value,notas:document.getElementById("prov-notas").value};if(!t.nombre){alert("El nombre es requerido");return}try{e?await fetch(f+"/finanzas/proveedores/"+e,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}):await fetch(f+"/finanzas/proveedores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}),document.querySelector('div[style*="position:fixed"]').remove(),je()}catch{alert("Error guardando proveedor")}};window.editarProveedor=async e=>{const a=(await(await fetch(f+"/finanzas/proveedores")).json()).find(n=>n.id===e);a&&mostrarFormProveedor(a)};async function Qe(){const e=document.getElementById("content");e.innerHTML=`
    ...
    <style>@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}</style>`;try{let B=function(k,O,F){return`<div style="display:flex;align-items:center;gap:6px">
        ...
        <div id="rotacion-lista" style="display:flex;flex-direction:column;gap:8px">
          ${z.map(k=>{const O=S[k.semaforo],F=Math.max(...z.map(j=>j.ventas.d30),1);return`
        ...
        </div>`},M=function(k){k=(k||"").toLowerCase();const O=z.filter(A=>(A.tallasDesglosadas.length>0||A.tallasData.some(R=>R.total>0))&&(!k||A.nombre.toLowerCase().includes(k)||(A.sku_interno||"").toLowerCase().includes(k))),F=z.filter(A=>A.tallasData.some(R=>R.total>0)).length,j=`
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px;padding:9px 14px;background:#f8fafc;border-radius:10px;border:1px solid #e2e8f0;flex-wrap:wrap">
        ...
        </div>`;function G(A){const R=Math.max(...A.map(V=>V.total),1);return A.map(V=>{const H=Math.round(V.total/R*100),W=H===0?"#f8fafc":H<20?"#fce7f3":H<50?"#fbcfe8":H<80?"#f472b6":"#E91E8C",U=H>=50?"white":H===0?"#cbd5e1":"#be185d",Q=H>=50?"rgba(255,255,255,0.75)":"#db2777";return`
          <div style="background:${W};border-radius:7px;padding:8px 5px;text-align:center;transition:transform 0.15s;min-width:58px;${H===0?"border:1px solid #e2e8f0":""}"
            ...
            <p style="font-size:0.9rem;font-weight:800;color:${U};line-height:1;margin-bottom:2px">${V.total}</p>
            <p style="font-size:0.65rem;font-weight:700;color:${U}">T${V.talla}</p>
              ...
              ${V?A.tallasDesglosadas.map(H=>`
                  <div style="margin-bottom:12px">
                    <div style="display:flex;align-items:center;gap:6px;margin-bottom:7px">
                      ...
                      ${H.tallas.find(W=>W.total===Math.max(...H.tallas.map(U=>U.total)))?`<span style="font-size:0.65rem;color:#be185d;background:#fdf2f8;padding:1px 7px;border-radius:100px">
                            top: T${H.tallas.find(W=>W.total===Math.max(...H.tallas.map(U=>U.total))).talla}
                          </span>`:""}
                    </div>
      ...
      `},D=function(){const k=z.filter(O=>O.variantesData.some(F=>F.total>0));return k.length===0?'<div style="padding:3rem;text-align:center;color:#94a3b8;font-size:0.9rem">No hay datos de ventas por variante aún</div>':k.map(O=>{const F=O.variantesData.slice(0,8),j=Math.max(...F.map(A=>A.total),1),G=F[0];return`
        <div style="background:white;border:1px solid #e2e8f0;border-radius:12px;padding:16px;margin-bottom:10px">
        ...
        </div>`}).join("")};var t=B,o=_,a=M,n=D;const[i,s,r,d]=await Promise.all([fetch(f+"/productos/"),fetch(f+"/variantes/?activa=eq.true"),fetch(f+"/movimientos/"),fetch(f+"/inventario/slim")]),l=await i.json(),c=await s.json(),p=await r.json(),m=await d.json(),u={},g={};c.forEach(k=>{u[k.id]=k,g[k.producto_id]||(g[k.producto_id]=[]),g[k.producto_id].push(k.id)});const y={};m.forEach(k=>{y[k.variante_id]=(y[k.variante_id]||0)+k.cantidad});const b=new Date,h=new Date(b-30*24*60*60*1e3),v=new Date(b-60*24*60*60*1e3),w=new Date(b-90*24*60*60*1e3),E={},x={},P={},L={};p.filter(k=>k.tipo==="venta").forEach(k=>{const O=u[k.variante_id];if(!O)return;const F=O.producto_id,j=Math.abs(k.cantidad),G=new Date(k.created_at);E[F]||(E[F]={d30:0,d60:0,d90:0,total:0}),E[F].total+=j,G>=h&&(E[F].d30+=j),G>=v&&(E[F].d60+=j),G>=w&&(E[F].d90+=j),x[k.variante_id]||(x[k.variante_id]={d30:0,d90:0,total:0}),x[k.variante_id].total+=j,G>=h&&(x[k.variante_id].d30+=j),G>=w&&(x[k.variante_id].d90+=j);const A=O.talla||"S/T",R=`${F}|${A}`;P[R]||(P[R]={talla:A,d30:0,d90:0,total:0}),P[R].total+=j,G>=h&&(P[R].d30+=j),G>=w&&(P[R].d90+=j);const V=O.color||"Sin color",H=`${F}|${V}|${A}`;L[H]||(L[H]={talla:A,color:V,d30:0,d90:0,total:0}),L[H].total+=j,G>=h&&(L[H].d30+=j),G>=w&&(L[H].d90+=j)});const z=l.map(k=>{const O=E[k.id]||{d30:0,d60:0,d90:0,total:0},F=g[k.id]||[],j=F.reduce((N,X)=>N+(y[X]||0),0),G=O.d30/4,A=G>0?Math.round(j/G*7):null;let R="gris",V="Sin ventas recientes";O.d30>=6?(R="verde",V="Rota bien — considerar resurtido"):O.d30>=2?(R="amarillo",V="Rotación moderada"):O.d90===0&&j>0?(R="rojo",V="Sin movimiento en 90 días — revisar"):O.d30>0&&(R="amarillo",V="Rotación lenta");const H=F.map(N=>{var X;return(X=u[N])==null?void 0:X.talla}).filter(Boolean).filter((N,X,Z)=>Z.indexOf(N)===X).map(N=>({talla:N,...P[`${k.id}|${N}`]||{d30:0,d90:0,total:0}})).sort((N,X)=>X.total-N.total),U=[...new Set(F.map(N=>{var X;return(X=u[N])==null?void 0:X.color}).filter(Boolean))].map(N=>{const X=F.filter(J=>{var Y;return((Y=u[J])==null?void 0:Y.color)===N}).map(J=>{var Y;return(Y=u[J])==null?void 0:Y.talla}).filter(Boolean).filter((J,Y,Ue)=>Ue.indexOf(J)===Y).map(J=>({talla:J,...L[`${k.id}|${N}|${J}`]||{d30:0,d90:0,total:0}})).sort((J,Y)=>parseFloat(J.talla)-parseFloat(Y.talla)),Z=X.reduce((J,Y)=>J+Y.total,0);return{color:N,tallas:X,total:Z}}).filter(N=>N.total>0).sort((N,X)=>X.total-N.total),Q=F.map(N=>{const X=u[N];if(!X)return null;const Z=x[N]||{d30:0,d90:0,total:0},J=y[N]||0;return{id:N,label:[X.talla,X.color].filter(Boolean).join(" / ")||N,stock:J,...Z}}).filter(Boolean).sort((N,X)=>X.total-N.total);return{...k,ventas:O,stockTotal:j,ventasSemana:G,diasInventario:A,semaforo:R,recomendacion:V,tallasData:H,tallasDesglosadas:U,variantesData:Q}}).sort((k,O)=>O.ventas.d30-k.ventas.d30);window._analisisData={productosConRotacion:z,ventasPorTalla:P};const q=z.reduce((k,O)=>k+O.ventas.d30,0),C=z.reduce((k,O)=>k+O.ventas.d90,0),T=z.filter(k=>k.semaforo==="verde").length,I=z.filter(k=>k.semaforo==="rojo").length,S={verde:{bg:"#dcfce7",color:"#166534",dot:"#16a34a",txt:"Rota bien"},amarillo:{bg:"#fef9c3",color:"#854d0e",dot:"#ca8a04",txt:"Rotación lenta"},rojo:{bg:"#fee2e2",color:"#991b1b",dot:"#dc2626",txt:"Sin movimiento"},gris:{bg:"#f1f5f9",color:"#64748b",dot:"#94a3b8",txt:"Sin datos"}};window._renderTabRotacion=_,window._renderTabTallas=M,window._renderTabVariantes=D,e.innerHTML=`
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:10px;margin-bottom:1.5rem">
        ...
        <div style="background:linear-gradient(135deg,#f5f3ff,#ede9fe);border:1px solid #ddd6fe;border-radius:14px;padding:1rem 1.1rem">
          <p style="font-size:1.5rem;font-weight:800;color:#6d28d9;line-height:1;margin-bottom:3px">${C}</p>
...
⚠️ Requiere que los productos estén en el catálogo de Meta Commerce.`;else if(d.style.background="#e8f5e9",d.style.borderColor="#a5d6a7",d.style.color="#333",o){let y;t==="personalizado"?y=(((g=document.getElementById("texto-personalizado"))==null?void 0:g.value)||"").replace("{nombre}","María"):y=o.mensaje("María"),d.textContent=y}}const l=document.getElementById("campana-count");l&&(l.textContent=i.length+" clientes");const c=document.getElementById("campana-lista");if(c){if(!i.length){c.innerHTML='<div style="padding:2rem;text-align:center;color:#888;font-size:0.85rem">No hay clientes en este segmento con teléfono registrado</div>';return}window._campanaFiltrados=i,window._campanaPlantillaId=t,c.innerHTML=i.map((y,b)=>{var E;let h;t==="personalizado"?h=(((E=document.getElementById("texto-personalizado"))==null?void 0:E.value)||"").replace("{nombre}",y.nombre.split(" ")[0]):h=o.mensaje(y.nombre.split(" ")[0]);const v=encodeURIComponent(h),w=(y.lada||"52")+y.telefono.replace(/\D/g,"");return`
...
`,renderInventario()}catch{e.innerHTML='<p style="padding:2rem;color:red">Error conectando con el servidor</p>'}}window.renderInventario=()=>{const{sucursales:e,productos:t,variantes:o,inventario:a}=window._invData,n=(document.getElementById("inv-buscar")?document.getElementById("inv-buscar").value:"").toLowerCase(),i=document.getElementById("inv-categoria")?document.getElementById("inv-categoria").value:"",s=document.getElementById("inv-talla")?document.getElementById("inv-talla").value:"",r=document.getElementById("inv-estado")?document.getElementById("inv-estado").value:"",d=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],l=t.filter(m=>!(!m.activo||n&&!m.nombre.toLowerCase().includes(n)&&!(m.sku_interno||"").toLowerCase().includes(n)||i&&m.categoria!==i)),c=e.map(m=>{const u=a.filter(y=>y.sucursal_id===m.id),g=l.map(y=>{const b=o.filter(E=>E.producto_id===y.id&&E.activa!==!1);if(b.length===0)return"";const v=[...new Set(b.map(E=>E.color).filter(Boolean))].map(E=>{const x=b.filter(z=>z.color===E).sort((z,q)=>d.indexOf(z.talla)-d.indexOf(q.talla));if(s&&!x.find(z=>z.talla===s))return"";const P=x[0]?x[0].color_hex:"#888",L=x[0]?x[0].foto_url:null,$=x.map(z=>{const q=u.find(B=>B.variante_id===z.id),C=q?q.cantidad:null,T=q?q.stock_minimo:3;if(s&&z.talla!==s||r&&(r==="agotado"&&C!==0||r==="bajo"&&(C===null||C===0||C>T)||r==="disponible"&&(C===null||C===0||C<=T)))return"";let I,S;return C===null?(I="#f0f0f0",S="#aaa"):C===0?(I="#ffebee",S="#c62828"):C<=T?(I="#fff8e1",S="#f57f17"):(I="#e8f5e9",S="#2e7d32"),`
            <div style="display:flex;align-items:center;justify-content:space-between;background:${I};border-radius:10px;padding:8px 12px;border:1px solid ${S}30">
              ...
              <div style="display:flex;align-items:center;gap:8px">
                <button onclick="cambiarStockInventario('${z.id}', '${m.id}', ${C!==null?C:0}, ${T}, -1)"
                ...
                <span id="stock-${z.id}-${m.id}" style="font-size:1.1rem;font-weight:700;color:${S};min-width:32px;text-align:center">${C!==null?C:"—"}</span>
                <button onclick="cambiarStockInventario('${z.id}', '${m.id}', ${C!==null?C:0}, ${T}, 1)"
                        style="background:#fff;border:1px solid #ddd;border-radius:6px;width:34px;height:34px;cursor:pointer;font-size:1.2rem;font-weight:700;touch-action:manipulation">+</button>
              </div>
            </div>
  ...
  `};window.guardarSalida=async()=>{const e=document.getElementById("sal").value,t=document.getElementById("sal-sucursal").value,o=document.getElementById("sal-cantidad").value,a=document.getElementById("sal-motivo").value;if(!e||!t||!o){alert("Por favor completa todos los campos");return}try{const i=await(await fetch(f+"/movimientos/entrada",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({variante_id:e,sucursal_id:t,cantidad:-parseInt(o),motivo:a})})).json();i.ok?(alert("Salida registrada. Anterior: "+i.cantidad_anterior+" pares ÔåÆ Nuevo: "+i.cantidad_nueva+" pares"),navegarA("inventario")):alert("Error: "+JSON.stringify(i))}catch{alert("Error conectando con el servidor")}};window.mostrarInventarioMasivo=async()=>{const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:#888">Cargando...</p>';try{const o=await(await fetch(f+"/sucursales/")).json(),n=await(await fetch(f+"/productos/")).json(),s=await(await fetch(f+"/variantes/")).json(),d=await(await fetch(f+"/inventario/")).json(),l=[...new Set(n.map(p=>p.categoria).filter(Boolean))],c=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"];window._invMasivo={sucursales:o,productos:n,variantes:s,inventario:d},e.innerHTML=`
    ...
    `,renderTablasMasivo()}catch{e.innerHTML='<p style="padding:2rem;color:red">Error cargando inventario</p>'}};window.renderTablasMasivo=()=>{const{productos:e,variantes:t,inventario:o}=window._invMasivo,a=document.getElementById("im-sucursal").value,n=document.getElementById("im-categoria").value,i=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],s=e.filter(c=>!(n&&c.categoria!==n)),r=o.filter(c=>c.sucursal_id===a),d=s.map(c=>{const p=t.filter(g=>g.producto_id===c.id);if(p.length===0)return"";const u=[...new Set(p.map(g=>g.color).filter(Boolean))].map(g=>{const y=p.filter(h=>h.color===g).sort((h,v)=>i.indexOf(h.talla)-i.indexOf(v.talla));return`
        <div style="margin-bottom:1rem">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            ${y.map(h=>{const v=r.find(P=>P.variante_id===h.id),w=v?v.cantidad:0,E=v?v.stock_minimo:3;let x="#ddd";return w===0?x="#ffcdd2":w<=E?x="#ffe082":x="#a5d6a7",`
                <div style="text-align:center">
...
<td>
  ...
  <button class="btn btn-secondary" style="padding:4px 8px;font-size:0.72rem;color:#c62828;border-color:#c62828" 
    ...
    Cancelar
        ...
        </tr>`}).join(""))};async function it(){try{const t=await(await fetch(f+"/pedidos/")).json(),a=await(await fetch(f+"/clientes/")).json(),i=await(await fetch(f+"/inventario/alertas")).json(),s=new Date;s.setHours(0,0,0,0);const r=new Date(s);r.setDate(r.getDate()-7);const d=new Date(s);d.setDate(d.getDate()-30);const l=t.filter(_=>["confirmado","pagado","enviado"].includes(_.status)),c=l.filter(_=>new Date(_.created_at)>=s),p=l.filter(_=>new Date(_.created_at)>=r),m=c.reduce((_,M)=>_+parseFloat(M.total||0),0),u=p.reduce((_,M)=>_+parseFloat(M.total||0),0),g=a.filter(_=>_.created_at&&new Date(_.created_at)>=d).length,y=["Dom","Lun","Mar","Mie","Jue","Vie","Sab"],b={};y.forEach(_=>b[_]=0),l.filter(_=>new Date(_.created_at)>=d).forEach(_=>{const M=y[new Date(_.created_at).getDay()];b[M]+=parseFloat(_.total||0)});const h={};l.forEach(_=>{h[_.canal||"sucursal"]=(h[_.canal||"sucursal"]||0)+parseFloat(_.total||0)});const v={};l.forEach(_=>{v[_.forma_pago||"efectivo"]=(v[_.forma_pago||"efectivo"]||0)+1});const w={};l.forEach(_=>{w[_.empleado||"Admin"]=(w[_.empleado||"Admin"]||0)+parseFloat(_.total||0)});const E={};l.forEach(_=>{const M=new Date(_.created_at).toLocaleDateString("es-MX",{month:"short",year:"numeric"});E[M]=(E[M]||0)+parseFloat(_.total||0)});const x={};l.forEach(_=>{_.clientes&&(x[_.clientes.nombre]=(x[_.clientes.nombre]||0)+parseFloat(_.total||0))});const P=Object.entries(x).sort((_,M)=>M[1]-_[1]).slice(0,5),L=Object.entries(b).sort((_,M)=>M[1]-_[1])[0],$=Object.entries(w).sort((_,M)=>M[1]-_[1])[0];if(!document.getElementById("dashboard-contenido"))return;const q=l.filter(_=>new Date(_.created_at)>=d),C=q.reduce((_,M)=>_+parseFloat(M.total||0),0),T=[];for(let _=6;_>=0;_--){const M=new Date(s);M.setDate(M.getDate()-_);const D=M.toISOString().split("T")[0],k=M.toLocaleDateString("es-MX",{weekday:"short",day:"numeric"}),O=l.filter(F=>{var j;return(j=F.created_at)==null?void 0:j.startsWith(D)}).reduce((F,j)=>F+parseFloat(j.total||0),0);T.push({label:k,total:O,ds:D})}const I=(_,M,D,k)=>{const O=document.getElementById(_),F=document.getElementById(_+"-sub");O&&(O.textContent=M,k&&(O.style.color=k)),F&&D&&(F.textContent=D)};I("kpi-ventas-hoy","$"+m.toLocaleString("es-MX",{maximumFractionDigits:0}),c.length+" pedidos","#C8967A"),I("kpi-pedidos-hoy",c.length,"confirmados"),I("kpi-ventas-7d","$"+u.toLocaleString("es-MX",{maximumFractionDigits:0}),p.length+" pedidos"),I("kpi-ventas-30d","$"+C.toLocaleString("es-MX",{maximumFractionDigits:0}),q.length+" pedidos"),I("kpi-clientes-nuevos",g,"últimos 30 días"),I("kpi-stock-bajo",i.length,i.length>0?"⚠ reabastecer":"✓ ok",i.length>0?"#f59e0b":"#10b981"),I("kpi-mejor-dia",L?L[0]:"—",L?"$"+L[1].toLocaleString("es-MX",{maximumFractionDigits:0}):""),I("kpi-total-clientes",a.length,"registrados");const S=document.getElementById("dash-top-clientes");S&&(S.innerHTML=P.length===0?'<p style="color:var(--text-muted);font-size:0.85rem">Sin datos aun</p>':P.map(([_,M],D)=>`
          ...
          ${i.length===0?'<div class="table-card" style="padding:2rem;text-align:center;color:var(--text-muted)">No hay pedidos pendientes de pago 🎉</div>':`<div class="table-card" style="padding:0;overflow:hidden">
            <div style="overflow-x:auto">
              <table style="width:100%;border-collapse:collapse;font-size:0.85rem">
                ...
                <tbody>
                  ${i.map(l=>{var h;const c=l.nombre_cliente||"—",p=l.email_cliente||"",m=l.telefono_cliente||"",u=l.horas_pendiente!=null?l.horas_pendiente<1?"hace &lt;1h":`hace ${l.horas_pendiente}h`:"—",g=!!l.recordatorio_pago_enviado_at,y=encodeURIComponent(`Hola ${c.split(" ")[0]}, te escribimos de Zapatillas May. Vimos que elegiste pago por ${((h=l.forma_pago)==null?void 0:h.toUpperCase())||"OXXO/SPEI"} para tu pedido de $${parseFloat(l.total||0).toFixed(0)} MXN pero aún no lo vemos acreditado. ¿Pudiste realizar el pago? Si tienes dudas con placer te ayudamos 😊`),b=m?`https://wa.me/52${m.replace(/\D/g,"")}?text=${y}`:"";return`<tr style="border-bottom:1px solid var(--border)">
                      <td style="padding:10px 14px">
                      ...
                      <td style="padding:10px 14px;color:var(--text-muted);font-size:0.8rem">${u}<br><span style="font-size:0.7rem">${s(l.created_at)}</span></td>
                      <td style="padding:10px 14px">
                        <div style="display:flex;gap:6px;flex-wrap:wrap">
                          ${p?`<button onclick="enviarRecordatorioPago('${l.id}', this)" style="padding:5px 12px;border-radius:20px;border:1.5px solid #1a56db;background:none;color:#1a56db;font-size:0.75rem;font-weight:600;cursor:pointer">${g?"📧 Re-enviar":"📧 Email"}</button>`:""}
                          ${b?`<a href="${b}" target="_blank" style="padding:5px 12px;border-radius:20px;border:1.5px solid #25D366;background:none;color:#15803d;font-size:0.75rem;font-weight:600;text-decoration:none;display:inline-block">💬 WhatsApp</a>`:""}
                        </div>
                        ${g?`<div style="font-size:0.68rem;color:#aaa;margin-top:4px">Avisado ${s(l.recordatorio_pago_enviado_at)}</div>`:""}
                      </td>
                    </tr>`}).join("")}
                </tbody>
              </table>
...
`})})),n.textContent=b}}catch(m){n.style.display="block",n.textContent="Error inesperado: "+m.message}finally{t.innerHTML=i,t.disabled=!1}}window.mlPublicarPreview=function(e){Ve(!0,e)};window.mlPublicarReal=function(e){confirm("¿Publicar TODAS las variantes activas de este producto en MercadoLibre? Esto creará nuevos items en tu cuenta.")&&Ve(!1,e)};window.descargarExcelTikTok=async function(e,t,o){const a=e.innerHTML;try{e.innerHTML="⏳ Generando...",e.disabled=!0;const n=await fetch(`${f}/tiktok/${t}`);if(!n.ok){const d=await n.text();throw new Error(`Error ${n.status}: ${d}`)}const i=await n.blob(),s=URL.createObjectURL(i),r=document.createElement("a");r.href=s,r.download=o,document.body.appendChild(r),r.click(),document.body.removeChild(r),URL.revokeObjectURL(s)}catch(n){alert("Error al descargar el archivo: "+n.message)}finally{e.innerHTML=a,e.disabled=!1}};async function ct(){const e=document.getElementById("content");e.innerHTML=`
  ...
  `,document.getElementById("ml-titulo").addEventListener("input",function(){document.getElementById("ml-titulo-count").textContent=this.value.length}),window.mlGenerarPreview=async()=>{var d,l,c,p;const o=document.getElementById("ml-sku").value.trim().toUpperCase(),a=document.getElementById("ml-listing").value,n=document.getElementById("ml-titulo").value.trim(),i=document.getElementById("ml-precio").value.trim(),s=i?parseFloat(i):null;if(!o){alert("Ingresa el SKU del producto");return}if(!n){alert("Ingresa el título para ML");return}if(n.length>60){alert("El título no puede superar 60 caracteres");return}if(s!==null&&(isNaN(s)||s<=0)){alert("El precio debe ser mayor a 0");return}const r=document.getElementById("ml-btn-preview");r.textContent="⏳ Generando...",r.disabled=!0;try{const m=await fetch(`${f}/ml/publicar`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sku_interno:o,listing_type:a,solo_preview:!0})}),u=await m.json();if(!m.ok){alert("Error: "+(u.detail||JSON.stringify(u)));return}const g=u.resultados||[];if(!g.length){alert("No se encontraron variantes activas");return}const y=document.getElementById("ml-descripcion"),b=((c=(l=(d=g[0])==null?void 0:d.preview)==null?void 0:l.description)==null?void 0:c.plain_text)||"";y.value.trim()||(y.value=b);const h={},v=[],w=new Set;for(const z of g){const q=z.color||"Sin color";h[q]||(h[q]=[]);for(const C of((p=z.preview)==null?void 0:p.pictures)||[])C.source&&(h[q].find(T=>T.source===C.source)||h[q].push(C),w.has(C.source)||(w.add(C.source),v.push(C)))}const E={},x=new Set;for(const z of Object.keys(h))E[z]=0,x.add(z);const P=Object.keys(h);if(P.length>0){const z=document.getElementById("ml-fotos-wrap"),q=document.getElementById("ml-fotos-grid"),C=()=>{T();const I=g.filter(S=>x.has(S.color||"Sin color"));t(I,n,s,E,y.value.trim(),h)},T=()=>{q.innerHTML=P.map(I=>{const S=h[I];if(!S.length)return"";const B=x.has(I),_=I.replace(/'/g,"\\'"),M=I.replace(/[^a-zA-Z0-9]/g,"_");return`
              <div style="margin-bottom:1rem;width:100%;opacity:${B?1:.4}">
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-bottom:6px">
                  ...
                  <span style="font-size:0.72rem;color:#888">(${g.filter(D=>(D.color||"Sin color")===I).length} tallas)</span>
                </label>
          ...
          ${(()=>{const m={};return a.forEach((u,g)=>{const y=u.variantes||{},b=y.productos||{},h=b.nombre||u.nombre||"—",v=y.color||u.color||"",w=!!u.es_corrida,E=w?"c|"+h+"|"+v:"s|"+u.id;m[E]||(m[E]={nombre:h,color:v,esCorrida:w,items:[],imagen:y.foto_url||b.imagen_principal||null}),m[E].items.push({...u,_idx:g})}),Object.values(m).map(u=>{const g=u.esCorrida,y=u.imagen;if(g){const b=u.items.reduce((E,x)=>E+x.cantidad,0),h=u.items.reduce((E,x)=>E+x.cantidad*x.precio_unitario,0),v=(h/b).toFixed(2),w=u.items.map(E=>E.id).join(",");return`
                  <div style="background:#fdf4ff;border-radius:8px;padding:12px;margin-bottom:8px;border:1px solid #e8d5f5">
                    ...
                    <div id="corrida-detalle-${w.replace(/,/g,"_").substring(0,20)}" style="display:none;margin-top:10px;border-top:1px solid #e8d5f5;padding-top:10px">
                      ${u.items.map(E=>{const P=(E.variantes||{}).talla||E.talla||"?",L=s.find(z=>z.variante_id===E.variante_id&&(r?z.sucursal_id===r:!0)),$=L?L.cantidad:999;return`
                          <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"
                  ...
                  </div>`}else{const b=u.items[0],h=b.variantes||{};h.productos;const v=h.talla||b.talla||"",w=s.find(x=>x.variante_id===b.variante_id&&(r?x.sucursal_id===r:!0)),E=w?w.cantidad:null;return`
