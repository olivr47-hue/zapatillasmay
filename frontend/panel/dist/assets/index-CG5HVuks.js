  ...
  `,await Ze(),Y=setInterval(async()=>{document.getElementById("ga-activos")?await Pe():(clearInterval(Y),Y=null)},3e4)}async function Ze(){await Promise.all([Pe(),Ye(),Qe()]),document.getElementById("ga-ultima-act").textContent="Actualizado: "+new Date().toLocaleTimeString("es-MX",{hour:"2-digit",minute:"2-digit",second:"2-digit"})}async function Pe(){var e;try{const o=await(await fetch(`${f}/analytics/tiempo-real`)).json();if(!o.configurado){et(o);return}const a=document.getElementById("ga-activos");if(!a)return;a.textContent=o.activos_ahora??0;const n=document.getElementById("ga-dispositivos");n&&o.por_dispositivo&&(n.textContent=Object.entries(o.por_dispositivo).map(([i,s])=>`${i}: ${s}`).join(" · "));const r=document.getElementById("ga-paginas-rt");r&&((e=o.por_pagina)!=null&&e.length)?r.innerHTML='<div style="font-size:0.65rem;color:#aaa;margin-bottom:4px;text-transform:uppercase;letter-spacing:.05em">Viendo ahora</div>'+o.por_pagina.map(i=>`
          <div style="display:flex;justify-content:space-between;align-items:center;padding:2px 0;border-bottom:1px solid #f5f5f5;gap:4px">
            <span style="font-size:0.68rem;color:#555;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1"
                  title="${i.pagina}">${i.pagina==="(not set)"?"inicio":i.pagina}</span>
