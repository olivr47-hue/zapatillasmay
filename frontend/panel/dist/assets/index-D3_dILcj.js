...
<div id="variantes-container">
  ...
  `,fetch(b+"/sucursales/").then(o=>o.json()).then(o=>{const n=document.getElementById("f-sucursal-stock");n&&(n.innerHTML=o.map(i=>`<option value="${i.id}">${i.nombre}</option>`).join("")),setTimeout(()=>actualizarTablaStock(),100)})};window.seleccionarColorPortada=e=>{if(!window._coloresExistentes)return;const t=window._coloresExistentes.splice(e,1)[0];window._coloresExistentes.unshift(t),document.querySelectorAll('[id^="portada-color-"]').forEach((o,n)=>{const i=n===0;o.style.borderColor=i?"#2e7d32":"#ddd",o.style.background=i?"#e8f5e9":"white",o.innerHTML=`
...
`).replace(/"/g,"&quot;")}" style="padding:4px 12px;border-radius:20px;border:1.5px solid #1d4ed8;background:none;color:#1d4ed8;font-size:0.75rem;font-weight:600;cursor:pointer">📋 Copiar</button>
  ...
  <button class="btn btn-secondary" onclick="imprimirTicketPOS('${n.id}',${n.total},${n.pedido_items?n.pedido_items.reduce((p,m)=>p+m.cantidad,0):0},'${n.forma_pago||"efectivo"}')">Reimprimir ticket</button>
    ...
    `};r(),window._renderEdicion=r};window.buscarProductoEdicion=e=>{const t=window._editVariantes||[],a=window._editProductos||[],o=document.getElementById("edit-prod-resultados");if(!o)return;if(window.posicionarDropdownCarrito("edit-prod-resultados","edit-buscar-prod"),!e||e.length<2){o.style.display="none";return}const n={};t.forEach(r=>{const s=a.find(c=>c.id===r.producto_id);if(!s)return;const d=((s.nombre||"")+" "+(r.color||"")+" "+(s.sku_interno||"")).toLowerCase();if(!e.toLowerCase().split(" ").every(c=>d.includes(c)))return;const l=s.id+"|"+(r.color||"");n[l]||(n[l]={prod:s,color:r.color||"",variantes:[]}),n[l].variantes.push(r)});const i=Object.values(n).slice(0,6);if(!i.length){o.innerHTML='<div style="padding:10px;font-size:0.85rem;color:#888">Sin resultados</div>',o.style.display="block";return}o.style.display="block",o.innerHTML=i.map(r=>`
  ...
  `).join("")};window.seleccionarProductoEdicion=(e,t)=>{var g;const a=window._editVariantes||[],o=window._editProductos||[],n=window._editInventario||[],i=((g=window._currentPedido)==null?void 0:g.sucursal_id)||"",r=o.find(u=>u.id===e);document.getElementById("edit-buscar-prod").value=`${r?r.nombre:""} · ${t}`,document.getElementById("edit-prod-resultados").style.display="none";const s=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],d=a.filter(u=>u.producto_id===e&&u.color===t).sort((u,y)=>s.indexOf(u.talla)-s.indexOf(y.talla));window._editSeleccionado={productoId:e,color:t,variantes:d,prod:r};const l=parseFloat(r?r.precio_menudeo:0)||0,c=parseFloat(r?r.precio_corrida:0)||(l>0?Math.round(l-100):0);document.getElementById("edit-nuevo-precio").value=l||"";const p=document.getElementById("edit-precios-info");p&&r&&(p.innerHTML=`
...
`).join("")}
    `;const d=document.getElementById("pos-total"),l=document.getElementById("pos-total-pares"),c=document.getElementById("pos-tipo-precio");d&&(d.textContent="$"+o.toFixed(2)),l&&(l.textContent=a),c&&(c.textContent=n);const p=document.getElementById("pos-flotante-pares"),m=document.getElementById("pos-flotante-total");p&&(p.textContent=a+" pares"),m&&(m.textContent="$"+o.toFixed(2)),(g=document.getElementById("pos-drawer"))!=null&&g.classList.contains("open")&&renderDrawerPOS(),ge()};window.editarPrecioPOS=(e,t)=>{if(!window._posCarrito[e])return;const a=parseFloat(t);if(!a||a<=0){alert("El precio debe ser mayor a $0");return}window._posCarrito[e].precio_unitario=a,window._posCarrito[e].precio_manual=!0;const o=window._posCarrito[e].cantidad,n=document.getElementById("subtotal-item-"+e);n&&(n.textContent="$"+(o*a).toFixed(2));const i=window._posCarrito.reduce((s,d)=>s+d.cantidad*d.precio_unitario,0),r=document.getElementById("pos-total");r&&(r.textContent="$"+i.toFixed(2)),ge()};window.editarPrecioCorridaPOS=(e,t)=>{const[a,o]=e.split("|"),n=parseFloat(t);if(!n||n<=0){alert("El precio debe ser mayor a $0");return}window._posCarrito.forEach(c=>{c.producto_id===a&&c.color===o&&c.es_corrida&&(c.precio_unitario=n,c.precio_manual=!0)});const r=window._posCarrito.filter(c=>c.producto_id===a&&c.color===o&&c.es_corrida).reduce((c,p)=>c+p.cantidad*p.precio_unitario,0),s=document.getElementById("subtotal-corrida-"+e.replace("|","-"));s&&(s.textContent="$"+r.toFixed(2));const d=window._posCarrito.reduce((c,p)=>c+p.cantidad*p.precio_unitario,0),l=document.getElementById("pos-total");l&&(l.textContent="$"+d.toFixed(2)),ge()};window.editarCorridaEnCarrito=e=>{const[t,a]=e.split("|"),{inventario:o,variantes:n}=window._posData,i=document.getElementById("pos-sucursal")?document.getElementById("pos-sucursal").value:"",r=o.filter(c=>c.sucursal_id===i),s=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],d=n.filter(c=>c.producto_id===t&&c.color===a).sort((c,p)=>s.indexOf(c.talla)-s.indexOf(p.talla)),l=document.createElement("div");l.id="pos-modal-editar-corrida",l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:1000;display:flex;align-items:center;justify-content:center;padding:1rem",l.innerHTML=`
