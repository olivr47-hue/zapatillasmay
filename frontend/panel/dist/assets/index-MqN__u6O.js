...
¿Quieres reactivar todos?`)&&(localStorage.removeItem("ordenes_pospuestos"),J())};window.guardarOrdenCompra2=async()=>{var i,r,s,d;const{seleccionados:e,sucursalId:t}=window._ordenModal||{};if(!e)return;const a=(i=document.getElementById("orden-fecha"))==null?void 0:i.value,o=((r=document.getElementById("orden-notas"))==null?void 0:r.value)||"",n={};e.forEach(l=>{var g,u;const c=l.proveedor_id||"sin-proveedor",p=((g=l.proveedor)==null?void 0:g.nombre)||"Sin proveedor";n[c]||(n[c]={nombre:p,items:[]});const m=l.variantes||[];if(m.length===0){const b=parseInt(((u=document.getElementById("qty-orden-"+l.producto_id))==null?void 0:u.value)||l.cantidad_sugerida);n[c].items.push({variante_id:null,cantidad:b,costo_unitario:l.costo_unitario})}else m.forEach(b=>{var h;const v=document.getElementById("chk-var-"+b.id);if(!(v!=null&&v.checked))return;const y=parseInt(((h=document.getElementById("qty-var-"+b.id))==null?void 0:h.value)||0);y<=0||n[c].items.push({variante_id:b.id,cantidad:y,costo_unitario:l.costo_unitario})})});try{for(const[l,c]of Object.entries(n)){if(c.items.length===0)continue;const p=c.items.reduce((b,v)=>b+v.cantidad*v.costo_unitario,0),u=(s=(await(await fetch(f+"/finanzas/ordenes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({proveedor_id:l==="sin-proveedor"?null:l,sucursal_id:t,status:"borrador",total:p,notas:o,fecha_entrega_estimada:a||null})})).json())[0])==null?void 0:s.id;if(u)for(const b of c.items)await fetch(f+"/finanzas/ordenes/"+u+"/items",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...b,subtotal:b.cantidad*b.costo_unitario})})}(d=document.getElementById("modal-orden"))==null||d.remove(),alert("Orden de compra guardada"),J()}catch(l){alert("Error guardando orden: "+l.message)}};function Ce(){return`
  ...
  `}async function W(){var t,a,o,n,i,r;const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:#888">Cargando finanzas...</p>';try{const d=await(await fetch(f+"/sucursales/")).json(),l=(t=d[0])==null?void 0:t.id,[c,p,m,g,u,b,v]=await Promise.all([fetch(f+"/finanzas/caja/hoy/"+l),fetch(f+"/finanzas/reporte/"+l),fetch(f+"/finanzas/gastos/"+l),fetch(f+"/finanzas/estado-resultados/"+l),fetch(f+"/finanzas/flujo/"+l),fetch(f+"/finanzas/cuentas-por-cobrar"),fetch(f+"/finanzas/gastos-categorias/"+l)]),y=await c.json(),h=await p.json(),w=await m.json(),C=await g.json(),_=await u.json(),k=await b.json(),$=await v.json(),E=y.find(S=>S.status==="abierta"),T=new Date().toISOString().split("T")[0],O=w.filter(S=>{var P;return(P=S.created_at)==null?void 0:P.startsWith(T)}),z=O.reduce((S,P)=>S+parseFloat(P.monto||0),0),I=k.reduce((S,P)=>S+parseFloat(P.total||0),0);e.innerHTML=`
      <div style="margin-bottom:1.5rem;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
        <div>
      ...
      <div style="background:${E?"#e8f5e9":"#ffebee"};border-radius:12px;padding:1.25rem;border:1px solid ${E?"#a5d6a7":"#ffcdd2"};margin-bottom:1.5rem;display:flex;align-items:center;gap:16px;flex-wrap:wrap">
        ...
        ${E?`
          <div style="display:flex;gap:16px;flex-wrap:wrap">
            <div style="text-align:center">
              <p style="font-size:1.2rem;font-weight:700;color:#2e7d32">$${(((a=_.hoy)==null?void 0:a.efectivo)||0).toFixed(0)}</p>
            ...
            <div style="text-align:center">
              <p style="font-size:1.2rem;font-weight:700;color:#6a1b9a">$${(((n=_.hoy)==null?void 0:n.spei)||0).toFixed(0)}</p>
              <p style="font-size:0.65rem;color:#888">SPEI</p>
            </div>
            <div style="text-align:center">
              <p style="font-size:1.2rem;font-weight:700;color:#E91E8C">$${(((i=_.hoy)==null?void 0:i.total)||0).toFixed(0)}</p>
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px;margin-bottom:1.5rem">
        ...
        <div style="background:white;border-radius:12px;padding:1.25rem;border:1px solid #eee;text-align:center">
          <p style="font-size:1.5rem;font-weight:700;color:#333">$${(((r=_.semana)==null?void 0:r.ingresos)||0).toFixed(0)}</p>
    ...
    `,window._finanzasData={sucursalId:l,gastosHoy:O,totalGastosHoy:z,estadoResultados:C,flujo:_,cxc:k,categorias:$,historial:[]},window._finanzasSucursalId=l;const x=await(await fetch(f+"/finanzas/caja/historial/"+l)).json();window._finanzasData.historial=x,mostrarTabFinanzas("gastos")}catch(s){e.innerHTML='<p style="padding:2rem;color:red">Error cargando finanzas: '+s.message+"</p>"}}window.mostrarTabFinanzas=e=>{var c,p,m,g,u,b,v,y,h,w,C,_,k;const{gastosHoy:t,totalGastosHoy:a,estadoResultados:o,flujo:n,cxc:i,categorias:r,historial:s,sucursalId:d}=window._finanzasData,l=document.getElementById("fin-tab-contenido");if(l)if(e==="gastos"){const $=r.reduce((E,T)=>E+T.total,0);l.innerHTML=`
    ...
    `:e==="flujo"?l.innerHTML=`
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:1rem;margin-bottom:1rem">
        ...
        <div style="background:white;border-radius:12px;border:1px solid #eee;padding:1.5rem">
          ...
          <div style="display:flex;justify-content:space-between;padding:8px 0;margin-top:4px">
            <span style="font-size:0.85rem;font-weight:700">Neto</span>
            <span style="font-weight:700;color:${(((_=n.mes)==null?void 0:_.neto)||0)>=0?"#2e7d32":"#c62828"};font-size:1.1rem">$${(((k=n.mes)==null?void 0:k.neto)||0).toFixed(0)}</span>
          </div>
        </div>
      </div>
    `:e==="caja"?l.innerHTML=`
...
⚠️ Requiere que los productos estén en el catálogo de Meta Commerce.`;else if(d.style.background="#e8f5e9",d.style.borderColor="#a5d6a7",d.style.color="#333",a){let b;t==="personalizado"?b=(((u=document.getElementById("texto-personalizado"))==null?void 0:u.value)||"").replace("{nombre}","María"):b=a.mensaje("María"),d.textContent=b}}const l=document.getElementById("campana-count");l&&(l.textContent=i.length+" clientes");const c=document.getElementById("campana-lista");if(c){if(!i.length){c.innerHTML='<div style="padding:2rem;text-align:center;color:#888;font-size:0.85rem">No hay clientes en este segmento con teléfono registrado</div>';return}window._campanaFiltrados=i,window._campanaPlantillaId=t,c.innerHTML=i.map((b,v)=>{var C;let y;t==="personalizado"?y=(((C=document.getElementById("texto-personalizado"))==null?void 0:C.value)||"").replace("{nombre}",b.nombre.split(" ")[0]):y=a.mensaje(b.nombre.split(" ")[0]);const h=encodeURIComponent(y),w=(b.lada||"52")+b.telefono.replace(/\D/g,"");return`
    ...
    <div style="background:white;border-radius:16px;padding:2rem;max-width:440px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.3);text-align:center">
      ...
      <p style="font-size:0.82rem;color:#888">Conectando con WhatsApp Business</p>
        ...
        </div>`};window._campanaJobId=u;const y=setInterval(async()=>{try{const w=await(await fetch(f+"/campanas/estado/"+u)).json();v(w),w.terminado&&(clearInterval(y),window._campanaUltimosResultados=w.resultados||[],setTimeout(()=>{const C=w.enviados||0,_=w.fallidos||0;l.innerHTML=`
...
`,renderInventario()}catch{e.innerHTML='<p style="padding:2rem;color:red">Error conectando con el servidor</p>'}}window.renderInventario=()=>{const{sucursales:e,productos:t,variantes:a,inventario:o}=window._invData,n=(document.getElementById("inv-buscar")?document.getElementById("inv-buscar").value:"").toLowerCase(),i=document.getElementById("inv-categoria")?document.getElementById("inv-categoria").value:"",r=document.getElementById("inv-talla")?document.getElementById("inv-talla").value:"",s=document.getElementById("inv-estado")?document.getElementById("inv-estado").value:"",d=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],l=t.filter(m=>!(!m.activo||n&&!m.nombre.toLowerCase().includes(n)&&!(m.sku_interno||"").toLowerCase().includes(n)||i&&m.categoria!==i)),c=e.map(m=>{const g=o.filter(b=>b.sucursal_id===m.id),u=l.map(b=>{const v=a.filter(C=>C.producto_id===b.id);if(v.length===0)return"";const h=[...new Set(v.map(C=>C.color).filter(Boolean))].map(C=>{const _=v.filter(T=>T.color===C).sort((T,O)=>d.indexOf(T.talla)-d.indexOf(O.talla));if(r&&!_.find(T=>T.talla===r))return"";const k=_[0]?_[0].color_hex:"#888",$=_[0]?_[0].foto_url:null,E=_.map(T=>{const O=g.find(S=>S.variante_id===T.id),z=O?O.cantidad:null,I=O?O.stock_minimo:3;if(r&&T.talla!==r||s&&(s==="agotado"&&z!==0||s==="bajo"&&(z===null||z===0||z>I)||s==="disponible"&&(z===null||z===0||z<=I)))return"";let A,x;return z===null?(A="#f0f0f0",x="#aaa"):z===0?(A="#ffebee",x="#c62828"):z<=I?(A="#fff8e1",x="#f57f17"):(A="#e8f5e9",x="#2e7d32"),`
            <div style="display:flex;align-items:center;justify-content:space-between;background:${A};border-radius:10px;padding:8px 12px;border:1px solid ${x}30">
  ...
  `};window.guardarSalida=async()=>{const e=document.getElementById("sal").value,t=document.getElementById("sal-sucursal").value,a=document.getElementById("sal-cantidad").value,o=document.getElementById("sal-motivo").value;if(!e||!t||!a){alert("Por favor completa todos los campos");return}try{const i=await(await fetch(f+"/movimientos/entrada",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({variante_id:e,sucursal_id:t,cantidad:-parseInt(a),motivo:o})})).json();i.ok?(alert("Salida registrada. Anterior: "+i.cantidad_anterior+" pares ÔåÆ Nuevo: "+i.cantidad_nueva+" pares"),navegarA("inventario")):alert("Error: "+JSON.stringify(i))}catch{alert("Error conectando con el servidor")}};window.mostrarInventarioMasivo=async()=>{const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:#888">Cargando...</p>';try{const a=await(await fetch(f+"/sucursales/")).json(),n=await(await fetch(f+"/productos/")).json(),r=await(await fetch(f+"/variantes/")).json(),d=await(await fetch(f+"/inventario/")).json(),l=[...new Set(n.map(p=>p.categoria).filter(Boolean))],c=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"];window._invMasivo={sucursales:a,productos:n,variantes:r,inventario:d},e.innerHTML=`
    ...
    `,renderTablasMasivo()}catch{e.innerHTML='<p style="padding:2rem;color:red">Error cargando inventario</p>'}};window.renderTablasMasivo=()=>{const{productos:e,variantes:t,inventario:a}=window._invMasivo,o=document.getElementById("im-sucursal").value,n=document.getElementById("im-categoria").value,i=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],r=e.filter(c=>!(n&&c.categoria!==n)),s=a.filter(c=>c.sucursal_id===o),d=r.map(c=>{const p=t.filter(u=>u.producto_id===c.id);if(p.length===0)return"";const g=[...new Set(p.map(u=>u.color).filter(Boolean))].map(u=>{const b=p.filter(y=>y.color===u).sort((y,h)=>i.indexOf(y.talla)-i.indexOf(h.talla));return`
        <div style="margin-bottom:1rem">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            ${b.map(y=>{const h=s.find(k=>k.variante_id===y.id),w=h?h.cantidad:0,C=h?h.stock_minimo:3;let _="#ddd";return w===0?_="#ffcdd2":w<=C?_="#ffe082":_="#a5d6a7",`
                <div style="text-align:center">
...
<td>
  ...
  <button class="btn btn-secondary" style="padding:4px 8px;font-size:0.72rem;color:#c62828;border-color:#c62828" 
    ...
    Cancelar
          ...
          </div>`).join(""));const A=document.getElementById("dash-ultimos-pedidos");A&&(A.innerHTML=t.slice(0,5).map(x=>`
