...
`);await fetch(f+"/chatbot/chats/"+e+"/imagen",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({imagen_url:t,caption:d,agente:n})})}else await fetch(f+"/chatbot/chats/"+e+"/mensaje",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({mensaje:o,agente:n})});await new Promise(d=>setTimeout(d,1500));const s=await(await fetch(f+"/chatbot/chats")).json();window._chatsData={},s.forEach(d=>window._chatsData[d.telefono]=d),abrirChat(e)}catch{alert("Error enviando producto")}};window.cargarEnviosMasivos=async function(){const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:#888">Cargando...</p>';try{const[t,o,a]=await Promise.all([fetch(f+"/chatbot/plantillas").then(i=>i.json()),fetch(f+"/clientes/").then(i=>i.json()),fetch(f+"/chatbot/chats").then(i=>i.json())]),n={};o.forEach(i=>{i.telefono&&(n[i.telefono]={telefono:i.telefono,nombre:i.nombre,fuente:"cliente"})}),a.forEach(i=>{n[i.telefono]||(n[i.telefono]={telefono:i.telefono,nombre:i.nombre||i.telefono,fuente:"whatsapp",etiqueta:i.etiqueta})});const r=Object.values(n);window._envioContactos=r,window._envioChats=a,window._envioClientes=o,e.innerHTML=`
  ...
  `).join("")};window.iniciarEnvioMasivo=async()=>{var n;const e=window._envioContactos||[];if(e.length===0){alert("No hay contactos seleccionados");return}const t=document.getElementById("envio-plantilla").value,o=document.getElementById("envio-imagen").value;if(!confirm(`¿Enviar "${t}" a ${e.length} contactos?`))return;const a=document.getElementById("btn-enviar-masivo");a.textContent="Enviando...",a.disabled=!0;try{const i=await(await fetch(f+"/chatbot/envio-masivo",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({plantilla:t,imagen_url:o,contactos:e})})).json(),s=document.getElementById("envio-resultado");s.style.display="block",s.style.background=i.fallidos>0?"#fff8e1":"#e8f5e9",s.innerHTML=`
    ...
    `,document.body.appendChild(s),s.addEventListener("click",d=>{d.target===s&&s.remove()})}catch(a){console.error("Error verOportunidad",a)}};window.actualizarEtapaOportunidad=async(e,t)=>{var o;try{await fetch(f+"/crm/oportunidades/"+e,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({etapa:t})}),(o=document.querySelector('div[style*="position:fixed"][style*="z-index:1000"]'))==null||o.remove(),mostrarPipeline()}catch{alert("Error actualizando etapa")}};window.completarTareaDashboard=async(e,t)=>{try{await fetch(f+"/chatbot/tareas/"+e,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({completada:t})})}catch(o){console.error(o)}};async function Ue(){const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:var(--text-muted)">Cargando...</p>';try{const o=await(await fetch(f+"/config/envio")).json();e.innerHTML=`
...
`})})),n.textContent=b}}catch(m){n.style.display="block",n.textContent="Error inesperado: "+m.message}finally{t.innerHTML=r,t.disabled=!1}}window.mlPublicarPreview=function(e){Pe(!0,e)};window.mlPublicarReal=function(e){confirm("¿Publicar TODAS las variantes activas de este producto en MercadoLibre? Esto creará nuevos items en tu cuenta.")&&Pe(!1,e)};window.descargarExcelTikTok=async function(e,t,o){const a=e.innerHTML;try{e.innerHTML="⏳ Generando...",e.disabled=!0;const n=await fetch(`${f}/tiktok/${t}`);if(!n.ok){const d=await n.text();throw new Error(`Error ${n.status}: ${d}`)}const r=await n.blob(),i=URL.createObjectURL(r),s=document.createElement("a");s.href=i,s.download=o,document.body.appendChild(s),s.click(),document.body.removeChild(s),URL.revokeObjectURL(i)}catch(n){alert("Error al descargar el archivo: "+n.message)}finally{e.innerHTML=a,e.disabled=!1}};async function Ke(){const e=document.getElementById("content");e.innerHTML=`
  ...
  `,document.getElementById("ml-titulo").addEventListener("input",function(){document.getElementById("ml-titulo-count").textContent=this.value.length}),window.mlGenerarPreview=async()=>{var l,c,p,m;const o=document.getElementById("ml-sku").value.trim().toUpperCase(),a=document.getElementById("ml-listing").value,n=document.getElementById("ml-titulo").value.trim(),r=document.getElementById("ml-precio").value.trim(),i=r?parseFloat(r):null;if(!o){alert("Ingresa el SKU del producto");return}if(!n){alert("Ingresa el título para ML");return}if(n.length>60){alert("El título no puede superar 60 caracteres");return}if(i!==null&&(isNaN(i)||i<=0)){alert("El precio debe ser mayor a 0");return}const s=document.getElementById("ml-btn-preview");s.textContent="⏳ Generando...",s.disabled=!0;let d=0;try{const u=await fetch(`${f}/ml/publicar`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sku_interno:o,listing_type:a,solo_preview:!0})}),g=await u.json();if(!u.ok){alert("Error: "+(g.detail||JSON.stringify(g)));return}const y=g.resultados||[];if(!y.length){alert("No se encontraron variantes activas");return}const b=document.getElementById("ml-descripcion"),h=((p=(c=(l=y[0])==null?void 0:l.preview)==null?void 0:c.description)==null?void 0:p.plain_text)||"";b.value.trim()||(b.value=h);const v=new Set,w=[];for(const x of y)for(const z of((m=x.preview)==null?void 0:m.pictures)||[])z.source&&!v.has(z.source)&&(v.add(z.source),w.push(z));if(w.length>=1){const x=document.getElementById("ml-fotos-wrap"),z=document.getElementById("ml-fotos-grid");z.innerHTML=w.map((_,E)=>`
      ...
      `).join("")||'<p style="color:#888">Sin detalles</p>'}catch(l){s.textContent="❌ Error de conexión",d.innerHTML=`<p style="color:red">${l.message}</p>`}finally{r.textContent="🚀 Publicar todas en ML",r.disabled=!1}}}let X=[],Q="";async function Ze(){const e=document.getElementById("content");e.innerHTML=`
    <style>
