...
¿Quieres reactivar todos?`)&&(localStorage.removeItem("ordenes_pospuestos"),re())};window.guardarOrdenCompra2=async()=>{var i,r,s,d;const{seleccionados:e,sucursalId:t}=window._ordenModal||{};if(!e)return;const o=(i=document.getElementById("orden-fecha"))==null?void 0:i.value,a=((r=document.getElementById("orden-notas"))==null?void 0:r.value)||"",n={};e.forEach(l=>{var g,u;const c=l.proveedor_id||"sin-proveedor",p=((g=l.proveedor)==null?void 0:g.nombre)||"Sin proveedor";n[c]||(n[c]={nombre:p,items:[]});const m=l.variantes||[];if(m.length===0){const b=parseInt(((u=document.getElementById("qty-orden-"+l.producto_id))==null?void 0:u.value)||l.cantidad_sugerida);n[c].items.push({variante_id:null,cantidad:b,costo_unitario:l.costo_unitario})}else m.forEach(b=>{var h;const y=document.getElementById("chk-var-"+b.id);if(!(y!=null&&y.checked))return;const v=parseInt(((h=document.getElementById("qty-var-"+b.id))==null?void 0:h.value)||0);v<=0||n[c].items.push({variante_id:b.id,cantidad:v,costo_unitario:l.costo_unitario})})});try{for(const[l,c]of Object.entries(n)){if(c.items.length===0)continue;const p=c.items.reduce((b,y)=>b+y.cantidad*y.costo_unitario,0),u=(s=(await(await fetch(f+"/finanzas/ordenes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({proveedor_id:l==="sin-proveedor"?null:l,sucursal_id:t,status:"borrador",total:p,notas:a,fecha_entrega_estimada:o||null})})).json())[0])==null?void 0:s.id;if(u)for(const b of c.items)await fetch(f+"/finanzas/ordenes/"+u+"/items",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...b,subtotal:b.cantidad*b.costo_unitario})})}(d=document.getElementById("modal-orden"))==null||d.remove(),alert("Orden de compra guardada"),re()}catch(l){alert("Error guardando orden: "+l.message)}};function et(){var o;const e=new Date().toLocaleDateString("es-MX",{weekday:"long",day:"numeric",month:"long"});return`
  ...
  `}async function se(){var t,o,a,n,i,r;const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:#888">Cargando finanzas...</p>';try{const d=await(await fetch(f+"/sucursales/")).json(),l=(t=d[0])==null?void 0:t.id,[c,p,m,g,u,b,y]=await Promise.all([fetch(f+"/finanzas/caja/hoy/"+l),fetch(f+"/finanzas/reporte/"+l),fetch(f+"/finanzas/gastos/"+l),fetch(f+"/finanzas/estado-resultados/"+l),fetch(f+"/finanzas/flujo/"+l),fetch(f+"/finanzas/cuentas-por-cobrar"),fetch(f+"/finanzas/gastos-categorias/"+l)]),v=await c.json(),h=await p.json(),$=await m.json(),E=await g.json(),x=await u.json(),j=await b.json(),L=await y.json(),w=v.find(B=>B.status==="abierta"),C=new Date().toISOString().split("T")[0],q=$.filter(B=>{var _;return(_=B.created_at)==null?void 0:_.startsWith(C)}),z=q.reduce((B,_)=>B+parseFloat(_.monto||0),0),T=j.reduce((B,_)=>B+parseFloat(_.total||0),0);e.innerHTML=`
    ...
    `}else if(e==="gastos"){const w=r.reduce((C,q)=>C+q.total,0);l.innerHTML=`
...
⚠️ Requiere que los productos estén en el catálogo de Meta Commerce.`;else if(d.style.background="#e8f5e9",d.style.borderColor="#a5d6a7",d.style.color="#333",o){let b;t==="personalizado"?b=(((u=document.getElementById("texto-personalizado"))==null?void 0:u.value)||"").replace("{nombre}","María"):b=o.mensaje("María"),d.textContent=b}}const l=document.getElementById("campana-count");l&&(l.textContent=i.length+" clientes");const c=document.getElementById("campana-lista");if(c){if(!i.length){c.innerHTML='<div style="padding:2rem;text-align:center;color:#888;font-size:0.85rem">No hay clientes en este segmento con teléfono registrado</div>';return}window._campanaFiltrados=i,window._campanaPlantillaId=t,c.innerHTML=i.map((b,y)=>{var E;let v;t==="personalizado"?v=(((E=document.getElementById("texto-personalizado"))==null?void 0:E.value)||"").replace("{nombre}",b.nombre.split(" ")[0]):v=o.mensaje(b.nombre.split(" ")[0]);const h=encodeURIComponent(v),$=(b.lada||"52")+b.telefono.replace(/\D/g,"");return`
    ...
    <div style="background:white;border-radius:16px;padding:2rem;max-width:440px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.3);text-align:center">
      ...
      <p style="font-size:0.82rem;color:#888">Conectando con WhatsApp Business</p>
        ...
        </div>`};window._campanaJobId=u;const v=setInterval(async()=>{try{const $=await(await fetch(f+"/campanas/estado/"+u)).json();y($),$.terminado&&(clearInterval(v),window._campanaUltimosResultados=$.resultados||[],setTimeout(()=>{const E=$.enviados||0,x=$.fallidos||0;l.innerHTML=`
...
<td>
  ...
  <button class="btn btn-secondary" style="padding:4px 8px;font-size:0.72rem;color:#c62828;border-color:#c62828" 
    ...
    Cancelar
        ...
        </tr>`}).join(""))};async function rt(){try{const t=await(await fetch(f+"/pedidos/")).json(),a=await(await fetch(f+"/clientes/")).json(),i=await(await fetch(f+"/inventario/alertas")).json(),r=new Date;r.setHours(0,0,0,0);const s=new Date(r);s.setDate(s.getDate()-7);const d=new Date(r);d.setDate(d.getDate()-30);const l=t.filter(_=>["confirmado","pagado","enviado"].includes(_.status)),c=l.filter(_=>new Date(_.created_at)>=r),p=l.filter(_=>new Date(_.created_at)>=s),m=c.reduce((_,M)=>_+parseFloat(M.total||0),0),g=p.reduce((_,M)=>_+parseFloat(M.total||0),0),u=a.filter(_=>_.created_at&&new Date(_.created_at)>=d).length,b=["Dom","Lun","Mar","Mie","Jue","Vie","Sab"],y={};b.forEach(_=>y[_]=0),l.filter(_=>new Date(_.created_at)>=d).forEach(_=>{const M=b[new Date(_.created_at).getDay()];y[M]+=parseFloat(_.total||0)});const v={};l.forEach(_=>{v[_.canal||"sucursal"]=(v[_.canal||"sucursal"]||0)+parseFloat(_.total||0)});const h={};l.forEach(_=>{h[_.forma_pago||"efectivo"]=(h[_.forma_pago||"efectivo"]||0)+1});const $={};l.forEach(_=>{$[_.empleado||"Admin"]=($[_.empleado||"Admin"]||0)+parseFloat(_.total||0)});const E={};l.forEach(_=>{const M=new Date(_.created_at).toLocaleDateString("es-MX",{month:"short",year:"numeric"});E[M]=(E[M]||0)+parseFloat(_.total||0)});const x={};l.forEach(_=>{_.clientes&&(x[_.clientes.nombre]=(x[_.clientes.nombre]||0)+parseFloat(_.total||0))});const j=Object.entries(x).sort((_,M)=>M[1]-_[1]).slice(0,5),L=Object.entries(y).sort((_,M)=>M[1]-_[1])[0],w=Object.entries($).sort((_,M)=>M[1]-_[1])[0];if(!document.getElementById("dashboard-contenido"))return;const q=l.filter(_=>new Date(_.created_at)>=d),z=q.reduce((_,M)=>_+parseFloat(M.total||0),0),T=[];for(let _=6;_>=0;_--){const M=new Date(r);M.setDate(M.getDate()-_);const D=M.toISOString().split("T")[0],k=M.toLocaleDateString("es-MX",{weekday:"short",day:"numeric"}),O=l.filter(F=>{var P;return(P=F.created_at)==null?void 0:P.startsWith(D)}).reduce((F,P)=>F+parseFloat(P.total||0),0);T.push({label:k,total:O,ds:D})}const I=(_,M,D,k)=>{const O=document.getElementById(_),F=document.getElementById(_+"-sub");O&&(O.textContent=M,k&&(O.style.color=k)),F&&D&&(F.textContent=D)};I("kpi-ventas-hoy","$"+m.toLocaleString("es-MX",{maximumFractionDigits:0}),c.length+" pedidos","#C8967A"),I("kpi-pedidos-hoy",c.length,"confirmados"),I("kpi-ventas-7d","$"+g.toLocaleString("es-MX",{maximumFractionDigits:0}),p.length+" pedidos"),I("kpi-ventas-30d","$"+z.toLocaleString("es-MX",{maximumFractionDigits:0}),q.length+" pedidos"),I("kpi-clientes-nuevos",u,"últimos 30 días"),I("kpi-stock-bajo",i.length,i.length>0?"⚠ reabastecer":"✓ ok",i.length>0?"#f59e0b":"#10b981"),I("kpi-mejor-dia",L?L[0]:"—",L?"$"+L[1].toLocaleString("es-MX",{maximumFractionDigits:0}):""),I("kpi-total-clientes",a.length,"registrados");const S=document.getElementById("dash-top-clientes");S&&(S.innerHTML=j.length===0?'<p style="color:var(--text-muted);font-size:0.85rem">Sin datos aun</p>':j.map(([_,M],D)=>`
...
`)[0].trim();c=v.match(/^https?:\/\/.+\.(jpg|jpeg|png|webp|gif)/i)?`<img src="${v}" style="max-width:200px;border-radius:8px;display:block;cursor:pointer" onclick="window.open('${v}')">`:`<p>${l}</p>`}else if(n.tipo==="documento_saliente"){const v=l.replace("[Documento] ","").split(" "),h=v[0]||"documento";c=`<a href="${v[1]||""}" target="_blank" class="wa-doc-link">
        ...
        ${h}</a>`}else if(n.tipo==="video_saliente")c=`<video src="${l.replace("[Video] ","")}" controls style="max-width:220px;border-radius:8px;display:block"></video>`;else if(n.tipo==="botones_saliente"){const v=l.replace("[Botones] ","").split(" -> "),h=v[0]||l,$=(v[1]||"").split(" | ");c=`<p style="margin:0 0 8px">${h}</p>
        <div style="display:flex;flex-direction:column;gap:4px">${$.map(E=>`<span style="border:1px solid rgba(8,145,178,0.3);border-radius:6px;padding:4px 10px;font-size:0.78rem;color:#0891b2;text-align:center">${E}</span>`).join("")}</div>`}else if(n.tipo==="lista_saliente"){const v=l.replace("[Lista] ","").split(" -> "),h=v[0]||l,$=(v[1]||"").split(", ").slice(0,5);c=`<p style="margin:0 0 6px">${h}</p>
...
`})})),n.textContent=y}}catch(m){n.style.display="block",n.textContent="Error inesperado: "+m.message}finally{t.innerHTML=i,t.disabled=!1}}window.mlPublicarPreview=function(e){Ge(!0,e)};window.mlPublicarReal=function(e){confirm("¿Publicar TODAS las variantes activas de este producto en MercadoLibre? Esto creará nuevos items en tu cuenta.")&&Ge(!1,e)};window.descargarExcelTikTok=async function(e,t,o){const a=e.innerHTML;try{e.innerHTML="⏳ Generando...",e.disabled=!0;const n=await fetch(`${f}/tiktok/${t}`);if(!n.ok){const d=await n.text();throw new Error(`Error ${n.status}: ${d}`)}const i=await n.blob(),r=URL.createObjectURL(i),s=document.createElement("a");s.href=r,s.download=o,document.body.appendChild(s),s.click(),document.body.removeChild(s),URL.revokeObjectURL(r)}catch(n){alert("Error al descargar el archivo: "+n.message)}finally{e.innerHTML=a,e.disabled=!1}};async function pt(){const e=document.getElementById("content");e.innerHTML=`
  ...
  `,document.getElementById("ml-titulo").addEventListener("input",function(){document.getElementById("ml-titulo-count").textContent=this.value.length}),window.mlGenerarPreview=async()=>{var d,l,c,p;const o=document.getElementById("ml-sku").value.trim().toUpperCase(),a=document.getElementById("ml-listing").value,n=document.getElementById("ml-titulo").value.trim(),i=document.getElementById("ml-precio").value.trim(),r=i?parseFloat(i):null;if(!o){alert("Ingresa el SKU del producto");return}if(!n){alert("Ingresa el título para ML");return}if(n.length>60){alert("El título no puede superar 60 caracteres");return}if(r!==null&&(isNaN(r)||r<=0)){alert("El precio debe ser mayor a 0");return}const s=document.getElementById("ml-btn-preview");s.textContent="⏳ Generando...",s.disabled=!0;try{const m=await fetch(`${f}/ml/publicar`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sku_interno:o,listing_type:a,solo_preview:!0})}),g=await m.json();if(!m.ok){alert("Error: "+(g.detail||JSON.stringify(g)));return}const u=g.resultados||[];if(!u.length){alert("No se encontraron variantes activas");return}const b=document.getElementById("ml-descripcion"),y=((c=(l=(d=u[0])==null?void 0:d.preview)==null?void 0:l.description)==null?void 0:c.plain_text)||"";b.value.trim()||(b.value=y);const v={},h=[],$=new Set;for(const C of u){const q=C.color||"Sin color";v[q]||(v[q]=[]);for(const z of((p=C.preview)==null?void 0:p.pictures)||[])z.source&&(v[q].find(T=>T.source===z.source)||v[q].push(z),$.has(z.source)||($.add(z.source),h.push(z)))}const E={},x=new Set;for(const C of Object.keys(v))E[C]=0,x.add(C);const j=Object.keys(v);if(j.length>0){const C=document.getElementById("ml-fotos-wrap"),q=document.getElementById("ml-fotos-grid"),z=()=>{T();const I=u.filter(S=>x.has(S.color||"Sin color"));t(I,n,r,E,b.value.trim(),v)},T=()=>{q.innerHTML=j.map(I=>{const S=v[I];if(!S.length)return"";const B=x.has(I),_=I.replace(/'/g,"\\'"),M=I.replace(/[^a-zA-Z0-9]/g,"_");return`
              ...
              </div>`}).join("")};T(),C.style.display="block",window.mlToggleColor=I=>{x.has(I)?x.delete(I):x.add(I),z()},window.mlSeleccionarPortadaColor=(I,S)=>{E[I]=S,z()}}const L=u.filter(C=>x.has(C.color||"Sin color"));t(L,n,r,E,b.value.trim(),v);const w=document.getElementById("ml-variantes-wrap");w.style.display="block",w.scrollIntoView({behavior:"smooth",block:"start"})}catch(m){alert("Error de conexión: "+m.message)}finally{s.textContent="🔍 Generar preview",s.disabled=!1}};function t(o,a,n,i,r,s){const d=document.getElementById("ml-variantes-titulo"),l=document.getElementById("ml-variantes-list");d.textContent=`Paso 2 — Revisa y edita (${o.length} variantes, título aplicado a todas)`,l.innerHTML=o.map((c,p)=>{const m={...c.preview};if(delete m.title,m.family_name=a,n!==null&&(m.price=n),r&&(m.description={plain_text:r}),s){const g=c.color||"Sin color",u=s[g]||[],b=i&&i[g]||0;let y=[...u];if(b>0&&b<y.length){const[v]=y.splice(b,1);y.unshift(v)}if(y.length<3&&s){for(const[v,h]of Object.entries(s))if(v!==g){for(const $ of h)if(y.find(E=>E.source===$.source)||y.push($),y.length>=12)break;if(y.length>=12)break}}m.pictures=y.slice(0,12)}return`
