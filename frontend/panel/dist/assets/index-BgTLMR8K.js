...
⚠️ Requiere que los productos estén en el catálogo de Meta Commerce.`;else if(d.style.background="#e8f5e9",d.style.borderColor="#a5d6a7",d.style.color="#333",o){let y;t==="personalizado"?y=(((g=document.getElementById("texto-personalizado"))==null?void 0:g.value)||"").replace("{nombre}","María"):y=o.mensaje("María"),d.textContent=y}}const l=document.getElementById("campana-count");l&&(l.textContent=i.length+" clientes");const c=document.getElementById("campana-lista");if(c){if(!i.length){c.innerHTML='<div style="padding:2rem;text-align:center;color:#888;font-size:0.85rem">No hay clientes en este segmento con teléfono registrado</div>';return}window._campanaFiltrados=i,window._campanaPlantillaId=t,c.innerHTML=i.map((y,b)=>{var I;let h;t==="personalizado"?h=(((I=document.getElementById("texto-personalizado"))==null?void 0:I.value)||"").replace("{nombre}",y.nombre.split(" ")[0]):h=o.mensaje(y.nombre.split(" ")[0]);const v=encodeURIComponent(h),w=(y.lada||"52")+y.telefono.replace(/\D/g,"");return`
  ...
  <button class="btn btn-secondary" onclick="imprimirTicketPOS('${n.id}',${n.total},${n.pedido_items?n.pedido_items.reduce((d,l)=>d+l.cantidad,0):0},'${n.forma_pago||"efectivo"}')">Reimprimir ticket</button>
    ...
    `}catch{t.innerHTML='<p style="padding:2rem;color:red">Error cargando pedido</p>'}};window.activarEdicionPedido=async e=>{var r;const t=document.getElementById("panel-edicion"),o=document.getElementById("items-lista");if(!t)return;const[a,n]=await Promise.all([fetch(f+"/variantes/").then(s=>s.json()),fetch(f+"/productos/").then(s=>s.json())]);window._editPedidoId=e,window._editItems=(((r=window._currentPedido)==null?void 0:r.pedido_items)||[]).map(s=>({...s})),window._editVariantes=a,window._editProductos=n,o.style.display="none",t.style.display="block";const i=()=>{const s=window._editItems;t.innerHTML=`
      <div style="background:#fff8e1;border:1px solid #ffe082;border-radius:8px;padding:1rem;margin-bottom:1rem">
        ...
        ${s.map((d,l)=>{const c=d.variantes||{};return`
            <div style="display:flex;align-items:center;gap:10px;padding:10px;background:white;border-radius:8px;margin-bottom:8px;border:1px solid #eee;flex-wrap:wrap">
              <div style="flex:1;min-width:140px">
                <p style="font-weight:600;font-size:0.85rem;margin:0">${(c.productos||{}).nombre||"—"} ${c.color||""} T${c.talla||""}</p>
              </div>
...
`})})),n.textContent=b}}catch(m){n.style.display="block",n.textContent="Error inesperado: "+m.message}finally{t.innerHTML=i,t.disabled=!1}}window.mlPublicarPreview=function(e){je(!0,e)};window.mlPublicarReal=function(e){confirm("¿Publicar TODAS las variantes activas de este producto en MercadoLibre? Esto creará nuevos items en tu cuenta.")&&je(!1,e)};window.descargarExcelTikTok=async function(e,t,o){const a=e.innerHTML;try{e.innerHTML="⏳ Generando...",e.disabled=!0;const n=await fetch(`${f}/tiktok/${t}`);if(!n.ok){const d=await n.text();throw new Error(`Error ${n.status}: ${d}`)}const i=await n.blob(),r=URL.createObjectURL(i),s=document.createElement("a");s.href=r,s.download=o,document.body.appendChild(s),s.click(),document.body.removeChild(s),URL.revokeObjectURL(r)}catch(n){alert("Error al descargar el archivo: "+n.message)}finally{e.innerHTML=a,e.disabled=!1}};async function Ke(){const e=document.getElementById("content");e.innerHTML=`
  ...
  `,document.getElementById("ml-titulo").addEventListener("input",function(){document.getElementById("ml-titulo-count").textContent=this.value.length}),window.mlGenerarPreview=async()=>{var l,c,p,m;const o=document.getElementById("ml-sku").value.trim().toUpperCase(),a=document.getElementById("ml-listing").value,n=document.getElementById("ml-titulo").value.trim(),i=document.getElementById("ml-precio").value.trim(),r=i?parseFloat(i):null;if(!o){alert("Ingresa el SKU del producto");return}if(!n){alert("Ingresa el título para ML");return}if(n.length>60){alert("El título no puede superar 60 caracteres");return}if(r!==null&&(isNaN(r)||r<=0)){alert("El precio debe ser mayor a 0");return}const s=document.getElementById("ml-btn-preview");s.textContent="⏳ Generando...",s.disabled=!0;let d=0;try{const u=await fetch(`${f}/ml/publicar`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sku_interno:o,listing_type:a,solo_preview:!0})}),g=await u.json();if(!u.ok){alert("Error: "+(g.detail||JSON.stringify(g)));return}const y=g.resultados||[];if(!y.length){alert("No se encontraron variantes activas");return}const b=document.getElementById("ml-descripcion"),h=((p=(c=(l=y[0])==null?void 0:l.preview)==null?void 0:c.description)==null?void 0:p.plain_text)||"";b.value.trim()||(b.value=h);const v=new Set,w=[];for(const x of y)for(const z of((m=x.preview)==null?void 0:m.pictures)||[])z.source&&!v.has(z.source)&&(v.add(z.source),w.push(z));if(w.length>=1){const x=document.getElementById("ml-fotos-wrap"),z=document.getElementById("ml-fotos-grid");z.innerHTML=w.map((_,E)=>`
        ...
        `).join(""),x.style.display="block",window.mlSeleccionarPortada=_=>{d=_,w.forEach((E,P)=>{var C;const M=document.getElementById(`ml-foto-thumb-${P}`);if(M&&(M.style.border=P===_?"3px solid #3483fa":"3px solid #ddd",(C=M.querySelector("div"))==null||C.remove(),P===_)){const S=document.createElement("div");S.style.cssText="position:absolute;bottom:0;left:0;right:0;background:#3483fa;color:#fff;font-size:0.6rem;text-align:center;padding:2px",S.textContent="PORTADA",M.appendChild(S)}}),t(y,n,r,d,b.value.trim(),w)}}t(y,n,r,d,b.value.trim());const I=document.getElementById("ml-variantes-wrap");I.style.display="block",I.scrollIntoView({behavior:"smooth",block:"start"})}catch(u){alert("Error de conexión: "+u.message)}finally{s.textContent="🔍 Generar preview",s.disabled=!1}};function t(o,a,n,i,r,s){const d=document.getElementById("ml-variantes-titulo"),l=document.getElementById("ml-variantes-list");d.textContent=`Paso 2 — Revisa y edita (${o.length} variantes, título aplicado a todas)`,l.innerHTML=o.map((c,p)=>{const m={...c.preview,family_name:a};if(delete m.title,n!==null&&(m.price=n),r&&(m.description={plain_text:r}),s&&s.length>0){let u=[...s];if(i>0&&i<u.length){const[g]=u.splice(i,1);u.unshift(g)}m.pictures=u.slice(0,12)}return`
  ...
  `,await Qe(),Y=setInterval(async()=>{document.getElementById("ga-activos")?await Pe():(clearInterval(Y),Y=null)},3e4)}async function Qe(){await Promise.all([Pe(),et(),tt()]),document.getElementById("ga-ultima-act").textContent="Actualizado: "+new Date().toLocaleTimeString("es-MX",{hour:"2-digit",minute:"2-digit",second:"2-digit"})}async function Pe(){var e;try{const o=await(await fetch(`${f}/analytics/tiempo-real`)).json();if(!o.configurado){ot(o);return}const a=document.getElementById("ga-activos");if(!a)return;a.textContent=o.activos_ahora??0;const n=document.getElementById("ga-dispositivos");n&&o.por_dispositivo&&(n.textContent=Object.entries(o.por_dispositivo).map(([r,s])=>`${r}: ${s}`).join(" · "));const i=document.getElementById("ga-paginas-rt");i&&((e=o.por_pais)!=null&&e.length)?i.innerHTML='<div style="font-size:0.65rem;color:#aaa;margin-bottom:4px;text-transform:uppercase;letter-spacing:.05em">Por país</div>'+o.por_pais.map(r=>`
