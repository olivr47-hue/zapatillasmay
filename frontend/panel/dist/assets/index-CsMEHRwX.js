              ...
              <div class="orden-item" data-urgente="${b.urgente}" style="padding:1rem 1.5rem;border-bottom:1px solid #f5f5f5;display:flex;align-items:center;gap:16px;flex-wrap:wrap">
                ...
                <div style="flex:1;min-width:140px">
                  ...
                  <div style="margin-top:6px;display:flex;flex-wrap:wrap;gap:4px">
                    ${b.variantes.map(x=>{const v=[x.talla,x.color].filter(Boolean).join(" / "),$=x.sin_stock;return`<span style="
...
¿Quieres reactivar todos?`)&&(localStorage.removeItem("ordenes_pospuestos"),Y())};window.guardarOrdenCompra2=async()=>{var r,i,s,d;const{seleccionados:e,sucursalId:t}=window._ordenModal||{};if(!e)return;const a=(r=document.getElementById("orden-fecha"))==null?void 0:r.value,o=((i=document.getElementById("orden-notas"))==null?void 0:i.value)||"",n={};e.forEach(l=>{var u,g;const c=l.proveedor_id||"sin-proveedor",p=((u=l.proveedor)==null?void 0:u.nombre)||"Sin proveedor";n[c]||(n[c]={nombre:p,items:[]});const m=l.variantes||[];if(m.length===0){const y=parseInt(((g=document.getElementById("qty-orden-"+l.producto_id))==null?void 0:g.value)||l.cantidad_sugerida);n[c].items.push({variante_id:null,cantidad:y,costo_unitario:l.costo_unitario})}else m.forEach(y=>{var v;const b=document.getElementById("chk-var-"+y.id);if(!(b!=null&&b.checked))return;const x=parseInt(((v=document.getElementById("qty-var-"+y.id))==null?void 0:v.value)||0);x<=0||n[c].items.push({variante_id:y.id,cantidad:x,costo_unitario:l.costo_unitario})})});try{for(const[l,c]of Object.entries(n)){if(c.items.length===0)continue;const p=c.items.reduce((y,b)=>y+b.cantidad*b.costo_unitario,0),g=(s=(await(await fetch(f+"/finanzas/ordenes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({proveedor_id:l==="sin-proveedor"?null:l,sucursal_id:t,status:"borrador",total:p,notas:o,fecha_entrega_estimada:a||null})})).json())[0])==null?void 0:s.id;if(g)for(const y of c.items)await fetch(f+"/finanzas/ordenes/"+g+"/items",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...y,subtotal:y.cantidad*y.costo_unitario})})}(d=document.getElementById("modal-orden"))==null||d.remove(),alert("Orden de compra guardada"),Y()}catch(l){alert("Error guardando orden: "+l.message)}};function je(){return`
  ...
  `}async function Q(){var t,a,o,n,r,i;const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:#888">Cargando finanzas...</p>';try{const d=await(await fetch(f+"/sucursales/")).json(),l=(t=d[0])==null?void 0:t.id,[c,p,m,u,g,y,b]=await Promise.all([fetch(f+"/finanzas/caja/hoy/"+l),fetch(f+"/finanzas/reporte/"+l),fetch(f+"/finanzas/gastos/"+l),fetch(f+"/finanzas/estado-resultados/"+l),fetch(f+"/finanzas/flujo/"+l),fetch(f+"/finanzas/cuentas-por-cobrar"),fetch(f+"/finanzas/gastos-categorias/"+l)]),x=await c.json(),v=await p.json(),$=await m.json(),k=await u.json(),h=await g.json(),z=await y.json(),_=await b.json(),w=x.find(C=>C.status==="abierta"),T=new Date().toISOString().split("T")[0],M=$.filter(C=>{var j;return(j=C.created_at)==null?void 0:j.startsWith(T)}),S=M.reduce((C,j)=>C+parseFloat(j.monto||0),0),I=z.reduce((C,j)=>C+parseFloat(j.total||0),0);e.innerHTML=`
      <div style="margin-bottom:1.5rem;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
        <div>
    ...
    `,window._finanzasData={sucursalId:l,gastosHoy:M,totalGastosHoy:S,estadoResultados:k,flujo:h,cxc:z,categorias:_,historial:[]},window._finanzasSucursalId=l;const E=await(await fetch(f+"/finanzas/caja/historial/"+l)).json();window._finanzasData.historial=E,mostrarTabFinanzas("gastos")}catch(s){e.innerHTML='<p style="padding:2rem;color:red">Error cargando finanzas: '+s.message+"</p>"}}window.mostrarTabFinanzas=e=>{var c,p,m,u,g,y,b,x,v,$,k,h,z;const{gastosHoy:t,totalGastosHoy:a,estadoResultados:o,flujo:n,cxc:r,categorias:i,historial:s,sucursalId:d}=window._finanzasData,l=document.getElementById("fin-tab-contenido");if(l)if(e==="gastos"){const _=i.reduce((w,T)=>w+T.total,0);l.innerHTML=`
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
        <div style="background:white;border-radius:12px;border:1px solid #eee;overflow:hidden">
          ...
          ${t.length===0?'<div style="padding:2rem;text-align:center;color:#888;font-size:0.85rem">Sin gastos hoy</div>':t.map(w=>`
              <div style="padding:0.75rem 1.5rem;border-bottom:1px solid #f5f5f5;display:flex;align-items:center;gap:12px">
                <div style="flex:1">
                  <p style="font-size:0.85rem;font-weight:600">${w.concepto}</p>
                  <p style="font-size:0.72rem;color:#888">${w.categoria} · ${new Date(w.created_at).toLocaleTimeString("es-MX",{hour:"2-digit",minute:"2-digit"})}</p>
                </div>
                <p style="font-weight:700;color:#c62828">-$${parseFloat(w.monto).toFixed(2)}</p>
                <button onclick="eliminarGasto('${w.id}')" style="background:none;border:none;color:#ccc;cursor:pointer;font-size:1.1rem">✕</button>
              </div>
            `).join("")}
        ...
        <div style="background:white;border-radius:12px;border:1px solid #eee;overflow:hidden">
          ...
          <div style="padding:1rem">
            ${i.length===0?'<p style="color:#888;text-align:center;padding:1rem">Sin gastos registrados</p>':i.map(w=>`
                <div style="margin-bottom:12px">
                  <div style="display:flex;justify-content:space-between;margin-bottom:4px">
                    <span style="font-size:0.82rem;font-weight:600;text-transform:capitalize">${w.categoria}</span>
                    <span style="font-size:0.82rem;color:#c62828;font-weight:700">$${w.total.toFixed(0)}</span>
                  </div>
                  <div style="background:#f5f5f5;border-radius:100px;height:8px;overflow:hidden">
                    <div style="background:#E91E8C;height:100%;width:${_>0?(w.total/_*100).toFixed(0):0}%;border-radius:100px;transition:width 0.5s"></div>
                  </div>
                </div>
              `).join("")}
          </div>
      ...
      <div style="background:white;border-radius:12px;border:1px solid #eee;overflow:hidden">
        ...
        <div style="padding:1rem 1.5rem;background:#f9f9f9;display:flex;gap:2rem;flex-wrap:wrap">
          <div>
            ...
            <p style="font-weight:700;color:#E91E8C">$${o.reduce((_,w)=>_+w.ventas,0).toFixed(0)}</p>
          </div>
          <div>
            <p style="font-size:0.72rem;color:#888">Total gastos 6 meses</p>
            <p style="font-weight:700;color:#c62828">$${o.reduce((_,w)=>_+w.gastos,0).toFixed(0)}</p>
          </div>
          <div>
            <p style="font-size:0.72rem;color:#888">Utilidad total</p>
            <p style="font-weight:700;color:#2e7d32">$${o.reduce((_,w)=>_+w.utilidad,0).toFixed(0)}</p>
          </div>
        </div>
      </div>
    `:e==="flujo"?l.innerHTML=`
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:1rem;margin-bottom:1rem">
        ...
        <div style="background:white;border-radius:12px;border:1px solid #eee;padding:1.5rem">
          ...
          <div style="display:flex;justify-content:space-between;padding:8px 0;margin-top:4px">
            <span style="font-size:0.85rem;font-weight:700">Neto</span>
            <span style="font-weight:700;color:${(((h=n.mes)==null?void 0:h.neto)||0)>=0?"#2e7d32":"#c62828"};font-size:1.1rem">$${(((z=n.mes)==null?void 0:z.neto)||0).toFixed(0)}</span>
          </div>
        </div>
      </div>
    `:e==="caja"?l.innerHTML=`
...
Diferencia: $${(n=i.diferencia)==null?void 0:n.toFixed(2)}`),Q()):alert("Error: "+JSON.stringify(i))}catch{alert("Error cerrando caja")}};window.agregarGasto=e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:1000;display:flex;align-items:center;justify-content:center;padding:1rem",t.innerHTML=`
  ...
  <button class="btn btn-secondary" style="padding:4px 10px;font-size:0.75rem;background:#ffebee;border-color:#c62828;color:#c62828" onclick="filtrarRotacion('rojo')">🔴 Sin movimiento</button>
    ...
    `,window._rotacionData=g}catch{e.innerHTML='<p style="padding:2rem;color:red">Error cargando análisis</p>'}}window.filtrarRotacion=e=>{document.querySelectorAll(".rotacion-item").forEach(t=>{t.style.display=e==="todos"||t.dataset.semaforo===e?"":"none"})};async function ye(){const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:#888">Cargando CRM...</p>';try{const[t,a,o]=await Promise.all([fetch(f+"/clientes/"),fetch(f+"/pedidos/"),fetch(f+"/crm/seguimientos/pendientes/todos")]),n=await t.json(),r=await a.json(),i=await o.json(),s=new Date,d=new Date(s-30*24*60*60*1e3),l=new Date(s-60*24*60*60*1e3),c=new Date(s-90*24*60*60*1e3),p=n.map(v=>{const $=r.filter(T=>T.cliente_id===v.id&&(T.status==="confirmado"||T.status==="pagado")),k=$.reduce((T,M)=>T+parseFloat(M.total||0),0),h=$.length>0?new Date($[0].created_at):null,z=h?Math.floor((s-h)/(1e3*60*60*24)):null,_=$.filter(T=>new Date(T.created_at)>=d).length;let w="nuevo";return $.length===0?w="nuevo":k>=5e3&&_>=1?w="vip":z>90?w="inactivo":z>30?w="riesgo":_>=2?w="frecuente":w="activo",{...v,totalGastado:k,ultimoPedido:h,diasSinComprar:z,pedidos30:_,segmento:w,totalPedidos:$.length}}),m=p.filter(v=>v.segmento==="vip"),u=p.filter(v=>v.segmento==="riesgo").sort((v,$)=>$.totalGastado-v.totalGastado),g=p.filter(v=>v.segmento==="inactivo").sort((v,$)=>$.totalGastado-v.totalGastado),y=[...p].sort((v,$)=>$.totalGastado-v.totalGastado).slice(0,10),b=r.filter(v=>new Date(v.created_at).toDateString()===s.toDateString()&&(v.status==="confirmado"||v.status==="pagado")).reduce((v,$)=>v+parseFloat($.total||0),0),x=r.filter(v=>new Date(v.created_at)>=d&&(v.status==="confirmado"||v.status==="pagado")).reduce((v,$)=>v+parseFloat($.total||0),0);e.innerHTML=`
      <div style="margin-bottom:1.5rem;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
        <div>
...
⚠️ Requiere que los productos estén en el catálogo de Meta Commerce.`;else if(d.style.background="#e8f5e9",d.style.borderColor="#a5d6a7",d.style.color="#333",a){let y;t==="personalizado"?y=(((g=document.getElementById("texto-personalizado"))==null?void 0:g.value)||"").replace("{nombre}","María"):y=a.mensaje("María"),d.textContent=y}}const l=document.getElementById("campana-count");l&&(l.textContent=r.length+" clientes");const c=document.getElementById("campana-lista");if(c){if(!r.length){c.innerHTML='<div style="padding:2rem;text-align:center;color:#888;font-size:0.85rem">No hay clientes en este segmento con teléfono registrado</div>';return}window._campanaFiltrados=r,window._campanaPlantillaId=t,c.innerHTML=r.map((y,b)=>{var k;let x;t==="personalizado"?x=(((k=document.getElementById("texto-personalizado"))==null?void 0:k.value)||"").replace("{nombre}",y.nombre.split(" ")[0]):x=a.mensaje(y.nombre.split(" ")[0]);const v=encodeURIComponent(x),$=(y.lada||"52")+y.telefono.replace(/\D/g,"");return`
    ...
    <div style="background:white;border-radius:16px;padding:2rem;max-width:440px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.3);text-align:center">
      ...
      <p style="font-size:0.82rem;color:#888">Conectando con WhatsApp Business</p>
        ...
        </div>`};window._campanaJobId=g;const x=setInterval(async()=>{try{const $=await(await fetch(f+"/campanas/estado/"+g)).json();b($),$.terminado&&(clearInterval(x),window._campanaUltimosResultados=$.resultados||[],setTimeout(()=>{const k=$.enviados||0,h=$.fallidos||0;l.innerHTML=`
    ...
    `};window._campanaAvanzar=()=>{if(i++,i>=r.length){s.innerHTML=`
      ...
      `;return}d()},d(),document.body.appendChild(s)};async function Pe(){const e=document.getElementById("content");try{const[t,a]=await Promise.all([fetch(f+"/clientes/"),fetch(f+"/pedidos/")]),o=await t.json(),n=await a.json(),r=new Date,i=new Date(r-30*24*60*60*1e3),s=new Date(r-90*24*60*60*1e3),d=o.map(u=>{const g=n.filter(_=>_.cliente_id===u.id&&(_.status==="confirmado"||_.status==="pagado")),y=g.reduce((_,w)=>_+parseFloat(w.total||0),0),b=g.length>0?new Date(g[0].created_at):null,x=g.filter(_=>new Date(_.created_at)>=i).length,v=b?Math.floor((r-b)/(1e3*60*60*24)):null;let $="nuevo",k="⚪ Nuevo",h="#f5f5f5",z="#888";return g.length===0?($="nuevo",k="⚪ Sin compras",h="#f5f5f5",z="#888"):y>=5e3&&x>=1?($="vip",k="⭐ VIP",h="#fff8e1",z="#f57f17"):v>90?($="inactivo",k="🔴 Inactivo",h="#ffebee",z="#c62828"):v>30?($="riesgo",k="🟡 En riesgo",h="#fff8e1",z="#f57f17"):x>=2?($="frecuente",k="🟢 Frecuente",h="#e8f5e9",z="#2e7d32"):($="activo",k="🔵 Activo",h="#e3f2fd",z="#1565c0"),{...u,totalGastado:y,ultimoPedido:b,pedidos30:x,diasSinComprar:v,segmento:$,segmentoLabel:k,segmentoBg:h,segmentoColor:z,totalPedidos:g.length}}).sort((u,g)=>g.totalGastado-u.totalGastado),l=d.filter(u=>u.segmento==="vip").length,c=d.filter(u=>u.segmento==="inactivo").length,p=d.filter(u=>u.segmento==="riesgo").length,m=d.filter(u=>u.segmento==="frecuente"||u.segmento==="activo").length;window._clientesData=d,e.innerHTML=`
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px;margin-bottom:1.5rem">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem">
        <div>
          ...
          <select class="form-input" id="f-categoria" required onchange="actualizarSKU()">
            ...
            ${Se.map(o=>`<option value="${o.value}" ${t.categoria===o.value?"selected":""}>${o.label}</option>`).join("")}
          </select>
        </div>
        <div>
...
${a.telefono?'<a href="https://wa.me/'+(a.lada||"52")+a.telefono.replace(/\D/g,"")+'" target="_blank" class="btn btn-secondary" style="background:#25D366;color:white;border-color:#25D366;margin-left:auto">WhatsApp</a>':""}      </div>
  ...
  `};window.guardarSalida=async()=>{const e=document.getElementById("sal").value,t=document.getElementById("sal-sucursal").value,a=document.getElementById("sal-cantidad").value,o=document.getElementById("sal-motivo").value;if(!e||!t||!a){alert("Por favor completa todos los campos");return}try{const r=await(await fetch(f+"/movimientos/entrada",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({variante_id:e,sucursal_id:t,cantidad:-parseInt(a),motivo:o})})).json();r.ok?(alert("Salida registrada. Anterior: "+r.cantidad_anterior+" pares ÔåÆ Nuevo: "+r.cantidad_nueva+" pares"),navegarA("inventario")):alert("Error: "+JSON.stringify(r))}catch{alert("Error conectando con el servidor")}};window.mostrarInventarioMasivo=async()=>{const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:#888">Cargando...</p>';try{const a=await(await fetch(f+"/sucursales/")).json(),n=await(await fetch(f+"/productos/")).json(),i=await(await fetch(f+"/variantes/")).json(),d=await(await fetch(f+"/inventario/")).json(),l=[...new Set(n.map(p=>p.categoria).filter(Boolean))],c=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"];window._invMasivo={sucursales:a,productos:n,variantes:i,inventario:d},e.innerHTML=`
    ...
    `,renderTablasMasivo()}catch{e.innerHTML='<p style="padding:2rem;color:red">Error cargando inventario</p>'}};window.renderTablasMasivo=()=>{const{productos:e,variantes:t,inventario:a}=window._invMasivo,o=document.getElementById("im-sucursal").value,n=document.getElementById("im-categoria").value,r=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],i=e.filter(c=>!(n&&c.categoria!==n)),s=a.filter(c=>c.sucursal_id===o),d=i.map(c=>{const p=t.filter(g=>g.producto_id===c.id);if(p.length===0)return"";const u=[...new Set(p.map(g=>g.color).filter(Boolean))].map(g=>{const y=p.filter(x=>x.color===g).sort((x,v)=>r.indexOf(x.talla)-r.indexOf(v.talla));return`
        <div style="margin-bottom:1rem">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            ${y.map(x=>{const v=s.find(z=>z.variante_id===x.id),$=v?v.cantidad:0,k=v?v.stock_minimo:3;let h="#ddd";return $===0?h="#ffcdd2":$<=k?h="#ffe082":h="#a5d6a7",`
                <div style="text-align:center">
  ...
  `,document.body.appendChild(c),c.addEventListener("click",p=>{p.target===c&&c.remove()}),window._posSeleccion={productoId:e,color:null},window._posBuffer={}};window.mostrarCorridaModalPOS=e=>{const{variantes:t,inventario:a}=window._posData,o=document.getElementById("pos-sucursal")?document.getElementById("pos-sucursal").value:"",n=a.filter(p=>p.sucursal_id===o),r=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],i=[...new Set(t.filter(p=>p.producto_id===e).map(p=>p.color).filter(Boolean))],s=document.querySelector(`button[onclick="mostrarCorridaModalPOS('${e}')"]`);s&&(s.style.display="none"),document.querySelectorAll("#pos-modal").forEach(p=>{p!==document.getElementById("pos-modal")&&p.remove()}),window._posSeleccion.color=null,window._corridaCantidades={};const d=document.getElementById("pos-tallas-panel");if(!d)return;const l=p=>{const m=t.filter(u=>u.producto_id===e&&u.color===p).sort((u,g)=>r.indexOf(u.talla)-r.indexOf(g.talla));m.filter(u=>{const g=n.find(y=>y.variante_id===u.id);return g&&g.cantidad>0}),d.innerHTML=`
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px">
        ${i.map(u=>{const g=t.find(v=>v.producto_id===e&&v.color===u),y=g?g.color_hex:"#888",b=g?g.foto_url:null,x=Object.entries(window._corridaCantidades).filter(([v])=>t.find($=>$.id===v&&$.color===u)).reduce((v,[,$])=>v+$,0);return`
...
<td>
  ...
  <button class="btn btn-secondary" style="padding:4px 8px;font-size:0.72rem;color:#c62828;border-color:#c62828" 
    ...
    Cancelar
        ...
        </tr>`}).join(""))};async function De(){try{const t=await(await fetch(f+"/pedidos/")).json(),o=await(await fetch(f+"/clientes/")).json(),r=await(await fetch(f+"/inventario/alertas")).json(),i=new Date;i.setHours(0,0,0,0);const s=new Date(i);s.setDate(s.getDate()-7);const d=new Date(i);d.setDate(d.getDate()-30);const l=t.filter(E=>E.status==="confirmado"||E.status==="pagado"),c=l.filter(E=>new Date(E.created_at)>=i),p=l.filter(E=>new Date(E.created_at)>=s),m=c.reduce((E,C)=>E+parseFloat(C.total||0),0),u=p.reduce((E,C)=>E+parseFloat(C.total||0),0),g=o.filter(E=>E.created_at&&new Date(E.created_at)>=d).length,y=["Dom","Lun","Mar","Mie","Jue","Vie","Sab"],b={};y.forEach(E=>b[E]=0),l.filter(E=>new Date(E.created_at)>=d).forEach(E=>{const C=y[new Date(E.created_at).getDay()];b[C]+=parseFloat(E.total||0)});const x={};l.forEach(E=>{x[E.canal||"sucursal"]=(x[E.canal||"sucursal"]||0)+parseFloat(E.total||0)});const v={};l.forEach(E=>{v[E.forma_pago||"efectivo"]=(v[E.forma_pago||"efectivo"]||0)+1});const $={};l.forEach(E=>{$[E.empleado||"Admin"]=($[E.empleado||"Admin"]||0)+parseFloat(E.total||0)});const k={};l.forEach(E=>{const C=new Date(E.created_at).toLocaleDateString("es-MX",{month:"short",year:"numeric"});k[C]=(k[C]||0)+parseFloat(E.total||0)});const h={};l.forEach(E=>{E.clientes&&(h[E.clientes.nombre]=(h[E.clientes.nombre]||0)+parseFloat(E.total||0))});const z=Object.entries(h).sort((E,C)=>C[1]-E[1]).slice(0,5),_=Object.entries(b).sort((E,C)=>C[1]-E[1])[0],w=Object.entries($).sort((E,C)=>C[1]-E[1])[0],T=document.getElementById("dashboard-contenido");if(!T)return;const M=T.querySelectorAll(".stat-card"),S=[{val:"$"+m.toFixed(0),sub:c.length+" pedidos hoy",color:"var(--pink)"},{val:c.length,sub:"confirmados hoy"},{val:"$"+u.toFixed(0),sub:p.length+" pedidos"},{val:g,sub:"ultimos 30 dias"},{val:r.length,sub:"por reabastecer",color:r.length>0?"var(--amber)":"var(--green)"},{val:_?_[0]:"—",sub:_?"$"+_[1].toFixed(0)+" prom.":""},{val:w?w[0]:"—",sub:w?"$"+w[1].toFixed(0):"",small:!0},{val:o.length,sub:"registrados"}];M.forEach((E,C)=>{if(!S[C])return;const j=E.querySelector(".stat-value"),B=E.querySelector(".stat-sub");j&&(j.textContent=S[C].val,j.style.color=S[C].color||"var(--text-primary)",S[C].small&&(j.style.fontSize="1rem")),B&&(B.textContent=S[C].sub||"")});const I=document.getElementById("dash-top-clientes");I&&(I.innerHTML=z.length===0?'<p style="color:var(--text-muted);font-size:0.85rem">Sin datos aun</p>':z.map(([E,C],j)=>`
...
<div>
  ...
  `,window.hacerLogin=async()=>{const t=document.getElementById("login-email").value,a=document.getElementById("login-password").value,o=document.getElementById("btn-login");if(document.getElementById("login-error"),!t||!a){e("Por favor completa todos los campos");return}o.textContent="Verificando...",o.disabled=!0;try{const n=await fetch("/api/empleados/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:t,password:a})}),r=await n.json();n.ok?(sessionStorage.setItem(ke,JSON.stringify(r)),r.token&&sessionStorage.setItem("erp_token",r.token),window._empleadoActual=r,ge()):(e(r.error||"Email o contrasena incorrectos"),o.textContent="Iniciar sesion",o.disabled=!1)}catch{e("Error conectando con el servidor"),o.textContent="Iniciar sesion",o.disabled=!1}};function e(t){const a=document.getElementById("login-error");a&&(a.textContent=t,a.style.display="block")}}window.authHeaders=()=>{const e=sessionStorage.getItem("erp_token");return e?{"Content-Type":"application/json",Authorization:`Bearer ${e}`}:{"Content-Type":"application/json"}};const pe=sessionStorage.getItem(ke);if(pe)try{window._empleadoActual=JSON.parse(pe),ge()}catch{ce()}else ce();
