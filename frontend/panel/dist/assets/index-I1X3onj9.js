  ...
  `,document.body.appendChild(o),o.addEventListener("click",a=>{a.target===o&&o.remove()})};window.guardarProveedor=async e=>{const t={nombre:document.getElementById("prov-nombre").value,contacto:document.getElementById("prov-contacto").value,telefono:document.getElementById("prov-telefono").value,email:document.getElementById("prov-email").value,ciudad:document.getElementById("prov-ciudad").value,direccion:document.getElementById("prov-direccion").value,notas:document.getElementById("prov-notas").value};if(!t.nombre){alert("El nombre es requerido");return}try{e?await fetch(f+"/finanzas/proveedores/"+e,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}):await fetch(f+"/finanzas/proveedores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}),document.querySelector('div[style*="position:fixed"]').remove(),je()}catch{alert("Error guardando proveedor")}};window.editarProveedor=async e=>{const a=(await(await fetch(f+"/finanzas/proveedores")).json()).find(n=>n.id===e);a&&mostrarFormProveedor(a)};async function tt(){const e=document.getElementById("content");e.innerHTML=`
    ...
    <style>@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}</style>`;try{let B=function(k,O,F){return`<div style="display:flex;align-items:center;gap:6px">
        ...
        <div id="rotacion-lista" style="display:flex;flex-direction:column;gap:8px">
          ${C.map(k=>{const O=S[k.semaforo],F=Math.max(...C.map(P=>P.ventas.d30),1);return`
        ...
        </div>`},M=function(k){k=(k||"").toLowerCase();const O=C.filter(A=>(A.tallasDesglosadas.length>0||A.tallasData.some(R=>R.total>0))&&(!k||A.nombre.toLowerCase().includes(k)||(A.sku_interno||"").toLowerCase().includes(k))),F=C.filter(A=>A.tallasData.some(R=>R.total>0)).length,P=`
      ...
      `},D=function(){const k=C.filter(O=>O.variantesData.some(F=>F.total>0));return k.length===0?'<div style="padding:3rem;text-align:center;color:#94a3b8;font-size:0.9rem">No hay datos de ventas por variante aún</div>':k.map(O=>{const F=O.variantesData.slice(0,8),P=Math.max(...F.map(A=>A.total),1),G=F[0];return`
        <div style="background:white;border:1px solid #e2e8f0;border-radius:12px;padding:16px;margin-bottom:10px">
        ...
        </div>`}).join("")};var t=B,o=_,a=M,n=D;const[i,r,s,l]=await Promise.all([fetch(f+"/productos/"),fetch(f+"/variantes/?activa=eq.true"),fetch(f+"/movimientos/"),fetch(f+"/inventario/slim")]),d=await i.json(),c=await r.json(),p=await s.json(),m=await l.json(),g={},u={};c.forEach(k=>{g[k.id]=k,u[k.producto_id]||(u[k.producto_id]=[]),u[k.producto_id].push(k.id)});const b={};m.forEach(k=>{b[k.variante_id]=(b[k.variante_id]||0)+k.cantidad});const y=new Date,v=new Date(y-30*24*60*60*1e3),h=new Date(y-60*24*60*60*1e3),$=new Date(y-90*24*60*60*1e3),E={},x={},j={},L={};p.filter(k=>k.tipo==="venta").forEach(k=>{const O=g[k.variante_id];if(!O)return;const F=O.producto_id,P=Math.abs(k.cantidad),G=new Date(k.created_at);E[F]||(E[F]={d30:0,d60:0,d90:0,total:0}),E[F].total+=P,G>=v&&(E[F].d30+=P),G>=h&&(E[F].d60+=P),G>=$&&(E[F].d90+=P),x[k.variante_id]||(x[k.variante_id]={d30:0,d90:0,total:0}),x[k.variante_id].total+=P,G>=v&&(x[k.variante_id].d30+=P),G>=$&&(x[k.variante_id].d90+=P);const A=O.talla||"S/T",R=`${F}|${A}`;j[R]||(j[R]={talla:A,d30:0,d90:0,total:0}),j[R].total+=P,G>=v&&(j[R].d30+=P),G>=$&&(j[R].d90+=P);const V=O.color||"Sin color",H=`${F}|${V}|${A}`;L[H]||(L[H]={talla:A,color:V,d30:0,d90:0,total:0}),L[H].total+=P,G>=v&&(L[H].d30+=P),G>=$&&(L[H].d90+=P)});const C=d.map(k=>{const O=E[k.id]||{d30:0,d60:0,d90:0,total:0},F=u[k.id]||[],P=F.reduce((N,U)=>N+(b[U]||0),0),G=O.d30/4,A=G>0?Math.round(P/G*7):null;let R="gris",V="Sin ventas recientes";O.d30>=6?(R="verde",V="Rota bien — considerar resurtido"):O.d30>=2?(R="amarillo",V="Rotación moderada"):O.d90===0&&P>0?(R="rojo",V="Sin movimiento en 90 días — revisar"):O.d30>0&&(R="amarillo",V="Rotación lenta");const H=F.map(N=>{var U;return(U=g[N])==null?void 0:U.talla}).filter(Boolean).filter((N,U,Z)=>Z.indexOf(N)===U).map(N=>({talla:N,...j[`${k.id}|${N}`]||{d30:0,d90:0,total:0}})).sort((N,U)=>U.total-N.total),X=[...new Set(F.map(N=>{var U;return(U=g[N])==null?void 0:U.color}).filter(Boolean))].map(N=>{const U=F.filter(J=>{var Y;return((Y=g[J])==null?void 0:Y.color)===N}).map(J=>{var Y;return(Y=g[J])==null?void 0:Y.talla}).filter(Boolean).filter((J,Y,We)=>We.indexOf(J)===Y).map(J=>({talla:J,...L[`${k.id}|${N}|${J}`]||{d30:0,d90:0,total:0}})).sort((J,Y)=>parseFloat(J.talla)-parseFloat(Y.talla)),Z=U.reduce((J,Y)=>J+Y.total,0);return{color:N,tallas:U,total:Z}}).filter(N=>N.total>0).sort((N,U)=>U.total-N.total),Q=F.map(N=>{const U=g[N];if(!U)return null;const Z=x[N]||{d30:0,d90:0,total:0},J=b[N]||0;return{id:N,label:[U.talla,U.color].filter(Boolean).join(" / ")||N,stock:J,...Z}}).filter(Boolean).sort((N,U)=>U.total-N.total);return{...k,ventas:O,stockTotal:P,ventasSemana:G,diasInventario:A,semaforo:R,recomendacion:V,tallasData:H,tallasDesglosadas:X,variantesData:Q}}).sort((k,O)=>O.ventas.d30-k.ventas.d30);window._analisisData={productosConRotacion:C,ventasPorTalla:j};const q=C.reduce((k,O)=>k+O.ventas.d30,0),z=C.reduce((k,O)=>k+O.ventas.d90,0),T=C.filter(k=>k.semaforo==="verde").length,I=C.filter(k=>k.semaforo==="rojo").length,S={verde:{bg:"#dcfce7",color:"#166534",dot:"#16a34a",txt:"Rota bien"},amarillo:{bg:"#fef9c3",color:"#854d0e",dot:"#ca8a04",txt:"Rotación lenta"},rojo:{bg:"#fee2e2",color:"#991b1b",dot:"#dc2626",txt:"Sin movimiento"},gris:{bg:"#f1f5f9",color:"#64748b",dot:"#94a3b8",txt:"Sin datos"}};window._renderTabRotacion=_,window._renderTabTallas=M,window._renderTabVariantes=D,e.innerHTML=`
...
${o.telefono?'<a href="https://wa.me/'+(o.lada||"52")+o.telefono.replace(/\D/g,"")+'" target="_blank" class="btn btn-secondary" style="background:#25D366;color:white;border-color:#25D366;margin-left:auto">WhatsApp</a>':""}      </div>
  ...
  `).join(""),window.recalcularTotal()}};window.cambiarCantidadItem=async(e,t)=>{if(t>0){const o=window._pedidoItems[e],a=document.getElementById("ped-sucursal")?document.getElementById("ped-sucursal").value:"";if(a)try{const r=(await(await fetch(f+"/inventario/sucursal/"+a)).json()).find(l=>l.variante_id===o.variante_id),s=r?r.cantidad:0;if(o.cantidad>=s){alert("No hay mas existencia disponible. Maximo: "+s+" pares");return}}catch{}}window._pedidoItems[e].cantidad=Math.max(1,window._pedidoItems[e].cantidad+t),window.recalcularTotal(),renderItemsPedido()};window.guardarPedido=async()=>{var l;if(window._creandoPedido)return;const e=document.getElementById("ped-cliente").value,t=document.getElementById("ped-canal").value,o=document.getElementById("ped-sucursal").value,a=document.getElementById("ped-pago").value,n=document.getElementById("ped-comentarios").value;if(!e){alert("Selecciona un cliente");return}if(window._pedidoItems.length===0){alert("Agrega al menos un producto");return}window._creandoPedido=!0;const i=document.getElementById("btn-ped-guardar");i&&(i.textContent="Guardando...",i.disabled=!0);const r=window._pedidoItems.reduce((d,c)=>d+c.cantidad*c.precio_unitario,0),s=a==="spei"?"pendiente_pago":"confirmado";try{const d=await fetch(f+"/pedidos/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({cliente_id:e,canal:t,sucursal_id:o,forma_pago:a,comentarios:n||null,total:r,subtotal:r,status:s})});if(!d.ok){alert("Error creando pedido"),i&&(i.textContent="Crear pedido",i.disabled=!1),window._creandoPedido=!1;return}const c=await d.json(),p=c.id||((l=c[0])==null?void 0:l.id);if(!p){alert("Error: no se obtuvo ID del pedido"),i&&(i.textContent="Crear pedido",i.disabled=!1),window._creandoPedido=!1;return}for(const m of window._pedidoItems)await fetch(f+"/pedidos/"+p+"/items",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({variante_id:m.variante_id,cantidad:m.cantidad,precio_unitario:m.precio_unitario,subtotal:m.cantidad*m.precio_unitario})});a!=="spei"&&a!=="mercadopago"&&await fetch(f+"/pedidos/"+p+"/confirmar",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({forma_pago:a})}),alert("Pedido creado correctamente"),window._pedidoItems=[],window._creandoPedido=!1,verPedido(p)}catch{alert("Error conectando con el servidor"),i&&(i.textContent="Crear pedido",i.disabled=!1),window._creandoPedido=!1}};window.abrirPreviewPedido=async e=>{const t=document.getElementById("pedido-preview-panel");if(!t){verPedido(e);return}if(t.dataset.pedidoId===e&&t.style.display!=="none"){t.style.display="none",t.dataset.pedidoId="";return}t.dataset.pedidoId=e,t.style.display="block",t.innerHTML='<p style="padding:1.5rem;color:#888;text-align:center">Cargando...</p>',t.scrollIntoView({behavior:"smooth",block:"nearest"});try{const[o,a]=await Promise.all([fetch(f+"/pedidos/"+e).then(l=>l.json()),fetch(f+"/pedidos/"+e+"/items").then(l=>l.json())]),n=Array.isArray(o)?o[0]:o,i=Array.isArray(a)?a:[],r=n.clientes||{},s={confirmado:"#2e7d32",pagado:"#2e7d32",cancelado:"#c62828",pendiente_pago:"#f57f17",borrador:"#f57f17"}[n.status]||"#888";t.innerHTML=`
      ...
      <div class="table-card" style="padding:2rem">
        ...
        ${(()=>{const l=(n.direccion_envio||n.direccion_entrega||"").trim();let d="";if(!l&&n.notas){const u=n.notas.match(/Env[ií]o a:\s*(.+)$/i);u&&(d=u[1].trim())}const c=l||d,p=r.nombre||n.nombre_cliente||"",m=r.telefono||n.telefono_cliente||"";return!c&&!p&&!m?"":`
          <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:1rem;margin-bottom:1.5rem">
            <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:8px">
              ...
              <button onclick="navigator.clipboard.writeText(this.dataset.copy).then(()=>{this.textContent='✓ Copiado';setTimeout(()=>this.textContent='📋 Copiar',1500)})" data-copy="${[p,m,c].filter(Boolean).join(`
...
<td>
  ...
  <button class="btn btn-secondary" style="padding:4px 8px;font-size:0.72rem;color:#c62828;border-color:#c62828" 
    ...
    Cancelar
        ...
        </tr>`}).join(""))};async function rt(){try{const t=await(await fetch(f+"/pedidos/")).json(),a=await(await fetch(f+"/clientes/")).json(),i=await(await fetch(f+"/inventario/alertas")).json(),r=new Date;r.setHours(0,0,0,0);const s=new Date(r);s.setDate(s.getDate()-7);const l=new Date(r);l.setDate(l.getDate()-30);const d=t.filter(_=>["confirmado","pagado","enviado"].includes(_.status)),c=d.filter(_=>new Date(_.created_at)>=r),p=d.filter(_=>new Date(_.created_at)>=s),m=c.reduce((_,M)=>_+parseFloat(M.total||0),0),g=p.reduce((_,M)=>_+parseFloat(M.total||0),0),u=a.filter(_=>_.created_at&&new Date(_.created_at)>=l).length,b=["Dom","Lun","Mar","Mie","Jue","Vie","Sab"],y={};b.forEach(_=>y[_]=0),d.filter(_=>new Date(_.created_at)>=l).forEach(_=>{const M=b[new Date(_.created_at).getDay()];y[M]+=parseFloat(_.total||0)});const v={};d.forEach(_=>{v[_.canal||"sucursal"]=(v[_.canal||"sucursal"]||0)+parseFloat(_.total||0)});const h={};d.forEach(_=>{h[_.forma_pago||"efectivo"]=(h[_.forma_pago||"efectivo"]||0)+1});const $={};d.forEach(_=>{$[_.empleado||"Admin"]=($[_.empleado||"Admin"]||0)+parseFloat(_.total||0)});const E={};d.forEach(_=>{const M=new Date(_.created_at).toLocaleDateString("es-MX",{month:"short",year:"numeric"});E[M]=(E[M]||0)+parseFloat(_.total||0)});const x={};d.forEach(_=>{_.clientes&&(x[_.clientes.nombre]=(x[_.clientes.nombre]||0)+parseFloat(_.total||0))});const j=Object.entries(x).sort((_,M)=>M[1]-_[1]).slice(0,5),L=Object.entries(y).sort((_,M)=>M[1]-_[1])[0],w=Object.entries($).sort((_,M)=>M[1]-_[1])[0];if(!document.getElementById("dashboard-contenido"))return;const q=d.filter(_=>new Date(_.created_at)>=l),z=q.reduce((_,M)=>_+parseFloat(M.total||0),0),T=[];for(let _=6;_>=0;_--){const M=new Date(r);M.setDate(M.getDate()-_);const D=M.toISOString().split("T")[0],k=M.toLocaleDateString("es-MX",{weekday:"short",day:"numeric"}),O=d.filter(F=>{var P;return(P=F.created_at)==null?void 0:P.startsWith(D)}).reduce((F,P)=>F+parseFloat(P.total||0),0);T.push({label:k,total:O,ds:D})}const I=(_,M,D,k)=>{const O=document.getElementById(_),F=document.getElementById(_+"-sub");O&&(O.textContent=M,k&&(O.style.color=k)),F&&D&&(F.textContent=D)};I("kpi-ventas-hoy","$"+m.toLocaleString("es-MX",{maximumFractionDigits:0}),c.length+" pedidos","#C8967A"),I("kpi-pedidos-hoy",c.length,"confirmados"),I("kpi-ventas-7d","$"+g.toLocaleString("es-MX",{maximumFractionDigits:0}),p.length+" pedidos"),I("kpi-ventas-30d","$"+z.toLocaleString("es-MX",{maximumFractionDigits:0}),q.length+" pedidos"),I("kpi-clientes-nuevos",u,"últimos 30 días"),I("kpi-stock-bajo",i.length,i.length>0?"⚠ reabastecer":"✓ ok",i.length>0?"#f59e0b":"#10b981"),I("kpi-mejor-dia",L?L[0]:"—",L?"$"+L[1].toLocaleString("es-MX",{maximumFractionDigits:0}):""),I("kpi-total-clientes",a.length,"registrados");const S=document.getElementById("dash-top-clientes");S&&(S.innerHTML=j.length===0?'<p style="color:var(--text-muted);font-size:0.85rem">Sin datos aun</p>':j.map(([_,M],D)=>`
