    ...
    <div class="main">
      <div class="topbar">
        <div style="display:flex;align-items:center;gap:1rem">
          ...
          <h1 id="topbar-title">${((o=te.find(n=>n.id===K))==null?void 0:o.label)||"Dashboard"}</h1>
        </div>
...
`,renderInventario()}catch{e.innerHTML='<p style="padding:2rem;color:red">Error conectando con el servidor</p>'}}window.renderInventario=()=>{const{sucursales:e,productos:t,variantes:a,inventario:o}=window._invData,n=(document.getElementById("inv-buscar")?document.getElementById("inv-buscar").value:"").toLowerCase(),r=document.getElementById("inv-categoria")?document.getElementById("inv-categoria").value:"",i=document.getElementById("inv-talla")?document.getElementById("inv-talla").value:"",s=document.getElementById("inv-estado")?document.getElementById("inv-estado").value:"",d=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],l=t.filter(m=>!(!m.activo||n&&!m.nombre.toLowerCase().includes(n)&&!(m.sku_interno||"").toLowerCase().includes(n)||r&&m.categoria!==r)),c=e.map(m=>{const u=o.filter(y=>y.sucursal_id===m.id),g=l.map(y=>{const b=a.filter(k=>k.producto_id===y.id);if(b.length===0)return"";const v=[...new Set(b.map(k=>k.color).filter(Boolean))].map(k=>{const x=b.filter(T=>T.color===k).sort((T,j)=>d.indexOf(T.talla)-d.indexOf(j.talla));if(i&&!x.find(T=>T.talla===i))return"";const C=x[0]?x[0].color_hex:"#888",_=x[0]?x[0].foto_url:null,$=x.map(T=>{const j=u.find(z=>z.variante_id===T.id),S=j?j.cantidad:null,I=j?j.stock_minimo:3;if(i&&T.talla!==i||s&&(s==="agotado"&&S!==0||s==="bajo"&&(S===null||S===0||S>I)||s==="disponible"&&(S===null||S===0||S<=I)))return"";let M,E;return S===null?(M="#f0f0f0",E="#aaa"):S===0?(M="#ffebee",E="#c62828"):S<=I?(M="#fff8e1",E="#f57f17"):(M="#e8f5e9",E="#2e7d32"),`
  ...
  `};window.guardarCambio=async()=>{const e=document.getElementById("cam-origen").value,t=document.getElementById("cam-destino").value,a=document.getElementById("cam-sucursal").value,o=document.getElementById("cam-motivo").value;if(!e||!t||!a){alert("Por favor selecciona ambos productos y la sucursal");return}if(e===t){alert("El producto que regresa y el que se lleva deben ser diferentes");return}try{const r=await(await fetch(f+"/movimientos/cambio",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({variante_origen_id:e,variante_destino_id:t,sucursal_id:a,motivo:o})})).json();r.ok?(alert("Cambio registrado. El inventario se actualizo automaticamente."),navegarA("inventario")):alert("Error: "+JSON.stringify(r))}catch{alert("Error conectando con el servidor")}};window.buscarVariante=(e,t)=>{const a=window._variantesCache||[],o=document.getElementById(t+"-resultados");if(!o)return;if(!e||e.length<2){o.style.display="none";return}const n=e.toLowerCase().split(" ").filter(s=>s),r=a.filter(s=>{const d=(s.productos&&s.productos.nombre||"").toLowerCase(),l=(s.color||"").toLowerCase(),c=(s.talla||"").toLowerCase(),p=(s.sku||"").toLowerCase(),m=d+" "+l+" "+c+" "+p;return n.every(u=>m.includes(u))}).slice(0,15);if(r.length===0){o.innerHTML='<div style="padding:10px 14px;color:#888;font-size:0.85rem">No se encontraron resultados</div>',o.style.display="block";return}const i=t==="ped-prod";o.innerHTML=r.map(s=>{const d=(s.productos&&s.productos.nombre||"")+" - "+s.color+" - T"+s.talla;return`
    ...
    `}).join(""),o.style.display="block"};window.seleccionarVariante=(e,t,a)=>{const o=document.getElementById(a);o&&(o.value=e);const n=document.getElementById(a+"-seleccionado");n&&(n.textContent="Ô£ô "+t,n.style.display="block");const r=document.getElementById(a+"-resultados");r&&(r.style.display="none")};function de(e,t){const a=t||{},o=!t;let n="";a.imagenes&&a.imagenes.length>0?n=a.imagenes.map((s,d)=>{const l=d===0;return`<div style="position:relative;cursor:pointer" data-url="${s}" data-es-portada="${l}" data-file-idx="${d}">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem">
        <div>
          ...
          <select class="form-input" id="f-categoria" required onchange="actualizarSKU()">
            ...
            ${Me.map(o=>`<option value="${o.value}" ${t.categoria===o.value?"selected":""}>${o.label}</option>`).join("")}
          </select>
        </div>
        <div>
...
<div id="variantes-container">
  ${window._coloresExistentes&&window._coloresExistentes.length>0?window._coloresExistentes.map((o,n)=>de(n,o)).join(""):de(0,null)}
</div>
...
${a.telefono?'<a href="https://wa.me/'+(a.lada||"52")+a.telefono.replace(/\D/g,"")+'" target="_blank" class="btn btn-secondary" style="background:#25D366;color:white;border-color:#25D366;margin-left:auto">WhatsApp</a>':""}      </div>
  ...
  `};window.guardarCliente=async e=>{const t=document.getElementById("cli-nombre").value;if(!t){alert("El nombre del cliente es obligatorio");return}const a=document.getElementById("btn-cli-guardar");a&&(a.textContent="Guardando...",a.disabled=!0);const o={nombre:t,telefono:document.getElementById("cli-telefono").value||null,email:document.getElementById("cli-email").value||null,tipo:document.getElementById("cli-tipo").value,direccion:document.getElementById("cli-direccion").value||null,lada:document.getElementById("cli-lada").value||"52",ciudad:document.getElementById("cli-ciudad").value||null,estado:document.getElementById("cli-estado").value||null,codigo_postal:document.getElementById("cli-cp").value||null,limite_credito:parseFloat(document.getElementById("cli-credito").value)||0,dias_credito:parseInt(document.getElementById("cli-dias").value)||0,comentarios_internos:document.getElementById("cli-comentarios").value||null,activo:!0};try{const n=e?"PATCH":"POST",r=e?f+"/clientes/"+e:f+"/clientes/",i=await fetch(r,{method:n,headers:{"Content-Type":"application/json"},body:JSON.stringify(o)});if(i.ok)alert("Cliente guardado correctamente"),navegarA("clientes");else{const s=await i.text();alert("Error al guardar: "+s),a&&(a.textContent="Guardar cliente",a.disabled=!1)}}catch{alert("Error conectando con el servidor"),a&&(a.textContent="Guardar cliente",a.disabled=!1)}};window.verCliente=async e=>{const t=document.getElementById("content");t.innerHTML='<p style="padding:2rem;color:#888">Cargando...</p>';try{const[a,o]=await Promise.all([fetch(f+"/clientes/"+e),fetch(f+"/pedidos/")]),n=await a.json(),r=await o.json();if(!n||n.length===0){alert("Cliente no encontrado");return}const i=n[0],s=r.filter(g=>g.cliente_id===e),d=s.filter(g=>g.status==="confirmado"||g.status==="pagado"),l=d.reduce((g,y)=>g+parseFloat(y.total||0),0),c=d.length>0?l/d.length:0,p=s.length>0?new Date(s[0].created_at):null,m=p?Math.floor((new Date-p)/(1e3*60*60*24)):null,u={};d.forEach(g=>{const y=new Date(g.created_at).toLocaleDateString("es-MX",{month:"short",year:"2-digit"});u[y]=(u[y]||0)+parseFloat(g.total||0)}),t.innerHTML=`
    ...
    `}catch{t.innerHTML='<p style="padding:2rem;color:red">Error cargando cliente</p>'}};window.guardarNotasCliente=async e=>{var a;const t=((a=document.getElementById("cli-notas-"+e))==null?void 0:a.value)||"";try{await fetch(f+"/clientes/"+e,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({comentarios_internos:t})});const o=document.querySelector(`button[onclick="guardarNotasCliente('${e}')"]`);o&&(o.textContent="✅ Guardado",setTimeout(()=>o.textContent="💾 Guardar notas",2e3))}catch{alert("Error guardando notas")}};window.editarCliente=e=>{mostrarFormCliente(e)};window.nuevoPedidoCliente=async(e,t)=>{await ke(),setTimeout(()=>{const a=document.getElementById("pos-cliente-buscar"),o=document.getElementById("pos-cliente"),n=document.getElementById("pos-cliente-seleccionado");a&&(a.value=""),o&&(o.value=e),n&&(n.textContent="✔ "+t+" — toca para cambiar",n.style.display="block");const r=document.getElementById("topbar-title");r&&(r.textContent="Punto de venta")},300)};window.verHistorialCliente=async e=>{const t=document.getElementById("content");t.innerHTML='<p style="padding:2rem;color:#888">Cargando historial...</p>';try{const n=(await(await fetch(f+"/pedidos/")).json()).filter(s=>s.cliente_id===e),r=n.length>0&&n[0].clientes?n[0].clientes:{},i=n.filter(s=>s.status==="confirmado"||s.status==="pagado").reduce((s,d)=>s+parseFloat(d.total||0),0);t.innerHTML=`
      <div class="table-card" style="padding:2rem">
        <div style="display:flex;align-items:center;gap:1rem;margin-bottom:1.5rem;flex-wrap:wrap">
          <button class="btn btn-secondary" onclick="navegarA('clientes')">← Volver</button>
  ...
  `};window.guardarTraspaso=async()=>{const e=document.getElementById("tra").value,t=document.getElementById("tra-origen").value,a=document.getElementById("tra-destino").value,o=document.getElementById("tra-cantidad").value;if(!e||!t||!a||!o){alert("Por favor completa todos los campos");return}if(t===a){alert("La sucursal origen y destino no pueden ser la misma");return}try{const r=await(await fetch(f+"/movimientos/traspaso",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({variante_id:e,sucursal_origen_id:t,sucursal_destino_id:a,cantidad:parseInt(o)})})).json();r.ok?(alert("Traspaso realizado correctamente. Se movieron "+r.cantidad_movida+" pares."),navegarA("inventario")):alert("Error: "+(r.error||JSON.stringify(r)))}catch{alert("Error conectando con el servidor")}};async function Ce(){const e=document.getElementById("content");try{const a=await(await fetch(f+"/pedidos/")).json();e.innerHTML=`
      <div style="margin-bottom:1rem;display:flex;gap:8px;flex-wrap:wrap;align-items:center">
  ...
  <button class="btn btn-secondary" style="padding:4px 8px;font-size:0.72rem;color:#c62828;border-color:#c62828" 
    ...
    Cancelar
      ...
      ${z.length===0?'<p style="color:#aaa;font-size:0.85rem;text-align:center;padding:1rem">Sin tareas pendientes</p>':z.map(B=>`
        ...
        `).join("")}
    `,document.getElementById("dashboard-contenido").appendChild(P)}catch(E){console.error("tareas:",E)}}catch(e){console.error("Error dashboard:",e)}}window.eliminarItemPedido=e=>{window._pedidoItems.splice(e,1),window.recalcularTotal(),window.renderItemsPedido()};window.cerrarSesionPanel=()=>{confirm("Cerrar sesion?")&&(sessionStorage.removeItem("erp_empleado"),sessionStorage.removeItem("erp_token"),window._empleadoActual=null,location.reload())};async function Se(){const e=document.getElementById("content");try{const a=await(await fetch(f+"/empleados/",{headers:window.authHeaders()})).json();e.innerHTML=`
      <div class="table-card">
        <div class="table-header">
  ...
  <div style="display:flex;gap:1rem;flex-wrap:wrap">
    ...
    <div class="table-card">
      ...
      <div style="display:flex;border-bottom:2px solid #e5e7eb;margin-bottom:20px;gap:4px">
        <button style="${pe(o,"subir")}" onclick="switchTabCat('subir')">📁 Subir imágenes</button>
        <button style="${pe(o,"seleccionar")}" onclick="switchTabCat('seleccionar')">🖼 De la tienda</button>
        <button style="${pe(o,"generar")}" onclick="switchTabCat('generar')">✨ Generar automático</button>
      </div>
...
`})})),n.textContent=b}}catch(m){n.style.display="block",n.textContent="Error inesperado: "+m.message}finally{t.innerHTML=r,t.disabled=!1}}window.mlPublicarPreview=function(e){je(!0,e)};window.mlPublicarReal=function(e){confirm("¿Publicar TODAS las variantes activas de este producto en MercadoLibre? Esto creará nuevos items en tu cuenta.")&&je(!1,e)};window.descargarExcelTikTok=async function(e,t,a){const o=e.innerHTML;try{e.innerHTML="⏳ Generando...",e.disabled=!0;const n=await fetch(`${f}/tiktok/${t}`);if(!n.ok){const d=await n.text();throw new Error(`Error ${n.status}: ${d}`)}const r=await n.blob(),i=URL.createObjectURL(r),s=document.createElement("a");s.href=i,s.download=a,document.body.appendChild(s),s.click(),document.body.removeChild(s),URL.revokeObjectURL(i)}catch(n){alert("Error al descargar el archivo: "+n.message)}finally{e.innerHTML=o,e.disabled=!1}};async function Je(){const e=document.getElementById("content");e.innerHTML=`
  ...
  `;try{F=(await(await fetch(`${f}/productos/`)).json()).filter(o=>o.activo).sort((o,n)=>{const r=o.orden_home??99999,i=n.orden_home??99999;return r!==i?r-i:new Date(n.created_at)-new Date(o.created_at)}),Q="",ne()}catch(t){document.getElementById("oh-lista").innerHTML=`<p style="color:red">Error: ${t.message}</p>`}}window._ohFiltrar=function(e){Q=e.toLowerCase().trim(),ne()};function ne(){const e=document.getElementById("oh-lista"),t=document.getElementById("oh-count");if(!e)return;const a=F.length,o=Q?F.filter(n=>(n.nombre||"").toLowerCase().includes(Q)||(n.sku_interno||"").toLowerCase().includes(Q)):F;if(t&&(t.textContent=Q?`${o.length} de ${a} modelos`:`${a} modelos activos`),!a){e.innerHTML='<p style="color:#888;text-align:center;padding:2rem">No hay productos activos</p>';return}if(!o.length){e.innerHTML='<p style="color:#aaa;text-align:center;padding:1.5rem">Sin resultados para esa búsqueda</p>';return}e.innerHTML=o.map(n=>{const r=F.indexOf(n),i=r+1,s=n.foto_principal||n.imagenes&&n.imagenes[0]||"",d=s?`<img class="oh-img" src="${s}" alt="" loading="lazy" onerror="this.style.display='none'">`:'<div class="oh-img" style="display:flex;align-items:center;justify-content:center;font-size:1.1rem">👠</div>';return`
      <div class="oh-row" id="oh-row-${r}" draggable="true"
        ...
        <input class="oh-pos-input" type="number" min="1" max="${a}" value="${i}"
  ...
  `,await Ze(),Y=setInterval(async()=>{document.getElementById("ga-activos")?await Pe():(clearInterval(Y),Y=null)},3e4)}async function Ze(){await Promise.all([Pe(),Ye(),Qe()]),document.getElementById("ga-ultima-act").textContent="Actualizado: "+new Date().toLocaleTimeString("es-MX",{hour:"2-digit",minute:"2-digit",second:"2-digit"})}async function Pe(){try{const t=await(await fetch(`${f}/analytics/tiempo-real`)).json();if(!t.configurado){et(t);return}const a=document.getElementById("ga-activos");if(!a)return;a.textContent=t.activos_ahora??0;const o=document.getElementById("ga-dispositivos");o&&t.por_dispositivo&&(o.textContent=Object.entries(t.por_dispositivo).map(([n,r])=>`${n}: ${r}`).join(" · "))}catch(e){console.warn("GA realtime:",e.message)}}async function Ye(){var e;try{const a=await(await fetch(`${f}/analytics/hoy`)).json();if(!a.configurado)return;const o=(i,s)=>{const d=document.getElementById(i);d&&(d.textContent=s)};o("ga-sesiones",a.sesiones??"—"),o("ga-usuarios",a.usuarios_activos??"—"),o("ga-pageviews",a.paginas_vistas??"—"),o("ga-nuevos",a.usuarios_nuevos??"—");const n=a.duracion_promedio_s;o("ga-duracion",n?`${Math.floor(n/60)}m ${n%60}s`:"—"),o("ga-rebote",a.tasa_rebote!=null?`${a.tasa_rebote}%`:"—");const r=document.getElementById("ga-top-paginas");r&&((e=a.top_paginas)!=null&&e.length)&&(r.innerHTML=a.top_paginas.map(i=>`
        ...
        </div>`).join(""))}catch(t){console.warn("GA hoy:",t.message)}}async function Qe(){var e;try{const a=await(await fetch(`${f}/analytics/semana`)).json();if(!a.configurado||!((e=a.dias)!=null&&e.length))return;const o=document.getElementById("ga-chart");if(!o||!window.Chart)return;o._chartInstance&&o._chartInstance.destroy(),o._chartInstance=new Chart(o,{type:"bar",data:{labels:a.dias.map(n=>n.fecha),datasets:[{label:"Sesiones",data:a.dias.map(n=>n.sesiones),backgroundColor:"rgba(52,131,250,0.7)",borderRadius:4},{label:"Usuarios",data:a.dias.map(n=>n.usuarios),backgroundColor:"rgba(34,197,94,0.5)",borderRadius:4}]},options:{responsive:!0,plugins:{legend:{position:"top"}},scales:{y:{beginAtZero:!0,ticks:{precision:0}}}}})}catch(t){console.warn("GA semana:",t.message)}}function et(e){const t=document.getElementById("ga-setup"),a=document.getElementById("ga-setup-body");!t||!a||(t.style.display="block",a.innerHTML=`
    ...
    </p>`,["ga-activos","ga-sesiones","ga-usuarios","ga-pageviews","ga-nuevos","ga-duracion","ga-rebote"].forEach(o=>{const n=document.getElementById(o);n&&(n.textContent="—")}))}document.querySelector("#app").style.cssText="display:flex;min-height:100vh;width:100%";const Be="erp_empleado";function fe(){document.querySelector("#app").innerHTML=`
  ...
  `,window.hacerLogin=async()=>{const t=document.getElementById("login-email").value,a=document.getElementById("login-password").value,o=document.getElementById("btn-login");if(document.getElementById("login-error"),!t||!a){e("Por favor completa todos los campos");return}o.textContent="Verificando...",o.disabled=!0;try{const n=await fetch("/api/empleados/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:t,password:a})}),r=await n.json();n.ok?(sessionStorage.setItem(Be,JSON.stringify(r)),r.token&&sessionStorage.setItem("erp_token",r.token),window._empleadoActual=r,he()):(e(r.error||"Email o contrasena incorrectos"),o.textContent="Iniciar sesion",o.disabled=!1)}catch{e("Error conectando con el servidor"),o.textContent="Iniciar sesion",o.disabled=!1}};function e(t){const a=document.getElementById("login-error");a&&(a.textContent=t,a.style.display="block")}}window.authHeaders=()=>{const e=sessionStorage.getItem("erp_token");return e?{"Content-Type":"application/json",Authorization:`Bearer ${e}`}:{"Content-Type":"application/json"}};const be=sessionStorage.getItem(Be);if(be)try{window._empleadoActual=JSON.parse(be),he()}catch{fe()}else fe();
