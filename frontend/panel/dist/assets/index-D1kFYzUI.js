...
${o.telefono?'<a href="https://wa.me/'+(o.lada||"52")+o.telefono.replace(/\D/g,"")+'" target="_blank" class="btn btn-secondary" style="background:#25D366;color:white;border-color:#25D366;margin-left:auto">WhatsApp</a>':""}      </div>
  ...
  `};window.guardarSalida=async()=>{const e=document.getElementById("sal").value,t=document.getElementById("sal-sucursal").value,o=document.getElementById("sal-cantidad").value,a=document.getElementById("sal-motivo").value;if(!e||!t||!o){alert("Por favor completa todos los campos");return}try{const i=await(await fetch(f+"/movimientos/entrada",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({variante_id:e,sucursal_id:t,cantidad:-parseInt(o),motivo:a})})).json();i.ok?(alert("Salida registrada. Anterior: "+i.cantidad_anterior+" pares ÔåÆ Nuevo: "+i.cantidad_nueva+" pares"),navegarA("inventario")):alert("Error: "+JSON.stringify(i))}catch{alert("Error conectando con el servidor")}};window.mostrarInventarioMasivo=async()=>{const e=document.getElementById("content");e.innerHTML='<p style="padding:2rem;color:#888">Cargando...</p>';try{const o=await(await fetch(f+"/sucursales/")).json(),n=await(await fetch(f+"/productos/")).json(),r=await(await fetch(f+"/variantes/")).json(),l=await(await fetch(f+"/inventario/")).json(),d=[...new Set(n.map(p=>p.categoria).filter(Boolean))],c=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"];window._invMasivo={sucursales:o,productos:n,variantes:r,inventario:l},e.innerHTML=`
    ...
    `,renderTablasMasivo()}catch{e.innerHTML='<p style="padding:2rem;color:red">Error cargando inventario</p>'}};window.renderTablasMasivo=()=>{const{productos:e,variantes:t,inventario:o}=window._invMasivo,a=document.getElementById("im-sucursal").value,n=document.getElementById("im-categoria").value,i=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],r=e.filter(c=>!(n&&c.categoria!==n)),s=o.filter(c=>c.sucursal_id===a),l=r.map(c=>{const p=t.filter(u=>u.producto_id===c.id);if(p.length===0)return"";const g=[...new Set(p.map(u=>u.color).filter(Boolean))].map(u=>{const b=p.filter(v=>v.color===u).sort((v,h)=>i.indexOf(v.talla)-i.indexOf(h.talla));return`
        <div style="margin-bottom:1rem">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            ${b.map(v=>{const h=s.find(P=>P.variante_id===v.id),$=h?h.cantidad:0,E=h?h.stock_minimo:3;let x="#ddd";return $===0?x="#ffcdd2":$<=E?x="#ffe082":x="#a5d6a7",`
                <div style="text-align:center">
  ...
  `,document.body.appendChild(o),o.addEventListener("click",a=>{a.target===o&&o.remove()})};window.confirmarEnvio=async function(e){const t=document.getElementById("env-paqueteria").value,o=document.getElementById("env-guia").value.trim(),a=document.getElementById("env-error"),n=document.getElementById("btn-confirmar-envio");if(!t||!o){a.textContent="Selecciona paquetería e ingresa el número de guía.",a.style.display="block";return}n.disabled=!0,n.textContent="Guardando...",a.style.display="none";try{const i=await fetch(f+`/pedidos/${e}/marcar-enviado`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({paqueteria:t,numero_guia:o})}),r=await i.json();if(!i.ok)throw new Error(r.error||"Error al guardar");try{const s=await fetch(f+"/pedidos/"+e).then(c=>c.json()),l=s.telefono_cliente,d=(s.nombre_cliente||"Cliente").split(" ")[0];l&&await fetch(f+"/chatbot/templates/enviar",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({telefono:l,template:"aviso_envio",params:[d,String(e).slice(-6),o,t]})})}catch{}document.getElementById("modal-envio").remove(),mostrarToast("✅ Pedido enviado — WhatsApp de tracking enviado al cliente"),await Oe()}catch(i){a.textContent=i.message,a.style.display="block",n.disabled=!1,n.textContent="Confirmar envío ✓"}};async function Oe(){const e=document.getElementById("content");try{const o=await(await fetch(f+"/pedidos/")).json(),a=new Date,n=new Date(a.getFullYear(),a.getMonth(),a.getDate()),i=new Date(a-7*24*60*60*1e3),r=o.filter(u=>u.status!=="cancelado"&&u.status!=="borrador"),s=r.filter(u=>new Date(u.created_at)>=n),l=s.reduce((u,b)=>u+parseFloat(b.total||0),0),d=r.filter(u=>new Date(u.created_at)>=i).reduce((u,b)=>u+parseFloat(b.total||0),0),c=o.filter(u=>u.status==="pendiente_pago").length,p=o.filter(u=>u.status==="pagado"&&u.mp_preference_id).length,m=o.filter(u=>u.forma_pago==="credito"&&u.status!=="cancelado").length,g=(u,b,y,v,h,$,E)=>`
      <div style="background:${h};border-radius:14px;padding:1.1rem 1.25rem;border:1px solid ${$};cursor:${E?"pointer":"default"};transition:transform 0.15s,box-shadow 0.15s"
        ...
        <p style="font-size:1.55rem;font-weight:800;color:${v};line-height:1;margin-bottom:3px;letter-spacing:-0.5px">${u}</p>
        <p style="font-size:0.7rem;font-weight:600;color:${v};opacity:0.85;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:1px">${b}</p>
      ...
      <div style="margin-bottom:1.25rem">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:10px;margin-bottom:16px">
          ${g(s.length,"Pedidos hoy",`$${l.toLocaleString("es-MX",{maximumFractionDigits:0})} vendidos`,"#E91E8C","linear-gradient(135deg,#fff0f8,#ffe4f2)","#f9a8d4","")}
          ${g("$"+Math.round(d/1e3)+"k","7 días",`${r.filter(u=>new Date(u.created_at)>=i).length} pedidos`,"#7c3aed","#f5f3ff","#ddd6fe","")}
          ...
          ${g(m,"En crédito","Pedidos activos","#0f766e","#f0fdfa","#99f6e4","cargarPedidosFiltro('credito')")}
        </div>
      ...
      <div class="table-card">
        <table>
          ...
          <tbody>
            ${o.length===0?'<tr><td colspan="8" style="text-align:center;color:#888;padding:2rem">No hay pedidos</td></tr>':o.map(u=>Le(u)).join("")}
          </tbody>
        </table>
      </div>
          ...
          ${(()=>{const m={};return a.forEach((g,u)=>{const b=g.variantes||{},y=b.productos||{},v=y.nombre||g.nombre||"—",h=b.color||g.color||"",$=!!g.es_corrida,E=$?"c|"+v+"|"+h:"s|"+g.id;m[E]||(m[E]={nombre:v,color:h,esCorrida:$,items:[],imagen:b.foto_url||y.imagen_principal||null}),m[E].items.push({...g,_idx:u})}),Object.values(m).map(g=>{const u=g.esCorrida,b=g.imagen;if(u){const y=g.items.reduce((E,x)=>E+x.cantidad,0),v=g.items.reduce((E,x)=>E+x.cantidad*x.precio_unitario,0),h=(v/y).toFixed(2),$=g.items.map(E=>E.id).join(",");return`
                  <div style="background:#fdf4ff;border-radius:8px;padding:12px;margin-bottom:8px;border:1px solid #e8d5f5">
                    ...
                    <div id="corrida-detalle-${$.replace(/,/g,"_").substring(0,20)}" style="display:none;margin-top:10px;border-top:1px solid #e8d5f5;padding-top:10px">
                      ${g.items.map(E=>{const P=(E.variantes||{}).talla||E.talla||"?",L=r.find(C=>C.variante_id===E.variante_id&&(s?C.sucursal_id===s:!0)),w=L?L.cantidad:999;return`
                          <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"
