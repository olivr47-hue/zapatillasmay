          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
            <div>
              ...
              <div style="display:flex;align-items:center;gap:8px">
                ...
                ${r>0?`<span class="wa-new-badge">${r}</span>`:""}
              </div>
            </div>
...
`).trim();if(n.match(/^https?:\/\/.+\.(jpg|jpeg|png|webp|gif)/i))return'<img src="'+n+'" style="max-width:200px;border-radius:8px;display:block">'+(i?'<p style="font-size:0.82rem;color:#333;white-space:pre-wrap;margin-top:6px">'+i+"</p>":"")}return'<p style="font-size:0.85rem;color:#333;white-space:pre-wrap">'+e.mensaje.replace(/\[.+?\]:\s*/,"")+"</p>"};window._renderBurbujas=e=>{const t=[...e.mensajes].reverse(),o=t.reduce((a,n,i)=>n.tipo==="manual"||n.tipo==="imagen_saliente"||n.tipo==="documento_saliente"||n.tipo==="video_saliente"?i:a,-1);return t.map((a,n)=>{var g;const i=a.tipo==="manual"||a.tipo==="imagen_saliente"||a.tipo==="documento_saliente"||a.tipo==="video_saliente",s=i?((g=a.mensaje.match(/\[(.+?)\]:/))==null?void 0:g[1])||"Admin":e.nombre||e.telefono,r=new Date(a.created_at).toLocaleTimeString("es-MX",{hour:"2-digit",minute:"2-digit"}),d=a.mensaje?a.mensaje.replace(/\[.+?\]:\s*/,""):"";let l="";if(a.tipo==="imagen_saliente"){const b=a.mensaje.replace(/\[.+?\]:\s*\[Imagen\]\s*/,"").split(`
`)[0].trim();l=b.match(/^https?:\/\/.+\.(jpg|jpeg|png|webp|gif)/i)?`<img src="${b}" style="max-width:200px;border-radius:8px;display:block;cursor:pointer" onclick="window.open('${b}')">`:`<p>${d}</p>`}else if(a.tipo==="documento_saliente"){const b=d.replace("[Documento] ","").split(" "),y=b[0]||"documento";l=`<a href="${b[1]||""}" target="_blank" class="wa-doc-link">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
        ${y}</a>`}else a.tipo==="video_saliente"?l=`<video src="${d.replace("[Video] ","")}" controls style="max-width:220px;border-radius:8px;display:block"></video>`:a.tipo==="imagen"?l='<p style="color:#64748b;font-size:0.8rem">[Imagen recibida]</p>':l=`<p>${d}</p>`;let c="";if(i&&n===o){const b=e.cliente_leyo_at?new Date(e.cliente_leyo_at):null,y=e.cliente_entrego_at?new Date(e.cliente_entrego_at):null,h=new Date(a.created_at);b&&b>=h?c='<span class="wa-read-receipt read" title="Visto">✓✓</span>':y&&y>=h?c='<span class="wa-read-receipt delivered" title="Entregado">✓✓</span>':c='<span class="wa-read-receipt sent" title="Enviado">✓</span>'}const p=`<button class="wa-reply-btn" onclick="iniciarReply('${e.telefono}','${(a.wa_message_id||"").replace(/'/g,"")}')" title="Responder">
