...
`).replace(/"/g,"&quot;")}" style="padding:4px 12px;border-radius:20px;border:1.5px solid #1d4ed8;background:none;color:#1d4ed8;font-size:0.75rem;font-weight:600;cursor:pointer">📋 Copiar</button>
  ...
  `).join("")};window.seleccionarProductoEdicion=(e,t)=>{var g;const o=window._editVariantes||[],a=window._editProductos||[],n=window._editInventario||[],i=((g=window._currentPedido)==null?void 0:g.sucursal_id)||"",r=a.find(u=>u.id===e);document.getElementById("edit-buscar-prod").value=`${r?r.nombre:""} · ${t}`,document.getElementById("edit-prod-resultados").style.display="none";const s=["22","22.5","23","23.5","24","24.5","25","25.5","26","26.5","27","Unica"],d=o.filter(u=>u.producto_id===e&&u.color===t).sort((u,y)=>s.indexOf(u.talla)-s.indexOf(y.talla));window._editSeleccionado={productoId:e,color:t,variantes:d,prod:r};const l=parseFloat(r?r.precio_menudeo:0)||0,c=parseFloat(r?r.precio_corrida:0)||(l>0?Math.round(l-100):0);document.getElementById("edit-nuevo-precio").value=l||"";const p=document.getElementById("edit-precios-info");p&&r&&(p.innerHTML=`
    ...
    `);const m=document.getElementById("edit-corrida-tallas-grid");m.innerHTML=d.map(u=>{const y=n.find(T=>T.variante_id===u.id&&(i?T.sucursal_id===i:!0)),f=y?y.cantidad:null,v=f!==null&&f===0,h=0,x=`edit-card-${u.id}`,$=`edit-qty-${u.id}`;return`
      <div id="${x}" class="size-card" style="display:flex;flex-direction:column;align-items:center;gap:6px;padding:10px 12px;border:2px solid ${v?"#fecaca":"#e2e8f0"};border-radius:12px;background:${v?"#fef2f2":"#ffffff"};min-width:70px;transition:all 0.2s ease;position:relative;opacity:${v?.6:1}">
        <span style="font-weight:800;font-size:0.9rem;color:#0f172a">T${u.talla}</span>
        <div style="display:flex;align-items:center;gap:4px">
          <button type="button" onclick="const inp=document.getElementById('${$}');inp.value=Math.max(0,parseInt(inp.value||0)-1);inp.dispatchEvent(new Event('input'))"
          ...
          <input type="number" value="${h}" min="0" max="${f!==null?f:999}" id="${$}" data-variante="${u.id}"
          ...
          <button type="button" onclick="const inp=document.getElementById('${$}');const max=parseInt(inp.max||999);const cur=parseInt(inp.value||0);if(cur<max){inp.value=cur+1;inp.dispatchEvent(new Event('input'))}else{inp.style.borderColor='#ef4444';setTimeout(()=>inp.style.borderColor='#cbd5e1',800)}"
            style="background:#f1f5f9;color:#475569;border:none;border-radius:6px;width:24px;height:24px;cursor:pointer;font-size:1rem;font-weight:700;display:flex;align-items:center;justify-content:center;transition:background 0.2s"
            onmouseenter="this.style.background='#e2e8f0'" onmouseleave="this.style.background='#f1f5f9'" ${v?"disabled":""}>+</button>
        </div>
          ...
          ${(()=>{const m={};return a.forEach((g,u)=>{const y=g.variantes||{},f=y.productos||{},v=f.nombre||g.nombre||"—",h=y.color||g.color||"",x=!!g.es_corrida,$=x?"c|"+v+"|"+h:"s|"+g.id;m[$]||(m[$]={nombre:v,color:h,esCorrida:x,items:[],imagen:y.foto_url||f.imagen_principal||null}),m[$].items.push({...g,_idx:u})}),Object.values(m).map(g=>{const u=g.esCorrida,y=g.imagen;if(u){const f=g.items.reduce(($,w)=>$+w.cantidad,0),v=g.items.reduce(($,w)=>$+w.cantidad*w.precio_unitario,0),h=(v/f).toFixed(2),x=g.items.map($=>$.id).join(",");return`
                  <div style="background:#fdf4ff;border-radius:8px;padding:12px;margin-bottom:8px;border:1px solid #e8d5f5">
                    ...
                    <div id="corrida-detalle-${x.replace(/,/g,"_").substring(0,20)}" style="display:none;margin-top:10px;border-top:1px solid #e8d5f5;padding-top:10px">
                      ${g.items.map($=>{const C=($.variantes||{}).talla||$.talla||"?",T=r.find(z=>z.variante_id===$.variante_id&&(s?z.sucursal_id===s:!0)),E=T?T.cantidad:999;return`
                          <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"
